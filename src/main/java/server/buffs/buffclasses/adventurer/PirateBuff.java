...
public class PirateBuff extends AbstractBuffClass {
	...
	public PirateBuff() {
		buffs = new int[] { 5101011, // Dark Clarity
	...
	public void handleBuff(MapleStatEffect eff, int skill) {
		...
		case 5221018: // Jolly Roger
			...
			eff.statups.put(MapleBuffStat.ABNORMAL_STATUS_R, eff.info.get(MapleStatInfo.y));// or
																							// x?
		...
		case 5221054: // Whaler's Potion
			eff.statups.put(MapleBuffStat.MAXHP, eff.info.get(MapleStatInfo.x)); // Max
																					// HP:
																					// +40%
			eff.statups.put(MapleBuffStat.ELEMENTAL_STATUS_R, eff.info.get(MapleStatInfo.y));// Status
																								// Ailment
																								// and
																								// Elemental
																								// Resistance:
																								// +15%
			eff.statups.put(MapleBuffStat.INVINCIBLE, eff.info.get(MapleStatInfo.w)); // Damage
																						// Intake:
																						// -15%
			break;
