...
public class DropRetriever {
	...
	private static boolean parsePage(final String url) {
		try {
			...
			if (currPage % 10 == 0) {
				...
				System.out.println("Monsters: " + parsedMobs + " || Monsters with drops: " + droppingMobs
						+ " || Monsters without drops: " + undroppingMobs + " || Items: " + drops.size()
						+ " || Errors: " + exceptions);
				System.out.println();
			}
	...
	public static void main(final String[] args) {
		...
		System.out.println("Total monsters parsed: " + parsedMobs + " || Total monsters with drops: " + droppingMobs
				+ "|| Total monsters without drops: " + undroppingMobs + " || Total items: " + drops.size()
				+ " || Total errors: " + exceptions);
		System.out.println("Data reading time: " + ((dataEndTime - startTime) / 1000)
				+ " seconds || SQL creation time: " + ((sqlEndTime - sqlStartTime) / 1000) + " seconds");
	...
	private static void parseMonsterSection(final String html_data) {
		try {
			parsedMobs++;
			int MonsterId = Integer.parseInt(getStringBetween(html_data, "alt=\"Mob:", "\" />")); // Will
																									// it
																									// blend?
																									// ;-)
