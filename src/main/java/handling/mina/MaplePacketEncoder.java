...
public class MaplePacketEncoder implements ProtocolEncoder {
	...
	public void encode(final IoSession session, final Object message, final ProtocolEncoderOutput out)
		...
		if (client != null) {
			...
			System.arraycopy(inputInitialPacket, 0, unencrypted, 0, inputInitialPacket.length); // Copy
																								// the
																								// input
																								// >
																								// "unencrypted"
			final byte[] ret = new byte[unencrypted.length + 4]; // Create new
																	// bytes
																	// with
																	// length =
																	// "unencrypted"
			...
			try {
				...
				MapleCustomEncryption.encryptData(unencrypted); // Encrypting
																// Data
				send_crypto.crypt(unencrypted); // Crypt it with IV
				System.arraycopy(header, 0, ret, 0, 4); // Copy the header >
														// "Ret", first 4 bytes
			} finally {
				mutex.unlock();
			}
			System.arraycopy(unencrypted, 0, ret, 4, unencrypted.length); // Copy
																			// the
																			// unencrypted
																			// >
																			// "ret"
