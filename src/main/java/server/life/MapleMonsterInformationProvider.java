...
public class MapleMonsterInformationProvider {
	...
	public void loadCustom() {
		globaldrops.add(new MonsterGlobalDropEntry(4001126, (int) (5 * 10000), -1, (byte) 0, 1, 3, 0)); // Maple
																										// Leaf
		// globaldrops.add(new MonsterGlobalDropEntry(4310050, (int) (1 *
		// 10000), -1, (byte) 0, 1, 1, 0)); //Old Maple Coin
		globaldrops.add(new MonsterGlobalDropEntry(2290285, (int) (0.5 * 10000), -1, (byte) 0, 1, 1, 0)); // Mystery
																											// Mastery
																											// Book
	...
	public void addMeso(MapleMonsterStats mons, ArrayList<MonsterDropEntry> ret) {
		...
		for (int i = 0; i < mons.dropsMeso(); i++) {
			ret.add(new MonsterDropEntry(0,
					mons.isBoss() && !mons.isPartyBonus() ? 1000000 : (mons.isPartyBonus() ? 600000 : 800000),
	...
	public List<Pair<Integer, String>> getAllMonsters() {
		final MapleDataProvider stringData = MapleDataProviderFactory
				.getDataProvider(new File(System.getProperty("wzpath") + "/String.wz"));
		...
		for (MapleData itemFolder : mobsData.getChildren()) {
			mobPairs.add(new Pair<>(Integer.parseInt(itemFolder.getName()),
					MapleDataTool.getString("name", itemFolder, "NO-NAME")));
		}
		return mobPairs;
	}

	public int chanceLogic(int itemId) { // not much logic in here. most of the
											// drops should already be there
											// anyway.
		if (GameConstants.getInventoryType(itemId) == MapleInventoryType.EQUIP) {
	...
	// no mesos if: mobid / 100000 == 97 or 95 or 93 or 91 or 90 or removeAfter
	// > 0 or invincible or onlyNormalAttack or friendly or dropitemperiod > 0
	// or cp > 0 or point > 0 or fixeddamage > 0 or selfd > 0 or mobType != null
	// and mobType.charat(0) == 7 or PDRate <= 0
}
