...
public class CommandProcessor {
					...
					try {
						if (!Modifier.isAbstract(c.getModifiers()) && !c.isSynthetic()) {
							...
							try {
								...
								enabled = true; // Enable all coded commands by
												// default.
							}
							if (o instanceof CommandExecute && enabled) {
								...
								commands.put(rankNeeded.getCommandPrefix() + c.getSimpleName().toLowerCase(),
										new MapleCommand((CommandExecute) o, rankNeeded.getLevel()));
								if (!rankNeeded.getCommandPrefix().equals(PlayerGMRank.GM.getCommandPrefix())
										&& !rankNeeded.getCommandPrefix()
												.equals(PlayerGMRank.NORMAL.getCommandPrefix())) { // add
																									// it
																									// again
																									// for
																									// GM
									commands.put("!" + c.getSimpleName().toLowerCase(),
											new MapleCommand((CommandExecute) o, PlayerGMRank.GM.getLevel()));
								}
							}
						}
	...
	public static boolean processCommand(MapleClient c, String line, CommandType type) {
		...
		if (String.valueOf(line.charAt(0)).equals(PlayerGMRank.NORMAL.getCommandPrefix())) {
			...
			try {
				int ret = co.execute(c, splitted); // Don't really care about
													// the return value. ;D
			} catch (Exception e) {
		...
		if (c.getPlayer().getGMLevel() > PlayerGMRank.NORMAL.getLevel()) {
			if (line.charAt(0) == '`' && c.getPlayer().getGMLevel() > 2) {
				for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
					cserv.broadcastGMMessage(tools.packet.CField.multiChat("[GM Chat] " + c.getPlayer().getName(),
							line.substring(1), 6));
				}
				return true;
			}
			if (line.split(" ")[0].equals("cmd")
					|| String.valueOf(line.charAt(0)).equals(PlayerGMRank.INTERN.getCommandPrefix())
					|| String.valueOf(line.charAt(0)).equals(PlayerGMRank.GM.getCommandPrefix())
					|| String.valueOf(line.charAt(0)).equals(PlayerGMRank.SUPERGM.getCommandPrefix())) { // Redundant
																											// for
																											// now,
																											// but
																											// in
				...
				if (c.getPlayer().getGMLevel() >= co.getReqGMLevel()) {
					...
					if (ret > 0 && c.getPlayer() != null) { // incase d/c after
															// command or
															// something
						if (c.getPlayer().isGM()) {
	...
	public static void logCommandToDB(MapleCharacter player, String command, String table) {
		...
		try {
			ps = DatabaseConnection.getConnection()
					.prepareStatement("INSERT INTO " + table + " (cid, command, mapid) VALUES (?, ?, ?)");
