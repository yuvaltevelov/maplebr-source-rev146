...
public class MapleStatEffect implements Serializable {
	...
	private byte expR, familiarTarget, recipeUseCount, recipeValidDay, reqSkillLevel, slotCount, effectedOnAlly,
			effectedOnEnemy, type, preventslip, immortal, bs;
	private short ignoreMob, mesoR, thaw, fatigueChange, lifeId, imhp, immp, inflation, useLevel, indiePdd, indieMdd,
			incPVPdamage, mobSkill, mobSkillLevel;
	private double hpR, mpR;
	private int sourceid, recipe, moveTo, moneyCon, morphId = 0, expinc, exp, consumeOnPickup, charColor, interval,
			rewardMeso, totalprob, cosmetic;
	...
	private static int parseEval(String path, MapleData source, int def, String variables, int level) {
		if (variables == null) {
			...
			} else if (dddd.substring(0, 1).equals("=")) { // lol nexon and
															// their mistakes
	...
	public static int parseEval(String data, int level) {
		...
		} else if (dddd.substring(0, 1).equals("=")) { // lol nexon and their
														// mistakes
			dddd = dddd.substring(1, dddd.length());
		}
		return (int) (new CaltechEval(dddd).evaluate());
	...
	private static MapleStatEffect loadFromData(final MapleData source, final int sourceid, final boolean skill,
		...
		if (ret.skill) {
			final int priceUnit = ret.info.get(MapleStatInfo.priceUnit); // Guild
																			// skills
			if (priceUnit > 0) {
			...
			// break;
			case 24100003: // TODO: for now, or could it be card stack? (1
							// count)
			case 24120002:
		...
		if (!ret.skill && ret.info.get(MapleStatInfo.time) > -1) {
			...
			ret.info.put(MapleStatInfo.time, (ret.info.get(MapleStatInfo.time) * 1000)); // items
																							// have
																							// their
																							// times
																							// stored
		...
		if (ret.overTime && ret.getSummonMovementType() == null && !ret.isEnergyCharge()) {
			...
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.SPEED,
					sourceid == 32120001 || sourceid == 32101003 ? ret.info.get(MapleStatInfo.x)
			...
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.DROP_RATE,
					GameConstants.getModifier(ret.sourceid, ret.itemup)); // defaults
																			// to
																			// 2x
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.MESO_RATE,
					GameConstants.getModifier(ret.sourceid, ret.mesoup)); // defaults
																			// to
																			// 2x
			...
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.MP_BOOST, Integer.valueOf(ret.immp)); // same
																											// one?
																											// lol
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.HP_BOOST_PERCENT,
					ret.info.get(MapleStatInfo.indieMhpR));
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.MP_BOOST_PERCENT,
					ret.info.get(MapleStatInfo.indieMmpR));
			...
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.ANGEL_JUMP,
					ret.info.get(MapleStatInfo.indieJump));
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.INDIE_SPEED,
			...
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.ANGEL_AVOID,
					ret.info.get(MapleStatInfo.indieEva));
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.ANGEL_STAT,
					ret.info.get(MapleStatInfo.indieAllStat));
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.ATTACK_SPEED,
					ret.info.get(MapleStatInfo.indieBooster));
			addBuffStatPairToListIfNotZero(ret.statups, MapleBuffStat.PVP_ATTACK,
			...
			if (sourceid == 5221006 || ret.isPirateMorph()) { // HACK: add
																// stance :D and
																// also this
																// buffstat has
																// to be the
		...
		if (ret.skill) { // hack because we can't get from the datafile...
			...
			if (!found) {
				...
				if (!handle) {
					...
					case 24111003:// Bad Luck Ward
						...
						// ret.statups.put(MapleBuffStat.ABNORMAL_STATUS_R,
						// ret.info.get(MapleStatInfo.x));
					...
					// case 51111004:
					// ret.statups.put(MapleBuffStat.ABNORMAL_STATUS_R,
					// ret.info.get(MapleStatInfo.y));
					// ret.statups.put(MapleBuffStat.ELEMENTAL_STATUS_R,
					// ret.info.get(MapleStatInfo.z));
					// ret.statups.put(MapleBuffStat.DEFENCE_BOOST_R,
					// ret.info.get(MapleStatInfo.x));
					// break;
					// case 51121006:
					// ret.statups.put(MapleBuffStat.DAMAGE_BUFF,
					// ret.info.get(MapleStatInfo.x));
					// ret.statups.put(MapleBuffStat.CRITICAL_RATE_BUFF,
					// ret.info.get(MapleStatInfo.criticaldamageMin.x));
					// ret.statups.put(MapleBuffStat.CRITICAL_RATE_BUFF,
					// ret.info.get(MapleStatInfo.criticaldamageMax.x));
					// break;
					case 2111008:
					case 2211008:
					case 12101005:
					...
					case 5220012:
						...
						ret.statups.put(MapleBuffStat.DAMAGE_RATE, (int) ret.info.get(MapleStatInfo.damR)); // i
																											// think
						break;
					case 5121009:
					case 15111005:
					...
					case 4321000: // tornado spin uses same buffstats
						...
						ret.statups.put(MapleBuffStat.DASH_JUMP, ret.info.get(MapleStatInfo.y)); // always
																									// 0
																									// but
																									// its
																									// there
					...
					case 3221002: // sharp eyes marksmen
						ret.statups.put(MapleBuffStat.SHARP_EYES,
								(ret.info.get(MapleStatInfo.x) << 8) + ret.info.get(MapleStatInfo.criticaldamageMax));
						break;
					...
					case 32111005: // body boost
						...
						ret.statups.put(MapleBuffStat.BODY_BOOST, (int) ret.level); // lots
																					// of
																					// variables
						break;
					case 22131002:
					...
					case 33121005:
						...
						ret.monsterStatus.put(MonsterStatus.MDEF, ret.info.get(MapleStatInfo.x)); // removed
																									// for
																									// taunt
						ret.monsterStatus.put(MonsterStatus.WDEF, ret.info.get(MapleStatInfo.x)); // removed
																									// for
																									// taunt
						break;
					case 31121003:
					...
					case 2221001:
						...
						ret.info.put(MapleStatInfo.time, ret.info.get(MapleStatInfo.time) * 2); // freezing
																								// skills
																								// are
																								// a
																								// little
					...
					case 5011002:
						...
						break;
					case 1121010: // enrage
						ret.statups.put(MapleBuffStat.ENRAGE,
								ret.info.get(MapleStatInfo.x) * 100 + ret.info.get(MapleStatInfo.mobCount));
						break;
					...
					case 31121005:
						...
						ret.statups.put(MapleBuffStat.DARK_METAMORPHOSIS, 6); // mob
																				// count
						break;
					case 2311003: // hs
					case 9001002: // GM hs
					...
					case 2321004: // Infinity
						...
						break;
					case 22181004:
						ret.statups.put(MapleBuffStat.ONYX_WILL, (int) ret.info.get(MapleStatInfo.damage)); // is
																											// this
																											// the
																											// right
																											// order
					...
					case 2321005: // holy shield, TODO JUMP
						// ret.statups.put(MapleBuffStat.HOLY_SHIELD,
						// GameConstants.GMS ? (int) ret.level :
						// ret.info.get(MapleStatInfo.x));
						ret.statups.put(MapleBuffStat.HOLY_SHIELD, ret.info.get(MapleStatInfo.x));
						ret.statups.put(MapleBuffStat.HP_BOOST, ret.info.get(MapleStatInfo.y));// fix
																								// names
					...
					case 35101009:
						...
						ret.statups.put(MapleBuffStat.MECH_CHANGE, level); // ya
																			// wtf
						break;
					...
					case 11121006:
						break;
					case 11121054:
						ret.statups.put(MapleBuffStat.DAMAGE_CAP_INCREASE,
								ret.info.get(MapleStatInfo.indieMaxDamageOver));
						break;
					case 4111002:
					case 4331002:
					case 15121004:
					...
					case 27111004:
						...
						break;
					case 27111006:
						ret.statups.put(MapleBuffStat.ENHANCED_MAXMP,
								Integer.valueOf(-((Integer) ret.info.get(MapleStatInfo.emad)).intValue()));
						break;
					case 30010242:
						ret.statups.put(MapleBuffStat.LUNAR_TIDE, Integer.valueOf(1));
						break;
					// case 27121052:
					// ret.monsterStatus.put(MonsterStatus.STUN,
					// Integer.valueOf(1));
					// case 27121054:
					// ret.statups.put(MapleBuffStat.LUMINOUS_GAUGE,
					...
					case 35121053:// for liberty mechanic
						...
						break;
					case 61101004:
						ret.statups.put(MapleBuffStat.BOOSTER,
								Integer.valueOf(-((Integer) ret.info.get(MapleStatInfo.x)).intValue()));
						break;
					case 61111003:
						ret.statups.put(MapleBuffStat.ABNORMAL_STATUS_R,
								Integer.valueOf(-((Integer) ret.info.get(MapleStatInfo.asrR)).intValue()));
						ret.statups.put(MapleBuffStat.ELEMENTAL_STATUS_R,
								Integer.valueOf(-((Integer) ret.info.get(MapleStatInfo.terR)).intValue()));
						break;
					case 60001216:
						ret.info.put(MapleStatInfo.time, Integer.valueOf(2100000000));
					...
					case 31211004: // Diabolic Recovery
						...
						// ret.statups.put(MapleBuffStat.RECOVERY,
						// ret.info.get(MapleStatInfo.x));
						break;
					case 31221004:
						ret.statups.put(MapleBuffStat.DAMAGE_PERCENT, ret.info.get(MapleStatInfo.indieDamR));
					...
					case 33121004:
						ret.statups.put(MapleBuffStat.SHARP_EYES,
								Integer.valueOf((((Integer) ret.info.get(MapleStatInfo.x)).intValue() << 8)
										+ ((Integer) ret.info.get(MapleStatInfo.criticaldamageMax)).intValue()));
						break;
					case 5321054:
					...
					case 13121004:// Touch of the Wind
						// ret.statups.put(MapleBuffStat.AVOID,
						// ret.info.get(MapleStatInfo.x));
						// ret.statups.put(MapleBuffStat.ACC,
						// ret.info.get(MapleStatInfo.y));
						// ret.statups.put(MapleBuffStat.HP_BOOST_PERCENT,
						// ret.info.get(MapleStatInfo.indieMhpR));
						// ret.statups.put(MapleBuffStat.ALBATROSS,
						// ret.info.get(MapleStatInfo.x));
						// statups.add(new Triple<MapleBuffStats, Integer,
						// Boolean>(MapleBuffStats.WN_BISS,
			...
			if (GameConstants.isBeginnerJob(sourceid / 10000)) {
				...
				// if (GameConstants.isBeginnerJob(sourceid / 10000)) {
				// switch (sourceid % 10000) {
				// //angelic blessing: HACK, we're actually supposed to use the
				// passives for atk/matk buff
				// case 1087:
				// ret.info.put(MapleStatInfo.time, 2100000000);
				// ret.statups.put(MapleBuffStat.INDIE_PAD, 10);
				// ret.statups.put(MapleBuffStat.INDIE_MAD, 10);
				...
				case 1010:
					...
					break;
				case 1001:
					if (sourceid / 10000 == 3001 || sourceid / 10000 == 3000) { // resistance
																				// is
																				// diff
				...
				case 8004:
					...
					break;
				// case 36121054:
				// ret.statups.put(MapleBuffStat.SUPPLY_SURPLUS,
				// Integer.valueOf(16));
				// break;
				case 8005:
				...
				case 104:
					...
					break;
				case 8002:
					ret.statups.put(MapleBuffStat.SHARP_EYES, (ret.info.get(MapleStatInfo.x) << 8)
	...
	public final void applyPassive(final MapleCharacter applyto, final MapleMapObject obj) {
		if (makeChanceResult() && !GameConstants.isDemonSlayer(applyto.getJob())) { // demon
																					// can't
																					// heal
																					// mp
				...
				if (!mob.getStats().isBoss()) {
					...
					if (absorbMp > 0) {
						...
						applyto.getMap().broadcastMessage(applyto,
								EffectPacket.showBuffeffect(applyto.getId(), sourceid, 1, applyto.getLevel(), level),
								false);
					}
				}
				break;
	...
	public final boolean applyTo(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary,
		...
		if (isHeal() && (applyfrom.getMapId() == 749040100 || applyto.getMapId() == 749040100)) {
			...
			applyfrom.getClient().getSession().write(CWvsContext.enableActions());
			return false;
		} else if (isShadow() && applyfrom.getJob() / 100 % 10 != 4) { // pirate/shadow
																		// = dc
			applyfrom.getClient().getSession().write(CWvsContext.enableActions());
			return false;
		...
		if (primary) {
			if (info.get(MapleStatInfo.itemConNo) != 0 && !applyto.isClone() && !applyto.inPVP()) {
				if (!applyto.haveItem(info.get(MapleStatInfo.itemCon), info.get(MapleStatInfo.itemConNo), false,
						true)) {
					applyto.getClient().getSession().write(CWvsContext.enableActions());
					return false;
				}
				MapleInventoryManipulator.removeById(applyto.getClient(),
						GameConstants.getInventoryType(info.get(MapleStatInfo.itemCon)),
						info.get(MapleStatInfo.itemCon), info.get(MapleStatInfo.itemConNo), false, true);
			}
		...
		if (mpchange != 0) {
			...
			if ((mpchange < 0 && GameConstants.isDemonSlayer(applyto.getJob()))
					|| !GameConstants.isDemonSlayer(applyto.getJob())) { // heal
		...
		if (expinc != 0) {
			...
			applyto.setExtractor(new MapleExtractor(applyto, sourceid, useLevel * 50, 1440)); // no
																								// clue
																								// about
																								// time
																								// left
			...
			if (effectedOnEnemy > 0) {
				...
				if (sourceid == 2910000 || sourceid == 2910001) { // red flag
					...
					applyto.getClient().getSession().write(EffectPacket
							.showOwnCraftingEffect("UI/UIWindow2.img/CTF/Effect", (byte) applyto.getDirection(), 0, 0));
					...
					if (applyto.getTeam() == (sourceid - 2910000)) { // restore
																		// duh
																		// flag
						if (sourceid == 2910000) {
						...
						if (sourceid == 2910000) {
							...
							applyto.getClient().getSession().write(EffectPacket.showOwnCraftingEffect(
									"UI/UIWindow2.img/CTF/Tail/Red", (byte) applyto.getDirection(), 600000, 0));
							applyto.getMap().broadcastMessage(
									applyto, EffectPacket.showCraftingEffect(applyto.getId(),
											"UI/UIWindow2.img/CTF/Tail/Red", (byte) applyto.getDirection(), 600000, 0),
									false);
						} else {
		...
		} else if (sourceid == 20031203 || sourceid == 20021110 || sourceid == 80001040) { // TODO:
																							// make
																							// them
																							// gms
																							// like
		...
		if (summonMovementType != null
			...
			if (isBeholder()) {
				...
				applyfrom.getClient().getSession().write(CWvsContext.enableActions()); // doubt
																						// we
																						// need
																						// this
																						// at
		...
		if (applyto.getJob() == 132) {
			if (applyto.getBuffedValue(MapleBuffStat.IGNORE_DEF) != 1)
				;
			{ // Sacrifice is the only skill Dark Knights have that give Ignore
				// Def hacky but works
		...
		if (GameConstants.isLuminous(applyto.getJob())) {
			if (applyto.getBuffedValue(MapleBuffStat.LUMINOUS_GAUGE) != 1)
				;
			{ // Sacrifice is the only skill Dark Knights have that give Ignore
				// Def hacky but works
				// World.Broadcast.broadcastMessage(CField.getGameMessage("Light?.",
				// (short) 8));
			}
		}
		...
		if (rewardItem != null && totalprob > 0) {
			for (Triple<Integer, Integer, Integer> reward : rewardItem) {
				if (MapleInventoryManipulator.checkSpace(applyto.getClient(), reward.left, reward.mid, "")
						&& reward.right > 0 && Randomizer.nextInt(totalprob) < reward.right) { // Total
																								// prob
		...
		if (familiarTarget == 2 && applyfrom.getParty() != null && primary) { // to
																				// party
	...
	public final boolean applyReturnScroll(final MapleCharacter applyto) {
		if (moveTo != -1) {
			if (applyto.getMap().getReturnMapId() != applyto.getMapId() || sourceid == 2031010 || sourceid == 2030021
					|| sourceid == 20021110 || sourceid == 2030028 || sourceid == 20031203) {
				MapleMap target;
				if (moveTo == 999999999) {
	...
	/*
	 * public final boolean applyReturnScroll(final MapleCharacter applyto) { if
	 * (moveTo != -1) { if (applyto.getMap().getReturnMapId() !=
	 * applyto.getMapId() || sourceid == 2031010 || sourceid == 2030021 ||
	 * sourceid == 20021110 || sourceid == 2030028 || sourceid == 20031203) {
	 * MapleMap target; if (moveTo == 999999999) { target =
	 * applyto.getMap().getReturnMap(); } else { target =
	 * ChannelServer.getInstance(applyto.getClient().getChannel()).getMapFactory
	 * ().getMap(moveTo); if (target.getId() / 10000000 != 60 &&
	 * applyto.getMapId() / 10000000 != 61) { if (target.getId() / 10000000 !=
	 * 21 && applyto.getMapId() / 10000000 != 20) { if (target.getId() /
	 * 10000000 != applyto.getMapId() / 10000000) { return false; } } } }
	 * applyto.changeMap(target, target.getPortal(0)); return true; } } return
	 * false; }
	 */
	...
	public final void handleExtraPVP(MapleCharacter applyfrom, MapleCharacter chr) {
		if (sourceid == 2311005 || sourceid == 5121005 || sourceid == 1201006
				|| (GameConstants.isBeginnerJob(sourceid / 10000) && sourceid % 10000 == 104)) { // doom,
																									// threaten,
																									// snatch
	...
	public final double getMaxDistanceSq() { // lt = infront of you, rb = behind
												// you; not gonna distanceSq the
												// two points since this is in
												// relative to player position
												// which is (0,0) and not both
												// directions, just one
	...
	public final void silentApplyBuff(final MapleCharacter chr, final long starttime, final int localDuration,
		...
		chr.registerEffect(this, starttime,
				BuffTimer.getInstance().schedule(new CancelEffectAction(chr, this, starttime, statup),
	...
	public final void applyEnergyBuff(final MapleCharacter applyto, final boolean infinity, int targets) {
		...
		if (infinity) {
			applyto.getClient().getSession()
					.write(BuffPacket.giveEnergyChargeTest(0, info.get(MapleStatInfo.time) / 1000, targets));
			applyto.registerEffect(this, starttime, null, applyto.getId());
		} else {
			...
			applyto.getMap().broadcastMessage(applyto,
					BuffPacket.giveEnergyChargeTest(applyto.getId(), 10000, info.get(MapleStatInfo.time) / 1000),
					false);
	...
	public void applyBuffEffect(final MapleCharacter applyfrom, final MapleCharacter applyto, final boolean primary,
		...
		case 42101002:
			if (applyto.getHaku() != null) {
				...
				applyto.getMap().broadcastMessage(applyto, CField.spawnHaku_bianshen(applyto.getId(),
						applyto.getHaku().getObjectId(), applyto.getHaku().getStats()), true);
			}
			break;
		case 13121004: // Touch of the Wind
			applyto.getClient().getSession().write(JobPacket.WindArcherPacket.giveWindArcherBuff(this.sourceid,
					localDuration, localstatups, this, applyto));
			normal = false;
			break;
		case 13120008:
		case 13111023:
			applyto.getClient().getSession().write(
					JobPacket.WindArcherPacket.giveWindArcherBuff(sourceid, nuffSkill, localstatups, this, applyto));
			normal = false;
			break;
		case 60001216:
		case 60001217:
		...
		case 100001263: { // Divine Force
			...
			// localstatups.put(MapleBuffStat.ELEMENT_RESIST,
			// info.get(MapleStatInfo.indieAsrR));
			applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid, localDuration, localstatups, this));
			break;
		}
		...
		case 15001022:// LIGHTNING Elemental ThunderBreaker
		{
			if (applyto.acaneAim > 0) {
				...
				applyto.getMap().broadcastMessage(applyto,
						BuffPacket.giveForeignBuff(applyto.getId(), localstatups, this), false);
				applyto.getClient().getSession()
		...
		case 36121054: {
			...
			// applyto.gainXenonSurplus((short) 10);
			applyfrom.getClient().getSession().write(CWvsContext.enableActions());
			// applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid,
		...
		case 4221013: {
			localstatups = new EnumMap<>(MapleBuffStat.class);
			localstatups.put(MapleBuffStat.INDIE_PAD,
		...
		case 20041239: {
			// World.Broadcast.broadcastMessage(CField.getGameMessage("Change2Light.",
			// (short) 8));
			applyto.getClient().getSession().write(JobPacket.LuminousPacket.giveLuminousState(20040220,
					applyto.getLightGauge(), applyto.getDarkGauge(), 10000));
			Timer.WorldTimer.getInstance().schedule(new Runnable() {
				@Override
				public void run() {
					applyto.dispelBuff(20040220);
					applyto.getClient().getSession().write(JobPacket.LuminousPacket.giveLuminousState(20040216,
							applyto.getLightGauge(), applyto.getDarkGauge(), 2000000000));
		...
		case 27121054: {
			// World.Broadcast.broadcastMessage(CField.getGameMessage("Equalize.",
			// (short) 8));
		...
		case 5311004: {
			...
			applyto.getMap().broadcastMessage(applyto,
					CField.EffectPacket.showDiceEffect(applyto.getId(), sourceid, zz, -1, level), false);
		...
		case 20031210:
			...
			applyto.getMap().broadcastMessage(applyto, PhantomPacket.gainCardStack(applyto.getId(),
					applyto.getRunningStack(), skillid == 24120002 ? 2 : 1, skillid, 0, skillid == 24100003 ? 5 : 10),
					true);
			applyto.getMap().broadcastMessage(applyto,
					CField.EffectPacket.showDiceEffect(applyto.getId(), this.sourceid, zz, -1, this.level), false);
			applyto.getClient().getSession()
					.write(CField.EffectPacket.showOwnDiceEffect(this.sourceid, zz, -1, this.level));
			localstatups = new EnumMap(MapleBuffStat.class);
			...
			applyto.getClient().getSession()
					.write(CWvsContext.BuffPacket.giveBuff(this.sourceid, localDuration, localstatups, this));
			applyfrom.getClient().getSession().write(CWvsContext.enableActions());
			normal = false;
			showEffect = false;
		...
		case 15001003: {
			...
			if (!applyto.isHidden()) {
				applyto.getMap().broadcastMessage(applyto,
						BuffPacket.giveForeignPirate(statups, localDuration / 1000, applyto.getId(), sourceid), false);
			}
			normal = false;
			break;
		}
		...
		case 23101003: {
			...
			// Integer>(MapleBuffStat.MAGIC_SHIELD, x));
			// applyto.getMap().broadcastMessage(applyto,
		...
		case 32111005: { // body boost
			...
			if (applyfrom.getStatForBuff(MapleBuffStat.DARK_AURA) != null) {
				sourcez = 32001003;
				statt = new Pair<>(MapleBuffStat.DARK_AURA, level + 10 + applyto.getTotalSkillLevel(sourcez)); // i
																												// think
			...
			/*
			 * ID: 110001501 NAME: Bear Mode ID: 110001502 NAME: Snow Leopard
			 * Mode ID: 110001503 NAME: Hawk Mode ID: 110001504 NAME: Cat Mode
			 */
		case 110001501:
		case 110001502:
		case 110001503:
		...
		case 32120000: { // adv dark aura
			...
			// statt.put(sourceid == 32110007 ? MapleBuffStat.BODY_BOOST :
			// MapleBuffStat.AURA, (int) (sourceid == 32120000 ?
			// applyfrom.getTotalSkillLevel(32001003) : level));
			applyto.getClient().getSession()
					.write(BuffPacket.giveBuff(sourceid == 32120000 ? 32001003 : sourceid, localDuration, statt, this));
			statt.clear();
		...
		case 32110000: { // advanced blue aura
			...
			applyto.cancelEffectFromBuffStat(MapleBuffStat.BODY_BOOST);
			final EnumMap<MapleBuffStat, Integer> statt = new EnumMap<>(MapleBuffStat.class);
			// statt.put(sourceid == 32110008 ? MapleBuffStat.BODY_BOOST :
			// MapleBuffStat.AURA, (int) (sourceid == 32110000 ?
			// applyfrom.getTotalSkillLevel(32111012) : level));
			applyto.getClient().getSession()
					.write(BuffPacket.giveBuff(sourceid == 32110000 ? 32111012 : sourceid, localDuration, statt, this));
			statt.clear();
		...
		case 32120001: { // advanced yellow aura
			...
			applyto.cancelEffectFromBuffStat(MapleBuffStat.BODY_BOOST);
			final EnumMap<MapleBuffStat, Integer> statt = new EnumMap<>(MapleBuffStat.class);
			// statt.put(sourceid == 32110009 ? MapleBuffStat.BODY_BOOST :
			// MapleBuffStat.AURA, (int) (sourceid == 32120001 ?
			// applyfrom.getTotalSkillLevel(32101003) : level));
			applyto.getClient().getSession()
					.write(BuffPacket.giveBuff(sourceid == 32120001 ? 32101003 : sourceid, localDuration, statt, this));
			statt.clear();
		...
		case 35111004: {// siege mode mech
			if (applyto.getBuffedValue(MapleBuffStat.MECH_CHANGE) != null
				...
				// SkillFactory.getSkill(35121013).getEffect(level).applyBuffEffect(applyfrom,
				// applyto, primary, newDuration);
				SkillFactory.getSkill(35121013).getEffect(applyto.getSkillLevel(35121013)).applyBuffEffect(applyfrom,
						applyto, primary, newDuration);
				normal = false;
		...
		case 35121013: { // adv siege
			normal = false;
			SkillFactory.getSkill(35121013).getEffect(applyto.getTotalSkillLevel(35111004)).applyBuffEffect(applyfrom,
					applyto, primary, newDuration);
			break;
		}
		case 35001001: // flame
		...
		case 4341052: // Asura By Mixtamal6
			...
			applyto.getClient().getSession()
					.write(CWvsContext.BuffPacket.giveBuff(this.sourceid, localDuration, localstatups, this));
			applyto.getClient().getSession().write(CWvsContext.BuffPacket.giveBuff(4341052, 60000, statups,
					SkillFactory.getSkill(4341052).getEffect(1)));
			applyto.getClient().getSession().write(CWvsContext.BuffPacket.giveForeignBuff(this.sourceid, statups,
					SkillFactory.getSkill(4341052).getEffect(1)));
			normal = false;
			break;
		...
		case 61121053: // hyper kaiser
		{
			...
			localstatups = new EnumMap<>(MapleBuffStat.class);
			applyto.resetKaiserCombo(); // Gets rid of Morph gauge <3
										// #MallyPower
			localstatups.put(MapleBuffStat.MORPH, info.get(MapleStatInfo.morph));
			applyto.getMap().broadcastMessage(applyto, BuffPacket.giveForeignBuff(applyto.getId(), localstatups, this),
					false);// Novak fixed dis lel
			applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid, localDuration, localstatups, this));
			normal = false;
		...
		case 4331003: { // Owl Spirit
			...
			applyto.getClient().getSession().write(BuffPacket.giveBuff(sourceid, localDuration, localstatups, this));
			applyto.setBattleshipHP(info.get(MapleStatInfo.x)); // a variable
																// that wouldnt'
																// be used by a
																// db
			normal = false;
		...
		case 31011001: {
			...
			/*
			 * case 24121054: {
			 * applyto.getClient().getSession().write(JobPacket.PhantomPacket.
			 * FinalJudgement());
			 * applyfrom.getClient().getSession().write(CWvsContext.
			 * enableActions()); break; }
			 */
		case 1321015: { // Sacrifice
			...
			// World.Broadcast.broadcastMessage(CField.getGameMessage("Sacrifice.",
			// (short) 5));
			applyto.addHP((int) ((applyto.getStat().getCurrentMaxHp() * (level / 100.0D)) * (getX() / 100.0D)));
			applyfrom.getClient().getSession().write(CWvsContext.enableActions());
			break;
		...
		case 41001001:
			if (applyfrom.getTotalSkillLevel(41110008) > 0) {
				SkillFactory.getSkill(41110008).getEffect(applyfrom.getTotalSkillLevel(41110008))
						.applyBuffEffect(applyfrom, applyto, primary, newDuration);
				return;
			}
			break;
		...
		case 35120014:
			...
			applyto.getMap().broadcastMessage(applyto, CField.EffectPacket.showDiceEffect(applyto.getId(),
					this.sourceid, zz, zz2 > 0 ? -1 : 0, this.level), false);
			applyto.getClient().getSession()
					.write(CField.EffectPacket.showOwnDiceEffect(this.sourceid, zz, zz2 > 0 ? -1 : 0, this.level));
			if ((zz <= 1) && (zz2 <= 1)) {
				return;
			}
			...
			if (buffid >= 100) {
				...
				applyto.dropMessage(-6,
						"[Double Lucky Dice] You have rolled two dice. (" + buffid / 10 + " and " + buffid % 10 + ")");
			}
			localstatups = new EnumMap(MapleBuffStat.class);
			localstatups.put(MapleBuffStat.DICE_ROLL, Integer.valueOf(buffid));
			applyto.getClient().getSession()
					.write(CWvsContext.BuffPacket.giveDice(zz, this.sourceid, localDuration, localstatups));
			normal = false;
			showEffect = false;
			break;
		// END novak added
		...
		default:
			if (isPirateMorph()) {
				...
				if ((mountid != 0) && (mountid2 != 0)) {
					...
					applyto.getClient().getSession()
							.write(CWvsContext.BuffPacket.giveMount(mountid, this.sourceid, stat));
					applyto.getMap().broadcastMessage(applyto,
							CWvsContext.BuffPacket.showMonsterRiding(applyto.getId(), stat, mountid, this.sourceid),
							false);
				} else {
					return;
				}
	...
	private int calcHPChange(final MapleCharacter applyfrom, final boolean primary) {
		...
		// actually receivers probably never get any hp when it's not heal but
		// whatever
		if (primary) {
	...
	private int calcMPChange(final MapleCharacter applyfrom, final boolean primary) {
		...
		if (info.get(MapleStatInfo.mp) != 0) {
			if (primary) {
				mpchange += alchemistModifyVal(applyfrom, info.get(MapleStatInfo.mp), false); // recovery
																								// up
																								// doesn't
																								// apply
																								// for
		...
		if (GameConstants.isDemonSlayer(applyfrom.getJob()) && GameConstants.isKanna(applyfrom.getJob())
				&& GameConstants.isZero(applyfrom.getJob())) {
			mpchange = 0;
		}
		if (primary) {
	...
	public final int calcPowerChange(final MapleCharacter applyfrom) {
		...
		if (info.get(MapleStatInfo.powerCon) != 0 && GameConstants.isXenon(applyfrom.getJob())) {
			// if (applyfrom.getBuffedValue(MapleBuffStat.AMARANTH_GENERATOR) !=
			// null) {
			// powerchange = 0;
			// } else {
	...
	public final boolean sameSource(final MapleStatEffect effect) {
		...
		switch (this.sourceid) { // All these are passive skills, will have to
	...
	public final int getCooldown(final MapleCharacter chra) {
		if (chra.getStat().coolTimeR > 0) {
			return Math.max(0, ((info.get(MapleStatInfo.cooltime) * (100 - (chra.getStat().coolTimeR / 100)))
					- chra.getStat().reduceCooltime));
		}
	...
	public final boolean isMonsterRiding() {
		...
		// GameConstants.getMountItem(sourceid, null) != 0/* ||
		// GameConstants.checkMountItem(sourceid) != 0*/);
	}
	...
	public boolean isMist() {
		return skill && (sourceid == 2111003 || sourceid == 4221006 || sourceid == 12111005 || sourceid == 14111006
				|| sourceid == 22161003 || sourceid == 32121006 || sourceid == 1076 || sourceid == 11076
				|| sourceid == 2311011 || sourceid == 4121015 || sourceid == 42111004 || sourceid == 42121005); // poison
																												// mist,
																												// smokescreen
																												// and
																												// flame
	...
	public final boolean isSoaring_Mount() {
		return skill && ((GameConstants.isBeginnerJob(sourceid / 10000) && sourceid % 10000 == 1142)
				|| sourceid == 80001089 || sourceid == 112111000);
	}
