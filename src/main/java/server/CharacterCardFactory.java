...
public class CharacterCardFactory {
	...
	private final Map<Integer, Integer> cardEffects = new HashMap<>(); // cardid,
																		// skillid
	private final Map<Integer, List<Integer>> uniqueEffects = new HashMap<>(); // skillid,
																				// cardids
	...
	public void initialize() {
		...
		// System.out.println("Loaded " + (cardEffects.size() +
		// uniqueEffects.size()) + " card effects");
	}

	public final Triple<Integer, Integer, Integer> getCardSkill(final int job, final int level) { // cardid,
																									// skillid,
																									// skilllevel
	...
	public final List<Integer> getUniqueSkills(final List<Integer> special) {
		...
		for (Entry<Integer, List<Integer>> m : uniqueEffects.entrySet()) {
			if (m.getValue().contains(special.get(0)) && m.getValue().contains(special.get(1))
					&& m.getValue().contains(special.get(2))) {
				uis.add(m.getKey());
			}
		}
		return uis;
	...
	public final Map<Integer, CardData> loadCharacterCards(final int accId, final int serverId) {
			...
			try (PreparedStatement ps = con.prepareStatement("SELECT * FROM `character_cards` WHERE `accid` = ?")) {
				...
				try (ResultSet rs = ps.executeQuery()) {
					...
					while (rs.next()) {
						...
						if (x == null || !canHaveCard(x.getLeft(), x.getRight())) { // we
																					// don't
																					// need
																					// to
																					// delete
		...
		for (int i = 1; i <= 6; i++) {
			if (cards.get(i) == null) {
				cards.put(i, new CardData(0, (short) 0, (short) 0)); // fill it
																		// in
			}
		}
		return cards;
	...
	public Map<Integer, Pair<Short, Short>> loadCharactersInfo(int accId, int serverId) {
		...
		try {
			...
			try (PreparedStatement ps = con
					.prepareStatement("SELECT id, level, job FROM characters WHERE accountid = ? AND world = ?")) {
				ps.setInt(1, accId);
