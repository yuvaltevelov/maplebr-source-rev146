...
public class CSMenuItem {
	...
	public static void loadFromDb() {
		...
		try {
			try (ResultSet rs = con.prepareStatement("SELECT * FROM cs_picture").executeQuery()) {
				while (rs.next()) {
					pictureItems.add(new CSMenuItem(rs.getInt("category"), rs.getInt("subcategory"),
							rs.getInt("parent"), rs.getString("image"), rs.getInt("sn"), rs.getInt("itemid"),
							rs.getByte("flag"), rs.getInt("originalPrice"), rs.getInt("salePrice"),
							rs.getInt("quantity"), rs.getInt("duration"), rs.getInt("likes")));
				}
			}
	...
	private CSMenuItem(int c, int sc, int p, String img, int sn, int id, byte flag, int op, int sp, int qty, int dur,
			int likes) {
		this.c = c;
		this.sc = sc;
		this.p = p;
	...
	public static void writeData(CSMenuItem csmi, MaplePacketLittleEndianWriter mplew) {
		...
		mplew.writeMapleAsciiString(csmi.img); // TODO add check if cat != 4
												// write empty string
		...
		mplew.write(HexTool.getByteArrayFromHexString("01 00 01 00 01 00 00 00 01 00 02 00 00 00")); // flags
																										// maybe
