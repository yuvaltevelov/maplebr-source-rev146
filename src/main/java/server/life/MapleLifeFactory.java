...
public class MapleLifeFactory {
	...
	private static final MapleDataProvider npcData = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wzpath") + "/Npc.wz"));
	...
	public static void loadQuestCounts() {
		...
		/*
		 * for (MapleData c : npcStringData) { int nid =
		 * Integer.parseInt(c.getName()); String n =
		 * StringUtil.getLeftPaddedStr(nid + ".img", '0', 11); try { if
		 * (npcData.getData(n) != null) {//only thing we really have to do is
		 * check if it exists. if we wanted to, we could get the script as well
		 * :3 String name = MapleDataTool.getString("name", c, "MISSINGNO"); if
		 * (name.contains("Maple TV") || name.contains("Baby Moon Bunny")) {
		 * continue; } npcNames.put(nid, name); } } catch (NullPointerException
		 * e) { } catch (RuntimeException e) { //swallow, don't add if } }
		 */
		Connection con = DatabaseConnection.getConnection();
		try {
			try (PreparedStatement ps = con.prepareStatement("SELECT * FROM wz_npcnamedata ORDER BY `npc`");
	...
	public static MapleMonsterStats getMonsterStats(int mid) {
		...
		if (stats == null) {
			...
			stats.setHp(GameConstants.getPartyPlayHP(mid) > 0 ? GameConstants.getPartyPlayHP(mid)
					: MapleDataTool.getLongConvert("maxHP", monsterInfoData));
			stats.setMp(MapleDataTool.getIntConvert("maxMP", monsterInfoData, 0));
			stats.setExp(mid == 9300027 ? 0
			...
			stats.setBoss(
					GameConstants.getPartyPlayHP(mid) > 0 || MapleDataTool.getIntConvert("boss", monsterInfoData, 0) > 0
							|| mid == 8810018 || mid == 9410066 || (mid >= 8810118 && mid <= 8810122));
			...
			/*
			 * for (int i = 0; true; i++) { // TODO: Check and reprint all
			 * available values again..doing like below is a ridiculous way
			 * final MapleData monsterAtt =
			 * monsterInfoData.getChildByPath("attack/" + i); final MapleData
			 * attackData = monsterData.getChildByPath("attack" + (i + 1) +
			 * "/info"); if (attackData == null || monsterAtt == null) { break;
			 * } final MobAttackInfo ret = new MobAttackInfo();
			 * 
			 * boolean deadlyAttack = monsterAtt.getChildByPath("deadlyAttack")
			 * != null; if (!deadlyAttack) { deadlyAttack =
			 * attackData.getChildByPath("deadlyAttack") != null; }
			 * ret.setDeadlyAttack(deadlyAttack);
			 * 
			 * int mpBurn = MapleDataTool.getInt("mpBurn", monsterAtt, 0); if
			 * (mpBurn == 0) { mpBurn = MapleDataTool.getInt("mpBurn",
			 * attackData, 0); } ret.setMpBurn(mpBurn);
			 * 
			 * int disease = MapleDataTool.getInt("disease", monsterAtt, 0); if
			 * (disease == 0) { disease = MapleDataTool.getInt("disease",
			 * attackData, 0); } ret.setDiseaseSkill(disease);
			 * 
			 * int level = MapleDataTool.getInt("level", monsterAtt, 0); if
			 * (level == 0) { level = MapleDataTool.getInt("level", attackData,
			 * 0); } ret.setDiseaseLevel(level);
			 * 
			 * int conMP = MapleDataTool.getInt("conMP", monsterAtt, 0); if
			 * (conMP == 0) { conMP = MapleDataTool.getInt("conMP", attackData,
			 * 0); } ret.setMpCon(conMP);
			 * 
			 * int attackAfter = MapleDataTool.getInt("attackAfter", monsterAtt,
			 * 0); if (attackAfter == 0) { attackAfter =
			 * MapleDataTool.getInt("attackAfter", attackData, 0); }
			 * ret.attackAfter = attackAfter;
			 * 
			 * int PADamage = MapleDataTool.getInt("PADamage", monsterAtt, 0);
			 * if (PADamage == 0) { PADamage = MapleDataTool.getInt("PADamage",
			 * attackData, 0); } ret.PADamage = PADamage;
			 * 
			 * int MADamage = MapleDataTool.getInt("MADamage", monsterAtt, 0);
			 * if (MADamage == 0) { MADamage = MapleDataTool.getInt("MADamage",
			 * attackData, 0); } ret.MADamage = MADamage;
			 * 
			 * boolean magic = MapleDataTool.getInt("magic", monsterAtt, 0) > 0;
			 * if (!magic) { magic = MapleDataTool.getInt("magic", attackData,
			 * 0) > 0; } ret.magic = magic; ret.isElement =
			 * monsterAtt.getChildByPath("elemAttr") != null; // we handle it
			 * like this, i don't know what it does
			 * 
			 * if (attackData.getChildByPath("range") != null) { // Definitely
			 * in attackData ret.range = MapleDataTool.getInt("range/r",
			 * attackData, 0); if (attackData.getChildByPath("range/lt") != null
			 * && attackData.getChildByPath("range/rb") != null) { ret.lt =
			 * (Point) attackData.getChildByPath("range/lt").getData(); ret.rb =
			 * (Point) attackData.getChildByPath("range/rb").getData(); } }
			...
			if (stats.getTagColor() > 0) {
				...
				hpdisplaytype = 2;
			} else if (!stats.isBoss() || mid == 9410066 || stats.isPartyBonus()) { // Not
																					// boss
																					// and
																					// dong
																					// dong
