...
public class MapleCharacterCards {
	...
	private final List<Pair<Integer, Integer>> skills = new LinkedList<>(); // id,
																			// level
	...
	public final void calculateEffects() {
		...
		for (Entry<Integer, CardData> x : cards.entrySet()) {
			if (x.getValue().cid > 0) { // exist
				final Triple<Integer, Integer, Integer> skillData = CharacterCardFactory.getInstance()
						.getCardSkill(x.getValue().job, x.getValue().level);
				if (x.getKey() < 4) {
					if (skillData != null) {
		...
		if (deck1amount == 3 && cardids1.size() == 3) {
			...
			for (final Integer ii : uid) {
				skills.add(new Pair<>(ii, GameConstants.getSkillLevel(lowD1))); // we
																				// can
																				// have
																				// more
																				// than
	...
	public final void recalcLocalStats(final MapleCharacter chr) {
		...
		if (pos != -1) {
			if (!CharacterCardFactory.getInstance().canHaveCard(chr.getLevel(), chr.getJob())) {
				cards.remove(pos); // we don't need to reset pos as its not
									// needed
			} else {
				cards.put(pos, new CardData(chr.getId(), chr.getLevel(), chr.getJob())); // override
																							// old
			}
		}
