...
public class DamageParse {
	...
	public static void applyAttack(AttackInfo attack, Skill theSkill, MapleCharacter player, int attackCount,
		...
		if (attack.skill != 0) {
			...
			if (GameConstants.isMulungSkill(attack.skill)) {
				...
				if ((attack.targets > effect.getMobCount()) && (attack.skill != 1211002) && (attack.skill != 1220010)) {
		...
		if (player.getClient().getChannelServer().isAdminOnly()) {
			player.dropMessage(-1,
					new StringBuilder().append("Animation: ")
							.append(Integer.toHexString(
									(attack.display & 0x8000) != 0 ? attack.display - 32768 : attack.display))
					.toString());
		}
			...
			if ((monster != null) && (monster.getLinkCID() <= 0)) {
					...
					if (fixeddmg != -1) {
						...
						if (Tempest) {
							...
							if (eachd.intValue() > maxDamagePerHit) {
								player.getCheatTracker().registerOffense(CheatingOffense.HIGH_DAMAGE,
										new StringBuilder().append("[Damage: ").append(eachd).append(", Expected: ")
												.append(maxDamagePerHit).append(", Mob: ").append(monster.getId())
												.append("] [Job: ").append(player.getJob()).append(", Level: ")
												.append(player.getLevel()).append(", Skill: ").append(attack.skill)
												.append("]").toString());
								if (attack.real) {
				...
				if ((GameConstants.getAttackDelay(attack.skill, theSkill) >= 100)
					...
					player.getCheatTracker().registerOffense(CheatingOffense.ATTACK_FARAWAY_MONSTER,
							new StringBuilder().append("[Distance: ")
									.append(player.getTruePosition().distanceSq(monster.getTruePosition()))
									.append(", Expected Distance: ")
									.append(GameConstants.getAttackRange(effect, player.getStat().defRange))
									.append(" Job: ").append(player.getJob()).append("]").toString());
				}
				...
				if ((totDamageToOneMonster > 0) || (attack.skill == 1221011) || (attack.skill == 21120006)) {
					...
					if (player.getJob() >= 410 && player.getJob() <= 412) {
						final Skill skill = SkillFactory
								.getSkill(player.getTotalSkillLevel(4120018) > 0 ? 4120018 : 4100011);
						if (player.getTotalSkillLevel(skill) > 0) {
							...
							if (eff.makeChanceResult() && monster != null) {
								...
								monster.applyStatus(player,
										new MonsterStatusEffect(MonsterStatus.POISON, 1, skill.getId(), null, false),
										true, eff.getDuration(), false, eff);
							}
							...
							for (AttackPair ap : attack.allDamage) {
								...
								System.out.println("Check is " + check != null ? true
										: false + " check skill is " + check.getSkill() + " from id is "
												+ check.getFromID());
								if (check != null && check.getSkill() == skill.getId()
										&& check.getFromID() == player.getId()) {
									final List<MapleMapObject> objs = player.getMap().getMapObjectsInRange(
											player.getPosition(), 500000, Arrays.asList(MapleMapObjectType.MONSTER));
									...
									// if (star != null) { // TODO: check for
									// quantity
									System.out.println("Star " + star.getItemId());
									player.getMap().broadcastMessage(AdventurerPacket.AssassinPacket.giveMarkOfTheif(
											player.getId(), source.getObjectId(), (skill.getId() + 1), monsters,
											player.getPosition(), monsters.get(0).getPosition(), star.getItemId()));
									// }
								}
							}
						}
	...
	public static final void applyAttackMagic(AttackInfo attack, Skill theSkill, MapleCharacter player,
		...
		} else if (((attack.hits > effect.getAttackCount()) && (effect.getAttackCount() != 0))
				|| (attack.targets > effect.getMobCount())) {
			player.getCheatTracker().registerOffense(CheatingOffense.MISMATCHING_BULLETCOUNT);
			// return;
		}
		...
		PlayerStats stats = player.getStat();
		Element element = player.getBuffedValue(MapleBuffStat.ELEMENT_RESET) != null ? Element.NEUTRAL
				: theSkill.getElement();
		...
		for (AttackPair oned : attack.allDamage) {
				...
				if ((!Tempest) && (!player.isGM())) {
					if ((!monster.isBuffed(MonsterStatus.MAGIC_IMMUNITY))
						...
						MaxDamagePerHit = CalculateMaxMagicDamagePerHit(player, theSkill, monster, monsterstats, stats,
								element, Integer.valueOf(CriticalDamage), maxDamagePerHit, effect);
					} else {
						MaxDamagePerHit = 1.0D;
					}
				}
			...
			if (GameConstants.isLuminous(player.getJob())) {
				if (player.getBuffedValue(MapleBuffStat.DARK_CRESCENDO) != 1)
					;
				{
					MapleStatEffect crescendo = SkillFactory.getSkill(27121005)
							.getEffect(player.getSkillLevel(27121005));
					if (crescendo != null) {
	...
	public static AttackInfo parseDmgM(LittleEndianAccessor lea, MapleCharacter chr)// reg
																					// att
	{
		AttackInfo ret = new AttackInfo();
		lea.skip(1);
