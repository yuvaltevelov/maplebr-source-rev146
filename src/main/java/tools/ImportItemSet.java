	...
	public static void main(String[] args) {
		while (true) {
			...
			for (Pair<Integer, String> item : MapleItemInformationProvider.getInstance().getAllItems2()) {
				if (item.getRight().startsWith(name) && !item.getRight().equals(name)
						&& (MapleItemInformationProvider.getInventoryType(item.getLeft()) == MapleInventoryType.EQUIP
								|| item.getRight().contains("recipe"))) {
