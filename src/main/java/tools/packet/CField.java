...
public class CField {
	...
	public static final byte[] Nexon_IP = new byte[] { (byte) 8, (byte) 31, (byte) 99, (byte) 141 };// current
																									// ip
	...
	public static byte[] getServerIP(MapleClient c, int port, int clientId) {
		...
		mplew.writeInt(1);
		mplew.writeShort(1);
		// mplew.write(HexTool.hex("00 00 00 00 00 00 00 00 00 00 01 00 00 00 01
		// 00"));
	...
	public static byte[] spawnPlayerMapobject(MapleCharacter chr) {
		...
		mplew.write(GameConstants.isZero(chr.getJob()) || GameConstants.isEvan(chr.getJob()) ? 0 : 1); // Shows
																										// the
																										// dragon
																										// in
																										// inventory
																										// it
																										// seems
		mplew.writeZeroBytes(13);// There must be something in here... But
									// what?...
		mplew.write(1);
		mplew.writeInt(CHAR_MAGIC_SPAWN);
	...
	public static byte[] spawnFamiliar(MonsterFamiliar mf, boolean spawn, boolean respawn) {
		...
		mplew.writeShort(
				respawn ? SendPacketOpcode.SPAWN_FAMILIAR_2.getValue() : SendPacketOpcode.SPAWN_FAMILIAR.getValue());
	...
	public static byte[] spawnObtacleAtomBomb() {
		...
		// Number of bomb objects to spawn. You can also just send multiple
		// packets instead of putting them all in one packet.
		mplew.writeInt(500);
		...
		if (unk == 1) {
			mplew.writeInt(300); // from
									// Effect.img/BasicEff/ObtacleAtomCreate/%d
