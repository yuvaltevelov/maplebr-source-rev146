...
public class Event_DojoAgent {
	...
	public static boolean warpStartDojo(final MapleCharacter c, final boolean party, final MapleMap currentmap) {
		...
		for (int x = 0; x < 10; x++) { // 15 maps each stage
			...
			for (int i = 1; i <= 47; i++) { // only 40 stages, but 47 maps
				MapleMap map = ch.getMapFactory().getMap(925020000 + 100 * i + x);// normal
																					// mode
				if (map.getCharactersSize() > 0) {
					canenterr = false;
					break;
		...
		for (int x = 0; x < 10; x++) { // 15 maps each stage // make this
										// better.....
			boolean canenterr = true;
			for (int i = 1; i <= 47; i++) { // only 40 stages, but 47 maps
				MapleMap map = ch.getMapFactory().getMap(925030000 + 100 * i + x);// hard
																					// mode
				if (map.getCharactersSize() > 0) {
					canenterr = false;
					break;
	...
	public static boolean warpNextMap(final MapleCharacter c, final boolean fromResting, final MapleMap currentmap) {
		try {
				...
				if (c.getParty() != null && c.getParty().getMembers().size() > 1) {
					for (MaplePartyCharacter mem : c.getParty().getMembers()) {
						...
						if (chr != null) {
							...
							// chr.getClient().getSession().write(WvsContext.Mulung_Pts(point,
							// dojo));
						}
					}
				} else {
					...
					// c.getClient().getSession().write(WvsContext.Mulung_Pts(point,
					// dojo));
				}
			...
			if (currentmap.getId() == 925020001) {// 925030100 925020001
				// final MapleMap mappzz =
				// ch.getMapFactory().getMap(925030100);//rooftop
				...
				final MapleMap mappzz = ch.getMapFactory().getMap(925020003);// rooftop
				c.changeMap(mappzz, mappzz.getPortal(1));
				c.getClient().getSession().write(CField.UIPacket.openUI(72));// Dojo
																				// ui
																				// TODO
																				// add
																				// points?
	...
	private static void spawnMonster(final MapleMap map, final int stage) {
		final int mobid;
		/*
		 * mobid = 9300183 + stage; if (mobid < 9300184 || mobid > 9300215) {
		 * return; }
		 */
		...
		//// }
		// For normal stages
		final int[] mob1 = { 9300209, 9300184, 9300191, 9300185 };// bluedad,
																	// mano,
																	// mushmom,
																	// sumpy
		int stage1 = mob1[Randomizer.nextInt(mob1.length)];
		final int[] mob2 = { 9300196, 9300187, 9300194, 9300190 };// zombiemom,
																	// kingslime,
																	// dyle,
																	// kingclang
		...
		final int[] mob6 = { 9300207, 9300200, 9300195 };// snakbar, taerone,
															// papapixie
