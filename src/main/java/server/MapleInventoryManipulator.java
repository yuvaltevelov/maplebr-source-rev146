...
public class MapleInventoryManipulator {
	...
	public static void addRing(MapleCharacter chr, int itemId, int ringId, int sn, String partner) {
		...
		chr.getClient().getSession().write(CSPacket.sendBoughtRings(GameConstants.isCrushRing(itemId), ring, sn,
	...
	public static int getUniqueId(int itemId, MaplePet pet) {
		...
		if (GameConstants.isPet(itemId)) {
			if (pet != null) {
				...
				|| MapleItemInformationProvider.getInstance().isCash(itemId)) { // less
																				// work
																				// to
																				// do
			uniqueid = MapleInventoryIdentifier.getInstance(); // shouldnt be
																// generated
																// yet, so put
																// it here
		}
	...
	public static byte addId(MapleClient c, int itemId, short quantity, String owner, MaplePet pet, long period,
		...
		if (itemId >= 4031332 && itemId <= 4031341) {
			c.getSession().write(
					CField.getGameMessage("Hint: Use @event to exchange a certificate of straight wins.", (short) 8));
		}
			...
			if (!GameConstants.isRechargable(itemId)) {
				...
				while (quantity > 0) {
					...
					if (newQ != 0) {
						...
						if (period > 0) {
							nItem.setExpiration(
									System.currentTimeMillis() + (period * (hours ? 1 : 24) * 60 * 60 * 1000));
						}
						if (pet != null) {
							nItem.setPet(pet);
	...
	private static Item checkEnhanced(final Item before, final MapleCharacter chr) {
		if (before instanceof Equip) {
			...
			if (eq.getState() == 0 && (eq.getUpgradeSlots() >= 1 || eq.getLevel() >= 1)
					&& GameConstants.canScroll(eq.getItemId()) && Randomizer.nextInt(100) >= 80) { // 20%
																									// chance
																									// of
																									// pot?
				eq.resetPotential();
	...
	public static boolean checkSpace(final MapleClient c, final int itemid, int quantity, final String owner) {
		...
		final MapleInventoryType type = GameConstants.getInventoryType(itemid);
		if (c == null || c.getPlayer() == null || c.getPlayer().getInventory(type) == null) { // wtf
																								// is
																								// causing
																								// this?
			return false;
	...
	public static void move(final MapleClient c, final MapleInventoryType type, final short src, final short dst) {
		...
		if (!type.equals(MapleInventoryType.EQUIP) && initialTarget != null
			...
			if ((olddstQ + oldsrcQ) > slotMax) {
				c.getSession().write(InventoryPacket.moveAndMergeWithRestInventoryItem(type, src, dst,
						(short) ((olddstQ + oldsrcQ) - slotMax), slotMax, bag, switchSrcDst, bothBag));
			} else {
				c.getSession()
						.write(InventoryPacket.moveAndMergeInventoryItem(type, src, dst,
								((Item) c.getPlayer().getInventory(type).getItem(dst)).getQuantity(), bag, switchSrcDst,
								bothBag));
			}
		} else {
			c.getSession().write(InventoryPacket.moveInventoryItem(type, switchSrcDst ? dst : src,
					switchSrcDst ? src : dst, eqIndicator, bag, bothBag));
		}
	}
	...
	public static void equip(final MapleClient c, final short src, short dst) {
		...
		// !c.getPlayer().isStaff()) {
		// c.getPlayer().dropMessage(1, "You are not allowed to use GM-Made
		// equips.");
		// c.getSession().write(CWvsContext.enableActions());
		// return;
		// }
		if (GameConstants.isOverPoweredEquip(c, source.getItemId(), src) && !c.getPlayer().isStaff()) {
			...
			// c.getPlayer().removeAll(source.getItemId(), false); //System
			// might be wrong
			c.getSession().write(CWvsContext.enableActions());
			return;
		}
		if (!c.getPlayer().isGM()) {
			if (source.getItemId() == 1112663 || source.getItemId() == 1112586) {
				c.getPlayer().dropMessage(1,
						"White Angelic Blessing, and Dark Angelic Blessing are currently not working.");
				c.getSession().write(CWvsContext.enableActions());
				return;
			}
		}
		...
		if (dst > -1200 && dst < -999 && !GameConstants.isEvanDragonItem(source.getItemId())
			...
			c.getSession().write(CWvsContext.enableActions());
			return;
			// } else if ((dst <= -1200 || (dst >= -999 && dst < -99)) &&
		...
		case -10: { // Shield
			...
			if (GameConstants.isKatara(source.getItemId())) {
				if ((chr.getJob() != 900 && !GameConstants.isDualBlade(chr.getJob())) || weapon == null
					...
					&& !GameConstants.isSpecialShield(source.getItemId())) {
				if (chr.getInventory(MapleInventoryType.EQUIP).isFull()) {
					c.getSession().write(InventoryPacket.getInventoryFull());
					c.getSession().write(InventoryPacket.getShowInventoryFull());
					return;
		...
		case -11: { // Weapon
			...
			if (shield != null && GameConstants.isTwoHanded(source.getItemId())
					&& !GameConstants.isSpecialShield(shield.getItemId())) {
				if (chr.getInventory(MapleInventoryType.EQUIP).isFull()) {
					c.getSession().write(InventoryPacket.getInventoryFull());
					c.getSession().write(InventoryPacket.getShowInventoryFull());
					return;
		...
		if (stats.get("equipTradeBlock") != null || source.getItemId() / 10000 == 167) { // Block
																							// trade
																							// when
																							// equipped.
			if (!ItemFlag.UNTRADABLE.check(flag)) {
				...
				source.setFlag(flag);
				c.getSession().write(InventoryPacket.updateSpecialItemUse_(source, MapleInventoryType.EQUIP.getType(),
						c.getPlayer()));
			}
		}
		if (source.getItemId() / 10000 == 166) {
			if (source.getAndroid() == null) {
				...
				source.setFlag(flag);
				c.getSession().write(CWvsContext.InventoryPacket.updateSpecialItemUse_(source,
						MapleInventoryType.EQUIP.getType(), c.getPlayer()));
			}
			chr.removeAndroid();
		...
		if (source.getCharmEXP() > 0 && !ItemFlag.CHARM_EQUIPPED.check(flag)) {
			...
			source.setFlag(flag);
			c.getSession().write(InventoryPacket.updateSpecialItemUse_(source,
		...
		if (source.getState() >= 17) {
			...
			int[] potentials = { source.getPotential1(), source.getPotential2(), source.getPotential3(),
					source.getBonusPotential1(), source.getBonusPotential2() };
		...
		c.getSession()
				.write(InventoryPacket.moveInventoryItem(MapleInventoryType.EQUIP, src, dst, (byte) 2, false, false));
		chr.equipChanged();
	}
	...
	public static void unequip(final MapleClient c, final short src, final short dst) {
			...
			for (int i : potentials) {
				if (i > 0) {
					...
					if (pot != null && pot.get("skillID") > 0) {
						ss.put(SkillFactory
								.getSkill(PlayerStats.getSkillByJob(pot.get("skillID"), c.getPlayer().getJob())),
								new SkillEntry((byte) 0, (byte) 0, -1));
					}
				}
			}
			c.getPlayer().changeSkillLevel_Skip(ss, true);
		...
		c.getSession()
				.write(InventoryPacket.moveInventoryItem(MapleInventoryType.EQUIP, src, dst, (byte) 1, false, false));
	...
	public static boolean drop(final MapleClient c, MapleInventoryType type, final short src, short quantity,
		...
		final Item source = c.getPlayer().getInventory(type).getItem(src);
		if (quantity < 0 || source == null || (GameConstants.GMS && src == -55)
				|| (!npcInduced && GameConstants.isPet(source.getItemId()))
				|| (quantity == 0 && !GameConstants.isRechargable(source.getItemId())) || c.getPlayer().inPVP()) {
			c.getSession().write(CWvsContext.enableActions());
			return false;
		}
		...
		if (quantity < source.getQuantity() && !GameConstants.isRechargable(source.getItemId())) {
			...
			if (ii.isDropRestricted(target.getItemId()) || ii.isAccountShared(target.getItemId())) {
				...
				if ((GameConstants.isPet(source.getItemId()) || ItemFlag.UNTRADABLE.check(flag))
						&& !GameConstants.isAnyDropMap(c.getPlayer().getMapId())) {
					c.getPlayer().getMap().disappearingItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos);
				} else {
					c.getPlayer().getMap().spawnItemDrop(c.getPlayer(), c.getPlayer(), target, dropPos, true, true);
				}
