...
public class PlayersHandler {
	...
	public static void Note(final LittleEndianAccessor slea, final MapleCharacter chr) {
		...
		case 1:
			...
			if (num < 0) { // note overflow, shouldn't happen much unless >
							// 32767
				num = 32767;
			}
	...
	public static void UseMechDoor(final LittleEndianAccessor slea, final MapleCharacter chr) {
		...
		final int mode = slea.readByte(); // specifies if backwarp or not, 1
											// town to target, 0 target to town
	...
	public static void TouchReactor(final LittleEndianAccessor slea, final MapleClient c) {
		final int oid = slea.readInt();
		final boolean touched = slea.available() == 0 || slea.readByte() > 0; // the
																				// byte
																				// is
																				// probably
																				// the
	...
	public static void startEvo(LittleEndianAccessor slea, MapleCharacter player, MapleClient c) {
		...
		/*
		 * final List<Integer> maps = new ArrayList<>(); switch (mapid) { case
		 * 0: maps.add(960010100); maps.add(960010101); maps.add(960010102);
		 * break; case 1: maps.add(960020100); maps.add(960020101);
		 * maps.add(960020102); maps.add(960020103); break; case 2:
		 * maps.add(960030100); break; case 3: maps.add(689000000);
		 * maps.add(689000010); break; default:
		 * 
		 * }
		 */
		// player.getClient().getChannelServer().getEventSM().getEventManager("EvolutionLab");
		// //Coming Soon
	...
	public static void HOLLY(MapleClient c, LittleEndianAccessor slea) {
		...
		if (skillid == 3121013) {
			...
			final MapleSummon tosummon = new MapleSummon(c.getPlayer(),
					SkillFactory.getSkill(3121013).getEffect(sum.getSkillLevel()),
					new Point(sum.getTruePosition().x, sum.getTruePosition().y), SummonMovementType.STATIONARY);
		...
		if (nut.getHits() > 2 && Math.random() < 0.4 && !nut.isStopped()) {
			...
			if (Math.random() < 0.05 && map.getBombings() > 0) {
				...
				if (c.getPlayer().getTeam() == 0) {
					...
					// c.getPlayer().getName() + " of Team Maple knocks down a "
					// + co + "."));
				} else {
					map.addStoryScore();
					// c.getPlayer().getMap().broadcastMessage(CWvsContext.broadcastMsg(5,
					// c.getPlayer().getName() + " of Team Story knocks down a "
					// + co + "."));
				}
	...
	// MapleSummon summons = new MapleSummon(summon.OwnerId(), skillid, poss,
	...
	public static void DoRing(final MapleClient c, final String name, final int itemid) {
		...
		WriteFuture write = chr.getClient().getSession()
				.write(CWvsContext.sendEngagementRequest(c.getPlayer().getName(), c.getPlayer().getId()));
	}
			...
			if (accepted) {
				...
				try {
					...
					chr.getClient().getSession()
							.write(CWvsContext.sendEngagement((byte) 0x10, newItemId, chr, c.getPlayer()));
	...
	public static void Report(final LittleEndianAccessor slea, final MapleClient c) {
		// 0 = success 1 = unable to locate 2 = once a day 3 = you've been
		// reported 4+ = unknown reason
	...
	public static void exitSilentCrusadeUI(final LittleEndianAccessor slea, final MapleClient c) {
		c.getPlayer().updateInfoQuest(1652, "alert=-1"); // Hide Silent Crusade
															// icon
	}
	...
	public static void claimSilentCrusadeReward(final LittleEndianAccessor slea, final MapleClient c) {
		...
		case 0:
			...
			c.getPlayer().updateInfoQuest(1648, "m0=2;m1=2;m2=2;m3=2;m4=2;r=1"); // Show
																					// Reward
																					// Claimed
			break;
		case 1:
			...
			c.getPlayer().updateInfoQuest(1649, "m0=2;m1=2;m2=2;m3=2;m4=2;r=1"); // Show
																					// Reward
																					// Claimed
			break;
		case 2:
			...
			c.getPlayer().updateInfoQuest(1650, "m0=2;m1=2;m2=2;m3=2;m4=2;r=1"); // Show
																					// Reward
																					// Claimed
			break;
		case 3:
			...
			c.getPlayer().updateInfoQuest(1651, "m0=2;m1=2;m2=2;m3=2;m4=2;r=1"); // Show
																					// Reward
																					// Claimed
			break;
		default:
	...
	public static void buySilentCrusade(final LittleEndianAccessor slea, final MapleClient c) {
		...
		short slot = slea.readShort(); // slot of item in the silent crusade
										// window
		int itemId = slea.readInt();
		short quantity = slea.readShort();
		int tokenPrice = 0, potentialGrade = 0;
		final MapleDataProvider prov = MapleDataProviderFactory
			...
			if (c.getPlayer().getQuestStatus(GameConstants.PLAYER_INFORMATION) > 0) {
				try {
					String[] info = c.getPlayer().getQuest(MapleQuest.getInstance(GameConstants.PLAYER_INFORMATION))
							.getCustomData().split(";");
					c.getSession().write(CWvsContext.loadInformation((byte) 2, Integer.parseInt(info[0]),
	...
	public static void LeavePVP(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (lv < 2 && c.getPlayer().getLevel() >= 120) { // gladiator, level
															// 120+
			x /= 2;
		}
		c.getPlayer().setTotalBattleExp(c.getPlayer().getTotalBattleExp() + ((x / 10) * 3 / 2));
		c.getPlayer().setBattlePoints(c.getPlayer().getBattlePoints() + ((x / 10) * 3 / 2)); // PVP
																								// 1.5
																								// EVENT!
		c.getPlayer().cancelAllBuffs();
		c.getPlayer().changeRemoval();
	...
	public static void EnterAzwan(final LittleEndianAccessor slea, final MapleClient c) {
		...
		int mapid = 262020000 + (mode * 1000) + difficult; // Supply doesn't
															// have difficult
															// but it's always 0
															// so idc
		...
		// EventManager em =
		// c.getChannelServer().getEventSM().getEventManager("Azwan");
		// EventInstanceManager eim = em.newInstance("Azwan");
		// eim.setProperty("Global_StartMap", mapid + "");
		// eim.setProperty("Global_ExitMap", (party == 1 ? 262000100 :
		// 262000200) + "");
		// eim.setProperty("Global_MinPerson", 1 + "");
		// eim.setProperty("Global_RewardMap", (party == 1 ? 262000100 :
		// 262000200) + "");
	...
	public static void magicWheel(LittleEndianAccessor slea, MapleClient c) {
		final byte mode = slea.readByte(); // 0 = open 2 = start 4 = receive
											// reward
		if (mode == 2) {
			slea.readInt(); // 4
			...
			// } catch (Exception ex) {
			// c.getSession().write(CWvsContext.magicWheel((byte) 0xA, null,
			// null, 0));
			// c.getSession().write(CWvsContext.enableActions());
			// return;
			// }
	...
	public static void onReward(LittleEndianAccessor slea, MapleClient c) throws SQLException {
		...
		if (mode == 2) { // Decline
			...
			if (type < 0 || type > 5) {
				System.out.println("[Hacking Attempt] " + c.getPlayer().getName()
						+ " has tried to receive reward with unavailable type.");
				c.getSession().write(CWvsContext.enableActions());
				return;
			}
			...
			if (reward.getType() != type || reward.getItem() != itemId || reward.getMaplePoints() != mp
				...
				System.out.println(
						"[Hacking Attempt] " + c.getPlayer().getName() + " has tried to exploit the reward receive.");
				c.getSession().write(CWvsContext.enableActions());
				return;
			}
			byte msg = 0x15;
			...
			case 3:
				...
				break;
			case 4:
				if (c.getPlayer().getMeso() + meso < Integer.MAX_VALUE && c.getPlayer().getMeso() + meso > 0) {
	...
	public static void updateRedLeafHigh(LittleEndianAccessor slea, MapleClient c) { // not
																						// finished
																						// yet
	...
	public static void StealSkill(LittleEndianAccessor slea, MapleClient c) {
		...
		// then a byte, 0 = learning, 1 = removing, but it doesnt matter since
		// we can just use cid
		if (cid <= 0) {
	...
	public static void viewSkills(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (!c.getChannelServer().getPlayerStorage().getCharacterById(victim).getSkills().isEmpty()
				&& GameConstants.isAdventurer(jobid)) {
	...
	public static void AttackPVP(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (chr == null || chr.isHidden() || !chr.isAlive() || chr.hasBlockedInventory() || chr.getMap() == null
				|| !chr.inPVP() || !chr.getEventInstance().getProperty("started").equals("1") || skillid >= 90000000) {
			c.getSession().write(CWvsContext.enableActions());
			return;
		}
		...
		int addedScore = 0, skillLevel = 0, trueSkillLevel = 0, animation = -1, attackCount, mobCount = 1,
				fakeMastery = chr.getStat().passive_mastery(), ignoreDEF = chr.getStat().ignoreTargetDEF,
				critRate = chr.getStat().passive_sharpeye_rate(), skillDamage = 100;
		...
		if (skillid > 0) {
			...
			if (animation == -1 && !skil.isMagic()) {
				final String after = aran ? "aran"
						: (katara ? "katara"
								: (weapon == null ? "barehands"
										: MapleItemInformationProvider.getInstance()
												.getAfterImage(weapon.getItemId())));
				if (after != null) {
					final List<Triple<String, Point, Point>> p = MapleItemInformationProvider.getInstance()
							.getAfterImage(after); // hack
					if (p != null) {
						ThreadLock.lock();
						try {
							while (animation == -1) {
								final Triple<String, Point, Point> ep = p.get(Randomizer.nextInt(p.size()));
								if (!ep.left.contains("stab") && (skillid == 4001002 || skillid == 14001002)) { // disorder
																												// hack
									continue;
								} else if (ep.left.contains("stab") && weapon != null
										&& weapon.getItemId() / 10000 == 144) {
									continue;
								}
			...
			if (skil.isMagic()) {
				fakeMastery = 0; // whoosh still comes if you put this higher
									// than 0
			}
			...
			if (effect.getCooldown(chr) > 0 && !chr.isGM()) {
				...
				if ((skillid != 35111004 && skillid != 35121013)
						|| chr.getBuffSource(MapleBuffStat.MECH_CHANGE) != skillid) { // Battleship
			...
			case 1112:
				if (PlayerHandler.isFinisher(skillid) > 0) { // finisher
					if (chr.getBuffedValue(MapleBuffStat.COMBO) == null
							|| chr.getBuffedValue(MapleBuffStat.COMBO) <= 2) {
						return;
					}
			...
			box = MapleStatEffect.calculateBoundingBox(chr.getTruePosition(), facingLeft, lt, rb,
		...
		} else if (!GameConstants.isMercedes(chr.getJob()) && chr.getBuffedValue(MapleBuffStat.SOULARROW) == null
				&& slot > 0) {
		...
		if (!area) {
			...
			for (MapleCharacter attacked : chr.getMap().getCharactersIntersect(box)) {
				if (attacked.getId() != chr.getId() && attacked.isAlive() && !attacked.isHidden()
					...
					double rawDamage = maxdamage
							/ Math.max(1, ((magic ? attacked.getStat().mdef : attacked.getStat().wdef)
					...
					try {
						for (int i = 0; i < attackCount; i++) {
							...
							int mploss = 0;
							double ourDamage = Randomizer.nextInt((int) Math.abs(Math.round(rawDamage - min)) + 2)
									+ min;
							if (attacked.getStat().dodgeChance > 0
								...
								ourDamage = 0;
								// i dont think level actually matters or it'd
								// be too op
								// } else if (attacked.getLevel() >
								// chr.getLevel() && Randomizer.nextInt(100) <
								// (attacked.getLevel() - chr.getLevel())) {
								// ourDamage = 0;
							} else if (attacked.getJob() == 122 && attacked.getTotalSkillLevel(1220006) > 0
									&& attacked.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -10) != null) {
								final MapleStatEffect eff = SkillFactory.getSkill(1220006)
										.getEffect(attacked.getTotalSkillLevel(1220006));
								if (eff.makeChanceResult()) {
									ourDamage = 0;
								}
							} else if (attacked.getJob() == 412 && attacked.getTotalSkillLevel(4120002) > 0) {
								final MapleStatEffect eff = SkillFactory.getSkill(4120002)
										.getEffect(attacked.getTotalSkillLevel(4120002));
								if (eff.makeChanceResult()) {
									ourDamage = 0;
								}
							} else if (attacked.getJob() == 422 && attacked.getTotalSkillLevel(4220006) > 0) {
								final MapleStatEffect eff = SkillFactory.getSkill(4220002)
										.getEffect(attacked.getTotalSkillLevel(4220002));
								if (eff.makeChanceResult()) {
									ourDamage = 0;
								}
							...
							if (ourDamage > 0 && skillid != 4211006 && skillid != 3211003 && skillid != 4111004
									&& (skillid == 4221001 || skillid == 3221007 || skillid == 23121003
											|| skillid == 4341005 || skillid == 4331006 || skillid == 21120005
											|| Randomizer.nextInt(100) < critRate)) {
								ourDamage *= (100.0 + (Randomizer
										.nextInt(Math.max(2,
												chr.getStat().passive_sharpeye_percent()
														- chr.getStat().passive_sharpeye_min_percent()))
										+ chr.getStat().passive_sharpeye_min_percent())) / 100.0;
								critical_ = true;
							}
							if (attacked.getBuffedValue(MapleBuffStat.MAGIC_GUARD) != null) {
								mploss = (int) Math.min(attacked.getStat().getMp(), (ourDamage
										* attacked.getBuffedValue(MapleBuffStat.MAGIC_GUARD).doubleValue() / 100.0));
							}
							ourDamage -= mploss;
					...
					addedScore += Math.min(attacked.getStat().getHp() / 100, (totalHPLoss / 100) + (totalMPLoss / 100)); // ive
																															// NO
																															// idea
					...
					chr.onAttack(attacked.getStat().getCurrentMaxHp(),
							attacked.getStat().getCurrentMaxMp(attacked.getJob()), skillid, attacked.getObjectId(),
							totalHPLoss, 0);
					...
					if (chr.getJob() == 121 || chr.getJob() == 122 || chr.getJob() == 2110 || chr.getJob() == 2111
							|| chr.getJob() == 2112) { // WHITEKNIGHT
						if (chr.getBuffSource(MapleBuffStat.WK_CHARGE) == 1211006
								|| chr.getBuffSource(MapleBuffStat.WK_CHARGE) == 21101006) {
							final MapleStatEffect eff = chr.getStatForBuff(MapleBuffStat.WK_CHARGE);
							if (eff.makeChanceResult()) {
								attacked.giveDebuff(MapleDisease.FREEZE, 1, eff.getDuration(),
										MapleDisease.FREEZE.getDisease(), 1);
							}
						}
						...
						if (eff != null && eff.makeChanceResult()) {
							attacked.giveDebuff(MapleDisease.SLOW, 100 - Math.abs(eff.getX()), eff.getDuration(),
									MapleDisease.SLOW.getDisease(), 1);
						}
					...
					} else if (chr.getJob() == 412 || chr.getJob() == 422 || chr.getJob() == 434 || chr.getJob() == 1411
							|| chr.getJob() == 1412) {
						...
						try {
							for (int i : skills) {
								...
								if (chr.getTotalSkillLevel(skill) > 0) {
									...
									if (venomEffect.makeChanceResult()) {// THIS
																			// MIGHT
																			// ACTUALLY
																			// BE
																			// THE
																			// DOT
										attacked.giveDebuff(MapleDisease.POISON, 1, venomEffect.getDuration(),
												MapleDisease.POISON.getDisease(), 1);
									}
									break;
								}
							}
					...
					if (ice == attacked.getId()) {
						chr.getClient().getSession().write(CField.getPVPIceHPBar(attacked.getStat().getHp(),
								attacked.getStat().getCurrentMaxHp()));
					} else {
						chr.getClient().getSession().write(CField.getPVPHPBar(attacked.getId(),
			...
			if (Integer.parseInt(chr.getEventInstance().getProperty("redflag")) == chr.getId()
					&& chr.getMap().getArea(1).contains(chr.getTruePosition())) {
				chr.getEventInstance().setProperty("redflag", "0");
				chr.getEventInstance().setProperty("blue",
						String.valueOf(Integer.parseInt(chr.getEventInstance().getProperty("blue")) + 1));
				...
				chr.getEventInstance().schedule("updateScoreboard", 1000);
					...
					&& chr.getMap().getArea(0).contains(chr.getTruePosition())) {
				chr.getEventInstance().setProperty("blueflag", "0");
				chr.getEventInstance().setProperty("red",
						String.valueOf(Integer.parseInt(chr.getEventInstance().getProperty("red")) + 1));
		...
		if (didAttack) {
			...
			if (skillid > 0 && (ourAttacks.size() > 0 || (skillid != 4331003 && skillid != 4341002))
					&& !GameConstants.isNoDelaySkill(skillid)) {
		...
		chr.getMap()
				.broadcastMessage(CField.pvpAttack(chr.getId(), chr.getLevel(), trueSkill, trueSkillLevel, speed,
						fakeMastery, visProjectile, attackCount, chargeTime, animation, facingLeft ? 1 : 0,
						chr.getStat().defRange, skillid, skillLevel, move, push, pull, ourAttacks));
	...
	public static boolean inArea(MapleCharacter chr) {
		...
		for (MapleMist mist : chr.getMap().getAllMistsThreadsafe()) {
			if (mist.getOwnerId() == chr.getId() && mist.isPoisonMist() == 2
