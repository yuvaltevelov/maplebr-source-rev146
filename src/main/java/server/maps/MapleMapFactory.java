...
public class MapleMapFactory {
	...
	private final MapleDataProvider source = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wzpath") + "/Map.wz"));
	private final MapleData nameData = MapleDataProviderFactory
	...
	public static int loadCustomLife() {
		...
		try {
			...
			// System.out.println("Successfully loaded " + customLife.size() + "
			// maps with custom life.");
	...
	public static void loadCustomNPC() {
		...
		// customNPC.add(new Pair<>(910000000, loadLife(9000019, 0, true, 47,
		// -266, 650, 650, 650, -266, "n", 0)));//cuz RPS no works..
		...
		// customNPC.add(new Pair<>(910000000, loadLife(1033225, 0, true, 42,
		// -266, 638, 638, 638, -266, "n", 0)));//Shadow Knight cuz not coded..
		...
		// customNPC.add(new Pair<>(910000000, loadLife(9900002, 0, false, 57,
		// 4, 603, 603, 603, 4, "n", 0)));
		...
		if (map == null) {
			...
			try {
				...
				for (MapleData life : mapData.getChildByPath("life")) {
					...
					if ((npcs || !type.equals("n")) && !limited.equals("Stage0")) { // alien
																					// pq
																					// stuff
				...
				if (reactors && herbRocks.size() > 0 && highestLevel >= 30 && map.getFirstUserEnter().equals("")
					...
					if (highestLevel >= 100) {
						for (int i = 0; i < 10; i++) {
							for (int x = 0; x < 4; x++) { // to make heartstones
															// rare
					...
					for (int i = 0; i < numSpawn && !herbRocks.isEmpty(); i++) {
						...
						myReactor.setDelay(idd % 100 == 11 ? 60000 : 5000); // in
																			// the
																			// reactor's
																			// wz
				...
				map.setClock(mapData.getChildByPath("clock") != null); // clock
																		// was
																		// changed
																		// in wz
																		// to
			...
			if (mapData.getChildByPath(String.valueOf(i)) != null && mapData.getChildByPath(i + "/obj") != null) {
				for (MapleData node : mapData.getChildByPath(i + "/obj")) {
					if (node.getChildByPath("SN_count") != null && node.getChildByPath("speed") != null) {
						...
						nodeInfo.addFlag(new Pair<>(name, name.endsWith("3") ? 1 : 0)); // idk,
																						// no
																						// indication
																						// in
																						// wz
