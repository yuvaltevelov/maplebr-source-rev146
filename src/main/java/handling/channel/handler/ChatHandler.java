...
public class ChatHandler {
	...
	public static void GeneralChat(final String text, final byte unk, final MapleClient c, final MapleCharacter chr) {
		if (text.length() > 0 && chr != null && chr.getMap() != null
				&& !CommandProcessor.processCommand(c, text, CommandType.NORMAL)) {
			...
			if (chr.getCanTalk() || chr.isStaff()) {
				...
				if (chr.isHidden()) {
					if (chr.isIntern() && !chr.isSuperGM() && unk == 0) {
						chr.getMap().broadcastGMMessage(chr,
								CField.getChatText(chr.getId(), text, c.getPlayer().isSuperGM(), (byte) 1), true);
						if (unk == 0) {
							chr.getMap().broadcastGMMessage(chr,
									CWvsContext.broadcastMsg(2, chr.getName() + " : " + text), true);
						}
					} else {
						chr.getMap().broadcastGMMessage(chr,
								CField.getChatText(chr.getId(), text, c.getPlayer().isSuperGM(), unk), true);
					}
				} else {
					chr.getCheatTracker().checkMsg();
					if (chr.isIntern() && !chr.isSuperGM() && unk == 0) {
						// chr.getMap().broadcastMessage(CField.getChatText(chr.getId(),
						// text, false, (byte) 1),
						// c.getPlayer().getTruePosition());
						chr.getMap().broadcastMessage(ColourChat(chr, text, unk, chr.getChatType()));
						chr.getMap()
								.broadcastMessage(CField.getChatText(chr.getId(), text, c.getPlayer().isSuperGM(), 1));
						/*
						 * if (unk == 0) {
						 * //chr.getMap().broadcastMessage(CWvsContext.
						 * broadcastMsg(2, chr.getName() + " : " + text),
						 * c.getPlayer().getTruePosition());
						 * chr.getMap().broadcastMessage(ColourChat(chr, text,
						 * unk, chr.getChatType()));
						 * chr.getMap().broadcastMessage(CField.getChatText(chr.
						 * getId(), text, false, 1)); }
						 */
					} else {
						// chr.getMap().broadcastMessage(CField.getChatText(chr.getId(),
						// text, c.getPlayer().isSuperGM(), unk),
						// c.getPlayer().getTruePosition());
						chr.getMap().broadcastMessage(ColourChat(chr, text, unk, chr.getChatType()));
						chr.getMap()
								.broadcastMessage(CField.getChatText(chr.getId(), text, c.getPlayer().isSuperGM(), 1));// was1
					}
				}
				...
				c.getSession()
						.write(CWvsContext.broadcastMsg(6, "You have been muted and are therefore unable to talk."));
			}
		}
	}

	public static void AdminChat(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		if (!c.getPlayer().isGM()) {// if ( (signed
									// int)CWvsContext::GetAdminLevel((void
									// *)v294) > 2 )
			return;
		}
		byte mode = slea.readByte();
		// not saving slides...
		byte[] packet = CWvsContext.broadcastMsg(slea.readByte(), slea.readMapleAsciiString());// maybe
																								// I
																								// should
																								// make
																								// a
	...
	public static void Others(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		...
		if (c.isMonitored()) {
			...
			World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(6, "[GM Message] "
					+ MapleCharacterUtil.makeMapleReadable(chr.getName()) + " said (" + chattype + "): " + chattext));
	...
	public static void Messenger(final LittleEndianAccessor slea, final MapleClient c) {
				...
				if (target != null) {
					if (target.getMessenger() == null) {
						if (!target.isIntern() || c.getPlayer().isIntern()) {
							...
							target.getClient().getSession()
									.write(CField.messengerInvite(c.getPlayer().getName(), messenger.getId()));
						} else {
							c.getSession().write(CField.messengerNote(input, 4, 0));
						}
					} else {
						c.getSession().write(CField.messengerChat(c.getPlayer().getName(),
					...
					if (World.isConnected(input)) {
						World.Messenger.messengerInvite(c.getPlayer().getName(), messenger.getId(), input,
								c.getChannel(), c.getPlayer().isIntern());
					} else {
						c.getSession().write(CField.messengerNote(input, 4, 0));
					}
				}
		...
		case 0x06: // message
			if (messenger != null) {
				...
				if (messenger.isMonitored() && chattext.length() > c.getPlayer().getName().length() + 3) { // name
																											// :
																											// NOT
																											// name0
																											// or
																											// name1
					World.Broadcast
							.broadcastGMMessage(CWvsContext.broadcastMsg(6,
									"[GM Message] " + MapleCharacterUtil.makeMapleReadable(c.getPlayer().getName())
											+ "(Messenger: " + messenger.getMemberNamesDEBUG() + ") said: "
											+ chattext));
				}
			}
			break;
		...
		case 5: { // Find
			...
			if (player != null) {
				if (!player.isIntern() || c.getPlayer().isIntern() && player.isIntern()) {
					...
					c.getSession()
							.write(CField.getFindReplyWithMap(player.getName(), player.getMap().getId(), mode == 72));// 68
				} else {
			...
			if (ch > 0) {
				...
				if (c.isMonitored()) {
					World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(6,
							c.getPlayer().getName() + " whispered " + recipient + " : " + text));
