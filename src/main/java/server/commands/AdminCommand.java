...
public class AdminCommand {
	...
	public static class UpdatePet extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
					PetPacket.petColor(c.getPlayer().getId(), (byte) 0, Color.yellow.getAlpha()), true);
			return 1;
		}
	}
		...
		public int execute(final MapleClient c, String[] splitted) {
			...
			WorldTimer.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					c.getSession().write(CField.EffectPacket.showRewardItemAnimation(2028162, "")); // sealed
																									// box
				}
			}, 10000);
			return 1;
	...
	public static class Reward extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			chr.addReward(Integer.parseInt(splitted[2]), Integer.parseInt(splitted[3]), Integer.parseInt(splitted[4]),
					Integer.parseInt(splitted[5]), Integer.parseInt(splitted[6]),
					StringUtil.joinStringFrom(splitted, 7));
			chr.updateReward();
			return 1;
		}
	}
	...
	public static class DoubleTime extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(4,
					"It's Miracle Time!  Between 2:00 PM and 4:00 PM (Pacific) today, Miracle, Premium, Revolutionary Miracle, Super Miracle, Enlightening Miracle and Carved Slot Miracle Cubes have increased chances to raise your item to the next potential tier!"));
			// }
	...
	public static class DoubleMiracleTime extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			for (ChannelServer cserv : ChannelServer.getAllInstances()) {
				for (MapleCharacter mch : cserv.getPlayerStorage().getAllCharacters()) {
					mch.dropMessage(0,
							"Double Miracle Time Event has " + (EventConstants.DoubleMiracleTime ? "began!" : "ended"));
				}
			}
			return 1;
		}
	...
	public static class WarpCashShop extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			client.getSession()
					.write(CField.getChannelChange(client, Integer.parseInt(CashShopServer.getIP().split(":")[1])));
	...
	public static class TestDirection extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			c.getSession()
					.write(CField.UIPacket.getDirectionInfo(StringUtil.joinStringFrom(splitted, 5),
							Integer.parseInt(splitted[1]), Integer.parseInt(splitted[2]), Integer.parseInt(splitted[3]),
	...
	public static class ToggleAutoRegister extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			c.getPlayer().dropMessage(0,
					"Auto Register has been " + (AutoRegister.autoRegister ? "enabled" : "disabled") + ".");
				...
				for (MapleCharacter mch : cserv.getPlayerStorage().getAllCharacters()) {
					if (c.canClickNPC()) {
						...
						mch.getClient().getSession()
								.write(CField.NPCPacket.getNPCTalk(9010010, (byte) 0,
										"You got the #t" + Integer.parseInt(splitted[1])
												+ "#, right? Click it to see what's inside. Go ahead and check your item inventory now, if you're curious.",
										"00 00", (byte) 1, 9010010));
					}
				}
			}
			...
			if (ts == null && (t == null || !t.isAlive())) {
				...
				ts = EventTimer.getInstance().register(new Runnable() {
					...
					public void run() {
						...
						World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(0,
								"The server will shutdown in " + minutesLeft + " minutes. Please log off safely."));
						minutesLeft--;
					}
				}, 60000);
			} else {
