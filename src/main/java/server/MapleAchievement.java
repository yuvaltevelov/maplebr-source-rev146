	...
	public void finishAchievement(MapleCharacter chr) {
		...
		if (notice && !chr.isGM()) {
			World.Broadcast.broadcastMessage(
					CWvsContext.broadcastMsg(6, "[Achievement] Congratulations to " + chr.getName() + " on " + name
							+ " and rewarded with " + (GameConstants.GMS ? (reward / 2) : reward) + " cash!"));
		} else {
			chr.getClient().getSession().write(CWvsContext.broadcastMsg(5, "[Achievement] You've gained "
					+ (GameConstants.GMS ? (reward / 2) : reward) + " Cash as you " + name + "."));
		}
	}
}
