...
public class MobSkill {
	...
	public boolean checkCurrentBuff(MapleCharacter player, MapleMonster monster) {
		...
		case 170:
			if (monster != null && monster.getMap() != null) {
				...
				for (int i = 0; i < 4; i++) {
					final Point point = monster.getMap().calcPointBelow(
							new Point(oldPos.x + (i % 2 == 0 ? x : -x), oldPos.y + (i / 2 == 0 ? y : -y)));
					if (point != null) {
				...
				monster.getMap().broadcastMessage(MobPacket.spawnMonster(monster, -1, 0, false),
	...
	public void applyEffect(MapleCharacter player, MapleMonster monster, boolean skill) {
		...
		case 129: // Banish
			if (monster != null && monster.getMap().getSquadByMap() == null) { // not
																				// pb/vonleon
																				// map
		...
		case 131: // Mist
			if (monster != null) {
				monster.getMap().spawnMist(
						new MapleMist(calculateBoundingBox(monster.getTruePosition(), true), monster, this), x * 10,
						false);
			}
			break;
		case 140:
		...
		case 201:
			...
			monster.getMap().spawnMonsterWithEffect(toSpawn, getSpawnEffect(),
					monster.getMap().calcPointBelow(new Point(52, 125 - 1)));
			break;
		}
