...
public class EventInstanceManager {
			...
			for (MapleCharacter chr : getPlayers()) {
				if (name.startsWith("PVP")) {
					chr.getClient().getSession()
							.write(CField.getPVPClock(Integer.parseInt(getProperty("type")), timesend));
				} else {
	...
	private boolean unregisterPlayer_NoLock(final MapleCharacter chr) {
		if (name.equals("CWKPQ")) { // hard code it because i said so
			final MapleSquad squad = ChannelServer.getInstance(channel).getMapleSquad("CWKPQ");// so
																								// fkin
																								// hacky
			if (squad != null) {
	...
	public final MapleMap setInstanceMap(final int mapid) { // gets instance map
															// from the
															// channelserv
		if (disposed) {
	...
	public final MapleMap getMapInstance(int args) {
		...
		try {
			...
			if (!instanced) {
				...
				// in case reactors need shuffling and we are actually loading
				// the map
