...
public class MapleSquad {
	...
	public static enum MapleSquadType {
		...
		bossbalrog(2), zak(2), chaoszak(3), horntail(2), chaosht(3), pinkbean(2), nmm_squad(2), vergamot(2), dunas(
				2), nibergen_squad(2), dunas2(2), core_blaze(2), aufheben(2), cwkpq(3), tokyo_2095(2), vonleon(
						3), scartar(2), cygnus(3), hilla(2), darkhilla(2), arkarium(3);
		...
		while (type.queue.get(ch).size() > 0 && ChannelServer.getInstance(ch).getMapleSquad(type) == null) {
			...
			if (theirCh > 0) {
				...
				if (lead != null && lead.getMapId() == beginMapId && lead.getClient().getChannel() == ch) {
					...
					getBeginMap().broadcastMessage(CWvsContext.broadcastMsg(6, nextPlayerId
				...
				getBeginMap().broadcastMessage(CWvsContext.broadcastMsg(6,
	...
	public void scheduleRemoval() {
		removal = EtcTimer.getInstance().schedule(new Runnable() {
			...
			@Override
			public void run() {
				if (status != 0 && leader != null && (getLeader() == null || status == 1)) { // leader
																								// itself
																								// =
																								// null
																								// means
