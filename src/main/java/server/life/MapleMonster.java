...
public class MapleMonster extends AbstractLoadedMapleLife {
	...
	private int linkoid = 0, lastNode = -1, highestDamageChar = 0, linkCID = 0; // Just
																				// a
																				// reference
																				// for
																				// monster
	...
	public final void damage(final MapleCharacter from, final long damage, final boolean updateAttackTime,
		...
		if (stats.getSelfD() != -1) {
							...
							if (cattacker.getAttacker().getMap() == from.getMap()) { // current
								...
								if (cattacker.getLastAttackTime() >= System.currentTimeMillis() - 4000) {
									cattacker.getAttacker().getClient().getSession()
											.write(MobPacket.showMonsterHP(getObjectId(), getHPPercent()));
								}
							}
						}
					}
			...
			if (sponge.get() != null) {
				if (sponge.get().hp > 0) { // If it's still alive, dont want
					...
					if (sponge.get().hp <= 0) {
						map.broadcastMessage(
								MobPacket.showBossHP(sponge.get().getId(), -1, sponge.get().getMobMaxHp()));
	...
	public final void heal(int hp, int mp, final boolean broadcast) {
		...
		} else if (sponge.get() != null) { // else if, since only sponge doesn't
											// broadcast
			sponge.get().hp += hp;
		}
	}
	...
	private void giveExpToCharacter(final MapleCharacter attacker, int exp, final boolean highestDamage,
		...
		if (exp > 0) {
			...
			exp = (int) Math.min(Integer.MAX_VALUE,
					exp * ChannelServer.getInstance(map.getChannel()).getExpRate(attacker.getWorld()));
			...
			attacker.gainExpMonster(exp, true, highestDamage, pty, Class_Bonus_EXP, Equipment_Bonus_EXP,
					Premium_Bonus_EXP, stats.isPartyBonus(), stats.getPartyBonusRate());
		}
	...
	public final int killBy(final MapleCharacter killer, final int lastSkill) {
		...
		} // you cant fix the skills ;o? not until someone sniffs forme wanna do
			// it on my cpu?;o hmm its hard because maple lags on tv
		...
		if (oldSponge != null && oldSponge.isAlive()) {
			...
			if (set) { // all sponge monsters are dead, please kill off the
						// sponge
	...
	public final void changeableMob(MapleCharacter chr) {
		if (getStats().isChangeableMob()) {
			...
			setStatChanged(true);
			/*
			 * if (monster.getId() == 9306300) { //this.getStats().setHp(damage
			 * + 1); // OverrideMonsterStats newstat = new
			 * OverrideMonsterStats(damage, this.getMp(), this.getMobExp());
	...
	public final void applyStatus(final MapleCharacter from, final MonsterStatusEffect status, final boolean poison,
			long duration, final boolean checkboss, final MapleStatEffect eff) {
		...
		if (stats.isBoss()) {
			...
			if (checkboss && stat != (MonsterStatus.SPEED) && stat != (MonsterStatus.NINJA_AMBUSH)
					&& stat != (MonsterStatus.WATK) && stat != (MonsterStatus.POISON)
					&& stat != MonsterStatus.VENOMOUS_WEAPON && stat != (MonsterStatus.DARKNESS)
					&& stat != (MonsterStatus.MAGIC_CRASH)) {
				return;
			}
		...
		if (stats.isFriendly() || isFake()) {
			if (stat == MonsterStatus.STUN || stat == MonsterStatus.SPEED || stat == MonsterStatus.POISON
					|| stat == MonsterStatus.VENOMOUS_WEAPON) {
				return;
			}
		}
		...
		if (poison && getHp() > 1) {
			status.setValue(status.getStati(),
					Integer.valueOf((int) ((eff.getDOT() + from.getStat().dot
							+ from.getStat().getDamageIncrease(eff.getSourceId()))
							* from.getStat().getCurrentMaxBaseDamage() / 100.0)));
			...
			status.setPoisonSchedule(
					Integer.valueOf((int) (eff.getDamage() * from.getStat().getCurrentMaxBaseDamage() / 100.0)), from);

		} else if (statusSkill == 4121004 || statusSkill == 4221004) {
			status.setValue(status.getStati(), Math.min(Short.MAX_VALUE,
					Integer.valueOf((int) (eff.getDamage() * from.getStat().getCurrentMaxBaseDamage() / 100.0))));
			int dam = Integer.valueOf((int) (aniTime / 1000 * status.getX() / 2));
			status.setPoisonSchedule(dam, from);
			if (dam > 0) {
				if (dam >= hp) {
	...
	public void applyStatus(MonsterStatusEffect status) { // ONLY USED FOR
															// POKEMONN, ONLY
															// WAY POISON CAN
															// FORCE ITSELF INTO
															// STATI.
	...
	public final void dispelSkill(final MobSkill skillId) {
		...
		for (Entry<MonsterStatus, MonsterStatusEffect> effects : stati.entrySet()) {
			...
			if (mse.getMobSkill() != null && mse.getMobSkill().getSkillId() == skillId.getSkillId()) { // not
																										// checking
																										// for
																										// level.
	...
	public final void applyMonsterBuff(final Map<MonsterStatus, Integer> effect, final int skillId, final long duration,
			...
			for (Entry<MonsterStatus, Integer> z : effect.entrySet()) {
				if (con != null) {
					map.broadcastMessage(con,
							MobPacket.applyMonsterStatus(getObjectId(), z.getKey(), z.getValue(), skill),
							getTruePosition());
					con.getClient().getSession()
							.write(MobPacket.applyMonsterStatus(getObjectId(), z.getKey(), z.getValue(), skill));
				} else {
					map.broadcastMessage(MobPacket.applyMonsterStatus(getObjectId(), z.getKey(), z.getValue(), skill),
							getTruePosition());
				}
			}
		}
	...
	public final void doPoison(final MonsterStatusEffect status, final WeakReference<MapleCharacter> weakChr) {
		if ((status.getStati() == MonsterStatus.VENOMOUS_WEAPON || status.getStati() == MonsterStatus.POISON)
				&& poisons.size() <= 0) {
			return;
		}
		if (status.getStati() != MonsterStatus.VENOMOUS_WEAPON && status.getStati() != MonsterStatus.POISON
		...
		public final void killedMob(final MapleMap map, final int baseExp, final boolean mostDamage,
			...
			for (final Entry<MapleCharacter, OnePartyAttacker> attacker : resolveAttackers().entrySet()) {
				...
				for (final MapleCharacter expReceiver : expApplicable) {
					...
					iexp += (int) Math.round(
							((attacker.getKey().getId() == expReceiver.getId() ? (GameConstants.GMS ? 0.2 : 0.6) : 0.0)
									+ levelMod) * innerBaseExp);
					expMap.put(expReceiver,
							new ExpMap(iexp, (byte) expApplicable.size(), Class_Bonus_EXP, Premium_Bonus_EXP));
				}
			}
			ExpMap expmap;
			...
			for (MonsterDropEntry d : dropEntry) { // set to 4x rate atm, 40%
				...
				if (d.itemId > 0 && d.questid == 0 && d.itemId / 10000 != 238
						&& Randomizer.nextInt(999999) < (int) (10 * d.chance * chServerrate * chr.getDropMod()
								* chr.getStat().dropBuff / 100.0 * (showdown / 100.0))) { // kinda
																							// op
	...
	public final void cancelSingleStatus(final MonsterStatusEffect stat) {
		if (stat == null || stat.getStati() == MonsterStatus.EMPTY || stat.getStati() == MonsterStatus.SUMMON
				|| !isAlive()) {
			return;
		}
	...
	public void doDropItem(long now) {
		...
		case 9300061:
			itemId = 4001101;
			break;
		default: // until we find out ... what other mobs use this and how to
					// get the ITEMID
			cancelDropItem();
			return;
		}
	...
	public void setLinkCID(int lc) {
		this.linkCID = lc;
		if (lc > 0) {
			stati.put(MonsterStatus.HYPNOTIZE,
					new MonsterStatusEffect(MonsterStatus.HYPNOTIZE, 60000, 30001062, null, false));
		}
	}
	...
	public void setBelongsTo(MapleCharacter chr) { // Only let it be controlled
													// once.
		if (belongsTo != -1) {
			return;
		}
		...
		belongsTo = chr.getId();
		endBelong = System.currentTimeMillis() + (stats.isBoss() ? 300000 : 30000); // 30
																					// seconds
																					// for
																					// the
																					// person
