...
public class Skill implements Comparator<Skill> {
	...
	private int id, animationTime = 0, masterLevel = 0, maxLevel = 0, delay = 0, trueMax = 0, eventTamingMob = 0,
			skillTamingMob = 0, skillType = 0, psd = 0, psdSkill = 0; // 4 is
																		// alert
	...
	public static Skill loadFromData(final int id, final MapleData data, final MapleData delayData) {
		...
		if (skillType == 2) {
			...
			if (action_ != null) {
				...
				if (d != null) {
					...
					if (SkillFactory.getDelay(d) != null) { // this should
															// return true
															// always
						ret.animation = new ArrayList<>();
		...
		if (level != null) {
			ret.maxLevel = MapleDataTool.getInt("maxLevel", level, 1); // 10
																		// just
																		// a
																		// failsafe,
																		// shouldn't
																		// actually
																		// happens
			ret.psdDamR = MapleDataTool.getString("damR", level, ""); // for the
																		// psdSkill
																		// tag
	...
	public boolean isFourthJob() {
		switch (id) { // I guess imma make an sql table to store these, so that
						// we could max them all out.
		case 3120011:
		case 3220010:
		case 4320005:
		case 4340010:
		...
		if ((this.id / 10000 == 2312) || (this.id / 10000 == 2712) || (this.id / 10000 == 6112)
				|| (this.id / 10000 == 6512)) {
			return true;
		}
	...
	public boolean isSpecialSkill() {
		int jobId = id / 10000;
		return jobId == 900 || jobId == 800 || jobId == 9000 || jobId == 9200 || jobId == 9201 || jobId == 9202
				|| jobId == 9203 || jobId == 9204;
	}
