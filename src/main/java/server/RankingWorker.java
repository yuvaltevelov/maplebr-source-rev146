...
public class RankingWorker {
	...
	public static void run() {
		...
		// System.out.println("Done loading Rankings in " +
	...
	private static void updateRanking(Connection con) throws Exception {
		...
		sb.append(
				" FROM characters AS c LEFT JOIN accounts AS a ON c.accountid = a.id WHERE c.gm = 0 AND a.banned = 0 AND c.level >= 30");
		...
		try (PreparedStatement charSelect = con.prepareStatement(sb.toString());
			...
			while (rs.next()) {
				...
				rankings.get(job / 100).add(new RankingInformation(rs.getString("name"), job, rs.getInt("level"),
						rs.getLong("exp"), jobRank, rs.getInt("fame")));
				ps.setInt(1, jobRank);
	...
	public static class RankingInformation {
		...
		public RankingInformation(String name, int job, int level, long exp, int rank, int fame) {
			...
			builder.append(fame);
			builder.append(" Fame");
			this.toString = builder.toString(); // Rank 1 : KiDALex - Level 200
												// Blade Master | 0 EXP, 30000
												// Fame
		}
