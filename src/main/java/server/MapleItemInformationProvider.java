...
public class MapleItemInformationProvider {
	...
	protected final MapleDataProvider chrData = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wzpath") + "/Character.wz"));
	protected final MapleDataProvider etcData = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wzpath") + "/Etc.wz"));
	protected final MapleDataProvider itemData = MapleDataProviderFactory
			.getDataProvider(new File(System.getProperty("wzpath") + "/Item.wz"));
	protected final MapleDataProvider stringData = MapleDataProviderFactory
	...
	protected final Map<Integer, Map<Integer, StructItemOption>> socketCache = new HashMap<>(); // Grade,
																								// (id,
																								// data)
	...
	protected final Map<Integer, Pair<Integer, Integer>> potLife = new HashMap<>(); // itemid
																					// to
																					// lifeid,
																					// levels
	protected final Map<Integer, StructFamiliar> familiars = new HashMap<>(); // by
																				// familiarID
	protected final Map<Integer, StructFamiliar> familiars_Item = new HashMap<>(); // by
																					// cardID
	protected final Map<Integer, StructFamiliar> familiars_Mob = new HashMap<>(); // by
																					// mobID
	...
	public void runEtc() {
		...
		for (MapleDataEntry b : a.getFiles()) {
			...
			for (Entry<String, Pair<Point, Point>> ez : dummy.entrySet()) {
				...
				} else if (ez.getKey().contains("PoleArm")) { // D = double
																// weapon
		...
		if (GameConstants.GMS) { // these must be loaded before items..
			...
			for (MapleDataEntry d : e.getFiles()) {
				...
				if (familiars.containsKey(id)) {
					familiars.get(id).grade = (byte) MapleDataTool.getInt("grade",
							chrData.getData("Familiar/" + d.getName()).getChildByPath("info"), 0);
				}
			}
			...
			for (MapleData d : mSetsData.getChildByPath("setList")) {
				...
				monsterBookSets.put(Integer.parseInt(d.getName()),
						new Triple<>(MapleDataTool.getInt("setScore", d, 0), set, potential));
			}
		}
	...
	public Equip levelUpEquip(Equip equip, Map<String, Integer> sta) {
		...
		try {
			for (Entry<String, Integer> stat : sta.entrySet()) {
				...
				case "STRMin":
					nEquip.setStr(
							(short) (nEquip.getStr() + rand(stat.getValue().intValue(), sta.get("STRMax").intValue())));
					break;
				case "DEXMin":
					nEquip.setDex(
							(short) (nEquip.getDex() + rand(stat.getValue().intValue(), sta.get("DEXMax").intValue())));
					break;
				case "INTMin":
					nEquip.setInt(
							(short) (nEquip.getInt() + rand(stat.getValue().intValue(), sta.get("INTMax").intValue())));
					break;
				case "LUKMin":
					nEquip.setLuk(
							(short) (nEquip.getLuk() + rand(stat.getValue().intValue(), sta.get("LUKMax").intValue())));
					break;
				case "PADMin":
					nEquip.setWatk((short) (nEquip.getWatk()
							+ rand(stat.getValue().intValue(), sta.get("PADMax").intValue())));
					break;
				case "PDDMin":
					nEquip.setWdef((short) (nEquip.getWdef()
							+ rand(stat.getValue().intValue(), sta.get("PDDMax").intValue())));
					break;
				case "MADMin":
					nEquip.setMatk((short) (nEquip.getMatk()
							+ rand(stat.getValue().intValue(), sta.get("MADMax").intValue())));
					break;
				case "MDDMin":
					nEquip.setMdef((short) (nEquip.getMdef()
							+ rand(stat.getValue().intValue(), sta.get("MDDMax").intValue())));
					break;
				case "ACCMin":
					nEquip.setAcc(
							(short) (nEquip.getAcc() + rand(stat.getValue().intValue(), sta.get("ACCMax").intValue())));
					break;
				case "EVAMin":
					nEquip.setAvoid((short) (nEquip.getAvoid()
							+ rand(stat.getValue().intValue(), sta.get("EVAMax").intValue())));
					break;
				case "SpeedMin":
					nEquip.setSpeed((short) (nEquip.getSpeed()
							+ rand(stat.getValue().intValue(), sta.get("SpeedMax").intValue())));
					break;
				case "JumpMin":
					nEquip.setJump((short) (nEquip.getJump()
							+ rand(stat.getValue().intValue(), sta.get("JumpMax").intValue())));
					break;
				case "MHPMin":
					nEquip.setHp(
							(short) (nEquip.getHp() + rand(stat.getValue().intValue(), sta.get("MHPMax").intValue())));
					break;
				case "MMPMin":
					nEquip.setMp(
							(short) (nEquip.getMp() + rand(stat.getValue().intValue(), sta.get("MMPMax").intValue())));
					break;
				case "MaxHPMin":
					nEquip.setHp((short) (nEquip.getHp()
							+ rand(stat.getValue().intValue(), sta.get("MaxHPMax").intValue())));
					break;
				case "MaxMPMin":
					nEquip.setMp((short) (nEquip.getMp()
							+ rand(stat.getValue().intValue(), sta.get("MaxMPMax").intValue())));
					break;
				}
			}
	...
	public final boolean canEquip(final Map<String, Integer> stats, final int itemid, final int level, final int job,
		...
		if ((level + supremacy) >= (reqLevel != 0 ? reqLevel
	...
	public final Item scrollEquipWithId(final Item equip, final Item scrollId, final boolean ws,
		...
		if (equip.getType() == 1) { // See Item.java
			...
			final int succ = (GameConstants.isTablet(scrollId.getItemId())
					...
					: ((GameConstants.isEquipScroll(scrollId.getItemId())
							|| GameConstants.isPotentialScroll(scrollId.getItemId()) || !stats.containsKey("success")
									? 0 : stats.get("success"))));
			final int curse = (GameConstants.isTablet(scrollId.getItemId())
					? GameConstants.getCurseTablet(scrollId.getItemId(), nEquip.getLevel())
					: ((GameConstants.isEquipScroll(scrollId.getItemId())
							|| GameConstants.isPotentialScroll(scrollId.getItemId()) || !stats.containsKey("cursed") ? 0
									: stats.get("cursed"))));
			final int added = (ItemFlag.LUCKY_DAY.check(equip.getFlag()) ? 10 : 0)
					+ (chr.getTrait(MapleTraitType.craft).getLevel() / 10);
			int success = succ + (vegas == 5610000 && succ == 10 ? 20 : (vegas == 5610001 && succ == 60 ? 30 : 0))
					+ added;
			if (ItemFlag.LUCKY_DAY.check(equip.getFlag()) && !GameConstants.isPotentialScroll(scrollId.getItemId())
					&& !GameConstants.isEquipScroll(scrollId.getItemId())
			...
			if (GameConstants.isPotentialScroll(scrollId.getItemId())
					|| GameConstants.isEquipScroll(scrollId.getItemId())
					|| GameConstants.isSpecialScroll(scrollId.getItemId()) || Randomizer.nextInt(100) <= success) {
				...
				case 2049005: {
					if (eqstats.containsKey("tuc")
							&& nEquip.getLevel() + nEquip.getUpgradeSlots() < eqstats.get("tuc")) {
				...
				default: {
					if (GameConstants.icsog(scrollId.getItemId())) {// Incredible
																	// chaos
																	// scroll of
																	// goodnes
						...
						final int chanc = Math.max((scrollId.getItemId() == 2049300 || scrollId.getItemId() == 2049303
								|| scrollId.getItemId() == 2049306
										? 100
										: (scrollId.getItemId() == 2049308 ? 50
												: scrollId.getItemId() == 2049305 ? 60 : 80))
								- (nEquip.getEnhance() * 10), 10) + added;
						if (Randomizer.nextInt(100) > chanc) {
							return null; // destroyed, nib
						}
						for (int i = 0; i < (scrollId.getItemId() == 2049308 ? 5
								: (scrollId.getItemId() == 2049305 ? 4
										: (scrollId.getItemId() == 2049304 ? 3
												: scrollId.getItemId() == 2049309 ? 2 : 1))); i++) {
							if (nEquip.getStr() > 0 || Randomizer.nextInt(50) == 1) { // 1/50
								nEquip.setStr((short) (nEquip.getStr() + Randomizer.nextInt(5)));
							}
							if (nEquip.getStr() > 0
									&& (GameConstants.isTyrant(nEquip.getItemId()) || Randomizer.nextInt(1) == 1)) {
								nEquip.setStr((short) (nEquip.getStr() + 22));
							}
							if (nEquip.getStr() > 0
									&& (GameConstants.isNovaGear(nEquip.getItemId()) || Randomizer.nextInt(1) == 1)) {
							...
							if (nEquip.getWatk() > 0 && !GameConstants.isWeapon(nEquip.getItemId())
									&& Randomizer.nextInt(5) == 1
									|| Randomizer.nextInt(5) == 1 && !GameConstants.isWeapon(nEquip.getItemId())) {
								nEquip.setWatk((short) (nEquip.getWatk() + Randomizer.nextInt(5)));
							}
							if (nEquip.getWatk() > 0 && GameConstants.isTyrant(nEquip.getItemId())
									&& nEquip.getEnhance() > 3) {
								nEquip.setWatk((short) (nEquip.getWatk() + 5));
							}
							if (nEquip.getWatk() > 0 && GameConstants.isNovaGear(nEquip.getItemId())
									&& nEquip.getEnhance() > 5) {
							...
							if (nEquip.getMatk() > 0 && GameConstants.isTyrant(nEquip.getItemId())
									&& nEquip.getEnhance() > 3) {
								nEquip.setMatk((short) (nEquip.getMatk() + 5));
							}
							if (nEquip.getMatk() > 0 && GameConstants.isNovaGear(nEquip.getItemId())
									&& nEquip.getEnhance() > 5) {
						...
						if (nEquip.getState() <= 17 && (scrollId.getItemId() / 100 == 20497)) {
							...
							final int chanc = (scrollId.getItemId() == 5534000 || scrollId.getItemId() == 2049402
									|| scrollId.getItemId() == 2049406
											? 100
											: scrollId.getItemId() == 2049701 ? 80
													: (scrollId.getItemId() == 2049400
															|| scrollId.getItemId() == 2049407
															|| scrollId.getItemId() == 2049413 ? 90 : 70))
									+ added;
							if (Randomizer.nextInt(100) > chanc) {
								return null; // destroyed, nib
							}
	...
	public final boolean isDropRestricted(final int itemId) {
		...
		return ((i.flag & 0x200) != 0 || (i.flag & 0x400) != 0 || GameConstants.isDropRestricted(itemId))
				&& (itemId == 3012000 || itemId == 3012015 || itemId / 10000 != 301) && itemId != 2041200
				&& itemId != 5640000 && itemId != 4170023 && itemId != 2040124 && itemId != 2040125 && itemId != 2040126
				&& itemId != 2040211 && itemId != 2040212 && itemId != 2040227 && itemId != 2040228 && itemId != 2040229
				&& itemId != 2040230 && itemId != 1002926 && itemId != 1002906 && itemId != 1002927;
	}
	...
	public final boolean cantSell(final int itemId) { // true = cant sell, false
														// = can sell
		final ItemInformation i = getItemInformation(itemId);
		if (i == null) {
			return false;
	...
	public void initItemRewardData(ResultSet sqlRewardData) throws SQLException {
		...
		add.period = (add.itemid == 1122017 ? Math.max(sqlRewardData.getInt("period"), 7200)
	...
	public void finalizeEquipData(ItemInformation item) {
		...
		if (item.equipStats.size() > 0) {
			...
			if (item.equipStats.get("cash") != null && item.eq.getCharmEXP() <= 0) { // set
																						// the
																						// exp
				short exp = 0;
				int identifier = itemId / 10000;
				if (GameConstants.isWeapon(itemId) || identifier == 106) { // weapon
																			// overall
					exp = 60;
				} else if (identifier == 100) { // hats
					exp = 50;
				} else if (GameConstants.isAccessory(itemId) || identifier == 102 || identifier == 108
						|| identifier == 107) { // gloves shoes accessory
					exp = 40;
				} else if (identifier == 104 || identifier == 105 || identifier == 110) { // top
																							// bottom
																							// cape
					exp = 30;
				}
	...
	public void initItemInformation(ResultSet sqlItemData) throws SQLException {
		...
		ret.itemId = itemId;
		ret.slotMax = GameConstants.getSlotMax(itemId) > 0 ? GameConstants.getSlotMax(itemId)
				: sqlItemData.getShort("slotMax");
