...
public class StatsHandling {
	...
	public static void DistributeAP(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		...
		if (chr.getRemainingAp() > 0) {
			...
			case 0x2000: // MP fix from RZ was 0x800
				...
				} else if (GameConstants.isDemonSlayer(job) || GameConstants.isAngelicBuster(job)
						|| GameConstants.isDemonAvenger(job)) { // Demon,
																// Angelic
																// Buster
					return;
	...
	public static void DistributeSP(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		...
		if (GameConstants.isBeginnerJob(skillid / 10000) && (skillid % 10000 == 1000 || skillid % 10000 == 1001
				|| skillid % 10000 == 1002 || skillid % 10000 == 2)) {
		...
		for (Pair<String, Integer> ski : skill.getRequiredSkills()) {
			if (ski.left.equals("level")) {
				...
				if (chr.getSkillLevel(SkillFactory.getSkill(left)) < ski.right) {
					...
					// "Trying to learn a skill without the required skill (" +
					// skillid + ")");
					return;
				}
			}
		...
		if (skill.isInvisible() && chr.getSkillLevel(skill) == 0) {
			if ((skill.isFourthJob() && chr.getMasterLevel(skill) == 0)
					|| (!skill.isFourthJob() && maxlevel < 10 && !GameConstants.isDualBlade(chr.getJob())
							&& !isBeginnerSkill && chr.getMasterLevel(skill) <= 0)) {
				c.getSession().write(CWvsContext.enableActions());
				// AutobanManager.getInstance().addPoints(c, 1000, 0, "Illegal
				// distribution of SP to invisible skills (" + skillid + ")");
				return;
			}
		}
		...
		if ((remainingSp >= amount && curLevel + amount <= maxlevel) && skill.canBeLearnedBy(chr.getJob())) {
			...
			chr.updateSingleStat(MapleStat.AVAILABLESP, 0); // we don't care the
															// value here
			chr.changeSingleSkillLevel(skill, (byte) (curLevel + amount), chr.getMasterLevel(skill));
			// } else if (!skill.canBeLearnedBy(chr.getJob())) {
			// AutobanManager.getInstance().addPoints(c, 1000, 0, "Trying to
			// learn a skill for a different job (" + skillid + ")");
		} else {
	...
	public static final void AutoAssignAP(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		/*
		 * if (chr.getRemainingAp() == amount + amount2) { switch (PrimaryStat)
		 * {
		 */ // testing fix from RZ
