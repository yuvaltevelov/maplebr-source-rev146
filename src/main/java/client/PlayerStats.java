...
public class PlayerStats implements Serializable {
	...
	public transient int localstr, localdex, localluk, localint_, localmaxhp, localmaxmp, magic, watk, hands, accuracy,
			targetPlus;
	...
	public transient double expBuff, dropBuff, mesoBuff, cashBuff, mesoGuard, mesoGuardMeso, expMod, dropMod,
	...
	public transient int recoverHP, recoverMP, mpconReduce, mpconPercent, incMesoProp, reduceCooltime, coolTimeR,
			...
			ignoreDAM_rate, hpRecover, hpRecoverProp, hpRecoverPercent, mpRecover, mpRecoverProp, RecoveryUP, BuffUP,
			RecoveryUP_Skill, BuffUP_Skill, incAllskill, combatOrders, ignoreTargetDEF, defRange, BuffUP_Summon,
			dodgeChance, speed, jump, harvestingTool, equipmentBonusExp, cashMod, levelBonus, ASR, TER, pickRate,
			decreaseDebuff, equippedFairy, equippedSummon, pvpDamage, hpRecoverTime, mpRecoverTime, dot, dotTime,
			questBonus, pvpRank, pvpExp, wdef, mdef, trueMastery, damX;
	...
	public void recalcLocalStats(boolean first_login, MapleCharacter chra) {
		if (chra.isClone()) {
			return; // clones share PlayerStats objects and do not need to be
					// recalculated
		}
		...
		while (itera.hasNext()) {
			...
			if (equip.getPosition() == -11) {
				if (GameConstants.isMagicWeapon(equip.getItemId())) {
					...
					if (eqstat != null) { // slow, poison, darkness, seal,
											// freeze
			...
			default:
				for (int eb_bonus : GameConstants.Equipments_Bonus) {
					if (equip.getItemId() == eb_bonus) {
						// equipmentBonusExp +=
						// GameConstants.Equipment_Bonus_EXP(eb_bonus);
						break;
					}
				}
				break;
			...
			if (set != null && set > 0) {
				...
				setHandling.put(set, value); // id of Set, number of items to go
												// with the set
			}
			if (equip.getIncSkill() > 0 && ii.getEquipSkills(equip.getItemId()) != null) {
				for (final int zzz : ii.getEquipSkills(equip.getItemId())) {
					...
					if (skil != null && skil.canBeLearnedBy(chra.getJob())) { // dont
																				// go
																				// over
																				// masterlevel
																				// :D
			...
			if (equip.getState() >= 17) {
				final int[] potentials = { equip.getPotential1(), equip.getPotential2(), equip.getPotential3(),
						equip.getBonusPotential1(), equip.getBonusPotential2() };
		...
		for (Item item : chra.getInventory(MapleInventoryType.CASH).newList()) {
			if (item.getItemId() / 100000 == 52) {
				if (expMod < 3 && (item.getItemId() == 5211060 || item.getItemId() == 5211050
						|| item.getItemId() == 5211051 || item.getItemId() == 5211052 || item.getItemId() == 5211053
		...
		if (chra.getEventInstance() != null && chra.getEventInstance().getName().startsWith("PVP")) { // hack
			...
			if (eff != null && eff.getSourceId() % 10000 == 1105) { // ice
																	// knight
				localmaxhp = 500000;
				localmaxmp = 500000;
			}
	...
	private void handlePassiveSkills(MapleCharacter chra) {
		...
		for (Skill sk : chra.getSkills().keySet()) {
			if (sk.getPsd() == 1) {
				...
				psdSkill.mid = sk.getPsdDamR(); // This only handles damage
												// increases; some skills have
												// effects other than that, so
												// TODO
		...
		case 2003: // Phantom noob
			...
			bx = SkillFactory.getSkill(20030206); // +40 DEX, Base Avoidability:
													// +20%
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
		...
		case 572: { // Jett
			...
			if (bof > 0) {
				...
				jump += eff.getPassiveJump();
				speed += eff.getSpeedMax(); // TODO: split speed max and speed.
											// (speed have a limit, while
											// speedMax will add to the max)
			} // TODO: research more on percentage hp/mp and stats, which
				// doesn't take effect to note.
		...
		case 422: { // Shadower
			...
			if (chra.getJob() == 411 || chra.getJob() == 412) {
				...
				bx = SkillFactory.getSkill(4110012); // Expert Throwing Star
														// Handling
				bof = chra.getTotalSkillLevel(bx);
				if (bof > 0) {
					eff = bx.getEffect(bof);
		...
		case 2218: {
			...
			bx = SkillFactory.getSkill(22170001); // magic mastery, this is an
													// invisible skill
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
		...
		case 4112:
			/*
			 * if (chra.getJob() >= 4111) { bx =
			 * SkillFactory.getSkill(41110006);// Willow Dodge bof =
			 * chra.getTotalSkillLevel(bx); if (bof > 0) { eff =
			 * bx.getEffect(bof); dodgeChance += bx.getEffect(bof).getER();
			 * dam_r *= (eff.getX() * eff.getY() + 100.0) / 100.0; bossdam_r *=
			 * (eff.getX() * eff.getY() + 100.0) / 100.0; } }
			 */
			if (chra.getJob() >= 4111) {
				...
				if (bof > 0) {
					eff = bx.getEffect(bof);
					dodgeChance += eff.getPercentAvoid();// dodgeChance +=
															// eff.getER();
				}
			}
			if (chra.getJob() >= 4110) {
		...
		case 2312: {
			...
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				ignoreTargetDEF += bx.getEffect(bof).getX(); // or should we do
																// 100?
			}
		...
		case 434: {
			...
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) { // Fatal Blow, Slash Storm, Tornado Spin, Bloody
							// Storm, Upper Stab, and Flying Assaulter
				eff = bx.getEffect(bof);
		...
		case 5112: { // Mihile 4
			...
			bx = SkillFactory.getSkill(51120002); // Mihile || Final Attack &&
			...
			bx = SkillFactory.getSkill(51110002); // Mihile || Righteous
													// Indignation
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
			...
			bx = SkillFactory.getSkill(51120001); // Mihile || Expert Sword
													// Mastery
			bof = chra.getTotalSkillLevel(bx);
			if (bof > 0) {
				eff = bx.getEffect(bof);
				...
				for (int i = eq.getEquipLevel() - lvlz; i > 0; i--) {
					...
					if (GameConstants.getStatFromWeapon(eq.getItemId()) == null
						...
						for (int zzz : ii.getEquipSkills(eq.getItemId())) {
							final Skill skil = SkillFactory.getSkill(zzz);
							if (skil != null && skil.canBeLearnedBy(chr.getJob())) { // dont
																						// go
																						// over
																						// masterlevel
																						// :D
	...
	private void CalcPassive_SharpEye(final MapleCharacter player) {
		...
		case 2112: {
			...
			critlevel = player.getTotalSkillLevel(critSkill);
			if (critlevel > 0) {
				this.crit_rate += (short) ((critSkill.getEffect(critlevel).getX()
	...
	private void CalcPassive_Mastery(final MapleCharacter player) {
		...
		final int skil;
		final MapleWeaponType weaponType = GameConstants
		...
		case AXE1H:
		case BLUNT1H:
			skil = GameConstants.isResist(player.getJob()) ? 31100004
					: (GameConstants.isKOC(player.getJob()) ? 11100000 : (player.getJob() > 112 ? 1200000 : 1100000)); // hero/pally
			break;
		case AXE2H:
		case SWORD1H:
		case SWORD2H:
		...
		case KNUCKLE:
			...
			break;
		case GUN:
			skil = GameConstants.isResist(player.getJob()) ? 35100000
					: (GameConstants.isJett(player.getJob()) ? 5700000 : 5200000);
			break;
		case DUAL_BOW:
			skil = 23100005;
			break;
		...
		case STAFF:
			acc = false;
			skil = GameConstants.isResist(player.getJob()) ? 32100006
	...
	public final void calculateMaxBaseDamage(final int watk, final int pvpDamage, MapleCharacter chra) {
		if (watk <= 0) {
			...
			final MapleWeaponType weapon2 = weapon_item2 == null ? MapleWeaponType.NOT_A_WEAPON
					: GameConstants.getWeaponType(weapon_item2.getItemId());
			int mainstat, secondarystat, mainstatpvp, secondarystatpvp;
			final boolean mage = (job >= 200 && job <= 232) || (job >= 1200 && job <= 1212)
			...
			localmaxbasepvpdamage = weapon.getMaxDamageMultiplier() * (4 * mainstatpvp + secondarystatpvp)
					* (100.0f + (pvpDamage / 100.0f));
			localmaxbasepvpdamageL = weapon.getMaxDamageMultiplier() * (4 * mainstat + secondarystat)
					* (100.0f + (pvpDamage / 100.0f));
			if (weapon2 != MapleWeaponType.NOT_A_WEAPON && weapon_item != null && weapon_item2 != null) {
				...
				localmaxbasedamage = weapon.getMaxDamageMultiplier() * (4 * mainstat + secondarystat)
						* ((watk - (mage ? we2.getMatk() : we2.getWatk())) / 100.0f);
				localmaxbasedamage += weapon2.getMaxDamageMultiplier() * (4 * mainstat + secondarystat)
	...
	public final void relocHeal(MapleCharacter chra) {
		...
		shouldHealMP = GameConstants.isDemonSlayer(chra.getJob()) ? 0 : (3 + recoverMP + (localint_ / 10)); // i
																											// think
		mpRecoverTime = 0;
		hpRecoverTime = 0;
		if (playerjob == 111 || playerjob == 112) {
			final Skill effect = SkillFactory.getSkill(1110000); // Improving MP
																	// Recovery
			...
			final Skill effect = SkillFactory.getSkill(11110000); // Improving
																	// MP
																	// Recovery
			final int lvl = chra.getSkillLevel(effect);
			if (lvl > 0) {
			...
			final Skill effect = SkillFactory.getSkill(20020109); // Improving
																	// MP
																	// Recovery
			final int lvl = chra.getSkillLevel(effect);
			if (lvl > 0) {
			...
			final Skill effect = SkillFactory.getSkill(31110009); // Improving
																	// MP
																	// Recovery
			final int lvl = chra.getSkillLevel(effect);
			if (lvl > 0) {
		...
		if (chra.getChair() != 0) { // Is sitting on a chair.
			shouldHealHP += 99; // Until the values of Chair heal has been
								// fixed,
			shouldHealMP += 99; // MP is different here, if chair data MP = 0,
								// heal + 1.5
		} else if (chra.getMap() != null) { // Because Heal isn't multipled when
											// there's a chair :)
		...
		for (final Triple<String, String, String> add : additions) {
			...
			case "elemboost":
				...
				if (add.getMid().equals("elemVol") && (craft == null || craft != null
						&& chra.getTrait(MapleTraitType.craft).getLocalTotalExp() >= Integer.parseInt(craft))) {
				...
				break;
			case "mobcategory": // skip the category, thinkings too expensive to
								// have yet another Map<Integer, Integer> for
								// damage calculations
			...
			case "critical": // lv critical lvl?
				...
				if ((job != null && canJob || job == null) && (level != null && canLevel || level == null)) {
					...
					case "damage":
						...
						passive_sharpeye_percent += right; // ???CONFIRM - not
															// sure if this is
															// max or minCritDmg
						break;
					}
			...
			case "boss": // ignore prob, just add
				...
				break;
			case "mobdie": // lv, hpIncRatioOnMobDie, hpRatioProp,
							// mpIncRatioOnMobDie, mpRatioProp, modify =D, don't
							// need mob to die
				craft = ii.getEquipAddReqs(itemId, add.getLeft(), "craft");
	...
	public void handleItemOption(StructItemOption soc, MapleCharacter chra, boolean first_login,
		...
		recoverHP += soc.get("RecoveryHP"); // This shouldn't be here, set 4
											// seconds.
		recoverMP += soc.get("RecoveryMP"); // This shouldn't be here, set 4
											// seconds.
		...
		TER += soc.get("incTerR"); // elemental resistance = avoid element
									// damage from monster
		...
		dropBuff *= (100 + soc.get("incRewardProp")) / 100.0; // extra drop rate
																// for item
		...
		// TODO: Auto Steal potentials (modify handleSteal), potentials with
		// invincible stuffs, abnormal status duration decrease,
		// poison, stun, etc (uses level field -> cast disease to mob/player),
		// face?
	}
	...
	public final boolean isRangedJob(final int job) {
		return GameConstants.isJett(job) || GameConstants.isMercedes(job) || GameConstants.isCannon(job) || job == 400
				|| (job / 10 == 52) || (job / 100 == 3) || (job / 100 == 13) || (job / 100 == 14) || (job / 100 == 33)
				|| (job / 100 == 35) || (job / 10 == 41);
	}
