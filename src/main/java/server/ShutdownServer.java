	...
	public void run() {
		if (mode == 0) {
			...
			World.Broadcast.broadcastMessage(
					CWvsContext.broadcastMsg(0, "The world is going to shutdown soon. Please log off safely."));
			 ...
			 * HiredMerchantSave.Execute(this); synchronized (this) { try {
			 * wait(); } catch (InterruptedException ex) {
			 * Logger.getLogger(ShutdownServer.class.getName()).log(Level.
			 * SEVERE, null, ex); } } while (FinishedThreads.incrementAndGet()
			 * != HiredMerchantSave.NumSavingThreads) { synchronized (this) {
			...
			try {
				World.Broadcast.broadcastMessage(
						CWvsContext.broadcastMsg(0, "The world is going to shutdown now. Please log off safely."));
			...
			System.exit(0); // not sure if this is really needed for
							// ChannelServer
		}
	}
}
