...
public class Equip extends Item implements Serializable {
	...
	// charm: -1 = has not been initialized yet, 0 = already been worn, >0 = has
	// teh charm exp
	private byte upgradeSlots = 0, level = 0, vicioushammer = 0, enhance = 0, enhanctBuff = 0, reqLevel = 0,
			yggdrasilWisdom = 0, bossDamage = 0, ignorePDR = 0, totalDamage = 0, allStat = 0, karmaCount = -1;
	private short str = 0, dex = 0, _int = 0, luk = 0, hp = 0, mp = 0, watk = 0, matk = 0, wdef = 0, mdef = 0, acc = 0,
			avoid = 0, hands = 0, speed = 0, jump = 0, charmExp = 0, pvpDamage = 0;
	private int durability = -1, incSkill = -1, potential1 = 0, potential2 = 0, potential3 = 0, potential4 = 0,
			potential5 = 0, potential6 = 0, potential7 = 0, potential8 = 0, bonuspotential1 = 0, bonuspotential2 = 0,
			bonuspotential3 = 0, fusionAnvil = 0, socket1 = -1, socket2 = -1, socket3 = -1;
	...
	public int getEquipLevel() {
		...
		for (int i = levelz; (GameConstants.getStatFromWeapon(getItemId()) == null
				? (i <= GameConstants.getMaxLevel(getItemId())) : (i < GameConstants.getMaxLevel(getItemId()))); i++) {
			if (expz >= GameConstants.getExpForLevel(i, getItemId())) {
				levelz++;
				expz -= GameConstants.getExpForLevel(i, getItemId());
			} else { // for 0, dont continue;
	...
	public byte getState() {
		...
		if ((potential1 >= 40000) || (potential2 >= 40000) || (potential3 >= 40000) || (potential4 >= 40000)
				|| (potential5 >= 40000)) {
			return 20;
		}
		if ((potential1 >= 30000) || (potential2 >= 30000) || (potential3 >= 30000) || (potential4 >= 30000)
				|| (potential5 >= 30000)) {
			return 19;
		}
		if ((potential1 >= 20000) || (potential2 >= 20000) || (potential3 >= 20000) || (potential4 >= 20000)
				|| (potential5 >= 20000)) {
			return 18;
		}
		if (pots >= 1) {
	...
	public void resetPotential_Fuse(boolean half, int potentialState) { // maker
																		// skill
																		// -
																		// equip
																		// first
		...
		setPotential2((Randomizer.nextInt(half ? 5 : 10) == 0 ? potentialState : 0)); // 1/10
																						// chance
																						// of
																						// 3
																						// line
		setPotential3((Randomizer.nextInt(half ? 5 : 10) == 0 ? potentialState : 0)); // just
																						// set
																						// it
																						// theoretically
	...
	public void resetBonusPotential_Fuse(boolean half, int potentialState) { // maker
																				// skill
																				// -
																				// equip
																				// first
		...
		setBonusPotential2((Randomizer.nextInt(half ? 5 : 10) == 0 ? potentialState : 0)); // 1/10
																							// chance
																							// of
																							// 3
																							// line
	...
	public void resetPotential() {
		...
		setPotential1(rank);
		setPotential2((Randomizer.nextInt(10) == 0 ? rank : 0)); // 1/10 chance
																	// of 3 line
		setPotential3((Randomizer.nextInt(10) == 0 ? rank : 0)); // just set it
																	// theoretically
	...
	public void resetBonusPotential() {
		...
		setBonusPotential2((Randomizer.nextInt(10) == 0 ? rank : 0)); // 1/10
																		// chance
																		// of 3
																		// line
		setBonusPotential3(0); // just set it theoretically
	...
	public void renewPotential(int type, int line, int toLock, boolean bonus) { // 0
																				// =
																				// normal
																				// miracle
																				// cube,
		...
		if (bonus) {
			...
			rank = (Randomizer.nextInt(100) < 4 * miracleRate && getBonusState() != 20 ? -(getBonusState() + 1)
					: -(getBonusState())); // 4 % chance to up 1 tier
			setBonusPotential1(rank);
		} else {
			rank = type == 2 ? -18
					: type == 5
							? (Randomizer.nextInt(100) < 3 * miracleRate && getState() != 20 ? -20
									: Randomizer.nextInt(100) < 10 * miracleRate && getState() != 20 ? -(getState() + 1)
											: -(getState()))
							: (Randomizer.nextInt(100) < 4 * miracleRate && getState() != (type == 3 ? 20 : 19)
		...
		if (getPotential3() > 0 && !bonus) {
			...
			case 1: // premium-> suppose to be 25%
				setPotential2(Randomizer.nextInt(10) == 0 ? rank : 0); // 1/10
																		// chance
																		// of 3
																		// line
				break;
			case 2: // epic pot
				setPotential2(Randomizer.nextInt(10) <= 1 ? rank : 0); // 2/10
																		// chance
																		// of 3
																		// line
				break;
			case 3: // super
				setPotential2(Randomizer.nextInt(10) <= 2 ? rank : 0); // 3/10
																		// chance
																		// of 3
																		// line
				break;
			case 4: // revolutionary
				setPotential2(Randomizer.nextInt(10) <= 3 ? rank : 0); // 4/10
																		// chance
																		// of 3
																		// line
				break;
			case 5: // enlightening
				setPotential4(Randomizer.nextInt(10) <= 2 ? rank : 0); // 3/10
																		// chance
																		// of 3
																		// line
				// setBonusPotential2(Randomizer.nextInt(10) <= 2 ? rank : 0);
				// //3/10 chance of 3 line
				break;
			case 6: // master
				if (!bonus) {
					return;
				}
				setBonusPotential2(Randomizer.nextInt(10) <= 2 ? rank : 0); // 3/10
																			// chance
																			// of
																			// 3
																			// line
		...
		if (bonus) {
			if (getBonusPotential2() > 0) {
				...
				setBonusPotential1(Randomizer.nextInt(100) <= 1 ? rank : 0); // 2/100
																				// to
																				// get
																				// 5
																				// lines
	...
	public short getSocketState() {
		int flag = 0;
		if (socket1 != -1 || socket2 != -1 || socket3 != -1) { // Got empty
																// sockets show
																// msg
	...
	public static Equip calculateEquipStatsTest(Equip eq) {
		...
		eq.getStatsTest().put(EquipStat.KARMA_COUNT, Long.valueOf(eq.getKarmaCount())); // no
																						// count
																						// =
																						// -1
