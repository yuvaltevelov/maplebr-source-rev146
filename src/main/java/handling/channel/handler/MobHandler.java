...
public class MobHandler {
	...
	public static final void MoveMonster(final LittleEndianAccessor rh, final MapleClient c, final MapleCharacter chr) {
		...
		int level = 0;
		/*
		 * if (!useSkill && skill != 0) { chr.send(MobPacket.talkMonster(oid,
		 * moveid)); chr.message("몬스터톡" + moveid); }
		 */
		...
		if (monster != null && c != null) {
			c.getSession().write(MobPacket.moveMonsterResponse(oid, moveid, monster.getMp(),
		...
		if (res != null) {
			...
			map.broadcastMessage(chr,
					MobPacket.moveMonster(useSkill, skill, skill1, skill2, skill3, skill4, oid, startPos, res),
					monster.getPosition());
	...
	public static final void AttackFamiliar(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		if ((attackPair.isEmpty()) || (!chr.getCheatTracker().checkFamiliarAttack(chr))
				|| (attackPair.size() > f.targetCount)) {
			return;
		}
		...
		for (Triple attack : attackPair) {
			...
			if ((mons != null) && (mons.isAlive()) && (!mons.getStats().isFriendly()) && (mons.getLinkCID() <= 0)
					&& (((List) attack.right).size() <= f.attackCount)) {
				if ((chr.getTruePosition().distanceSq(mons.getTruePosition()) > 640000.0D)
						|| (chr.getSummonedFamiliar().getTruePosition()
								.distanceSq(mons.getTruePosition()) > GameConstants.getAttackRange(f.lt, f.rb))) {
				...
				if ((f.makeChanceResult()) && (mons.isAlive())) {
					for (MonsterStatus s : f.status) {
						mons.applyStatus(chr,
								new MonsterStatusEffect(s, Integer.valueOf(f.speed),
										MonsterStatusEffect.genericSkill(s), null, false),
								false, f.time * 1000, false, null);
					}
					if (f.knockback) {
	...
	public static final void TouchFamiliar(LittleEndianAccessor slea, MapleClient c, MapleCharacter chr) {
		...
		if ((!target.getStats().isFriendly()) && (chr.getCheatTracker().checkFamiliarAttack(chr))) {
			chr.getMap().broadcastMessage(chr,
					CField.touchFamiliar(chr.getId(), unk, target.getObjectId(), type, 600, damage),
					chr.getTruePosition());
