...
public class MapleQuestAction implements Serializable {
	...
	public MapleQuestAction(MapleQuestActionType type, ResultSet rse, MapleQuest quest, PreparedStatement pss,
		...
		case skill:
			...
			while (rs.next()) {
				boolean add = skill
						.add(new Triple<>(rs.getInt("skillid"), rs.getInt("skillLevel"), rs.getInt("masterLevel")));
			}
			rs.close();
			break;
		}
	...
	public void runStart(MapleCharacter c, Integer extSelection) {
		...
		case exp:
			...
			c.gainExp(intStore * GameConstants.getExpRate_Quest(c.getLevel()) * (c.getStat().questBonus)
					* ((c.getTrait(MapleTraitType.sense).getLevel() * 3 / 10) + 100) / 100, true, true, true);
			break;
		case item:
			...
			for (QuestItem item : items) {
				...
				if (count < 0) { // remove items
					...
					MapleInventoryManipulator.addById(c.getClient(), id, count, "", null, period, false,
	...
	public boolean checkEnd(MapleCharacter c, Integer extSelection) {
		...
		case money: {
			...
			if (c.getMeso() + meso < 0) { // Giving, overflow
				...
				return false;
			} else if (meso < 0 && c.getMeso() < Math.abs(meso)) { // remove
																	// meso
	...
	private static List<Integer> getJobBy5ByteEncoding(int encoded) {
		...
		if ((encoded & 0x40000000) != 0) { // i haven't seen any higher than
											// this o.o
			ret.add(3000);
			ret.add(3200);
			ret.add(3300);
