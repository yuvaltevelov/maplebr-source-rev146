...
public class ReactorActionManager extends AbstractPlayerInteraction {
	...
	public void setQuestStatus(int questId, byte status){
		Map<MapleQuest, MapleQuestStatus> map = this.getC().getPlayer().getQuest_Map();
		for(Entry<MapleQuest, MapleQuestStatus> quest : map.entrySet()){
			MapleQuest mQuest = quest.getKey();
			if(mQuest.getId() == questId){
				this.getC().getPlayer().forceCompleteQuest(mQuest.getId());
			}
