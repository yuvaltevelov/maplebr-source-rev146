	...
	public void hitReactor(int charPos, short stance, MapleClient c) {
		if (stats.getType(state) < 999 && stats.getType(state) != -1) {
			...
			if (!(stats.getType(state) == 2 && (charPos == 0 || charPos == 2))) { // next
				...
				if (stats.getNextState(state) == -1 || stats.getType(state) == 999) { // end
																						// of
																						// reactor
					if ((stats.getType(state) < 100 || stats.getType(state) == 999) && delay > 0) { // reactor
																									// broken
						map.destroyReactor(getObjectId());
					...
					boolean done = false;
					map.broadcastMessage(CField.triggerReactor(this, stance)); // magatia
																				// is
																				// weird
																				// cause
																				// full
																				...
																				// hat
																				// o.o
					if (state == stats.getNextState(state) || rid == 2618000 || rid == 2309000) { // current
																									// state
																									// =
																									// next
																									// state,
