...
public class PlayerNPC extends MapleNPC implements MapleCharacterLook {
	...
	public PlayerNPC(MapleCharacter cid, int npc, MapleMap map, MapleCharacter base) {
		...
		setCoords(base.getTruePosition().x, base.getTruePosition().y, 0, base.getFH()); // 0
																						// =
																						// facing
																						// dir?
																						// no
	...
	public static void updateByCharId(MapleCharacter chr) {
		if (World.Find.findChannel(chr.getId()) > 0) { // if character is in
														// cserv
	...
	public void saveToDB() {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement(
					"INSERT INTO playernpcs(name, hair, face, skin, x, y, map, charid, scriptid, foothold, dir, gender, pets, job, elf, faceMarking, ears, tail) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
			int k = 0;
