	...
	public static class Seal extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			if (splitted.length < 2) {
				...
				c.getSession().write(CField.sendBoxDebug(opcode, 2028162, items)); // sealed
																					// box
				return 1;
			}
		}
	...
	public static class Find extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			if (splitted.length == 1) {
				...
				if (type.equalsIgnoreCase("NPC")) {
					...
					for (MapleData mapAreaData : data.getChildren()) {
						for (MapleData mapIdData : mapAreaData.getChildren()) {
							mapPairList
									.add(new Pair<>(Integer.parseInt(mapIdData.getName()),
				...
				c.getSession()
						.write(NPCPacket.getNPCTalk(9010000, (byte) 0, sb.toString(), "00 00", (byte) 0, 9010000));
			}
			return 0;
		}
	}
			...
			if (player.getMapId() != 109010100) {
				...
				for (MapleCharacter chr : player.getMap().getCharacters()) {
					if (!chr.isIntern()) {
						...
						// MapleInventoryManipulator.removeById(chr.getClient(),
						// MapleInventoryType.USE, 2100067,
						// chr.getItemQuantity(2100067, false), true, true);
						// chr.gainItem(2100067, 30);
						// MapleInventoryManipulator.removeById(chr.getClient(),
						// MapleInventoryType.ETC, 4031868,
						// chr.getItemQuantity(4031868, false), true, true);
						// chr.gainItem(4031868, (short) 5);
						// chr.dropMessage(0, "You have been granted 5
						// jewels(lifes) and 30 bombs.");
						// chr.dropMessage(0, "Pick up as many bombs and jewels
						// as you can!");
						// chr.dropMessage(0, "Check inventory for Bomb under
						// use");
					}
				}
