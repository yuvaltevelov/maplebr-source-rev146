...
public abstract class AbstractPlayerInteraction {
	...
	public final void openNpc(int npc, String filename) {
		if(MapleServerConfig.isDebugMode){
			System.out.println("Trying to load npc " + npc + " with file name  + " + filename);
		}
	...
	public final void openNpc(MapleClient client, int npc, String filename) {
		if(MapleServerConfig.isDebugMode){
			System.out.println("Trying to load npc " + npc + " , filename , " + filename + " client ," + client.getAccID());
		}
	...
	public final void warp(final int map, final int portal) {
		...
		if (portal != 0 && map == c.getPlayer().getMapId()) { // test
			...
			if (portalPos.distanceSq(getPlayer().getTruePosition()) < 90000.0) { // estimation
				c.getSession().write(CField.instantMapWarp((byte) portal)); // until
																			// we
																			// get
																			// packet
																			// for
	...
	public final void gainNX(final int amount) {
		c.getPlayer().modifyCSPoints(4, amount, true); // theremk you can change
														// it to prepaid yae
														// since cspoiint is
														// xncredit so make it
														// prepaid
	...
	public final void gainItemPeriod(final int id, final short quantity, final int period) { // period
																								// is
																								// in
																								// days
	...
	public final void gainItemPeriod(final int id, final short quantity, final int period, boolean hours) { // period
																											// is
																											// in
																											// days
	...
	// public final void gainItem(final int id, final short quantity, final
	// boolean randomStats, final long period, boolean hours, final int slots,
	// boolean potential, final String owner) {
	// gainItem(id, quantity, randomStats, period, hours, slots, potential,
	// owner, c);
	// }
	...
	public final void gainItem(final int id, final short quantity, final boolean randomStats, final long period,
		...
		if (quantity >= 0) {
			...
			if (type.equals(MapleInventoryType.EQUIP) && !GameConstants.isThrowingStar(id)
				...
				MapleInventoryManipulator.addById(cg, id, quantity, owner == null ? "" : owner, null, period, hours,
	...
	public final void givePartyExp_PQ(final int maxLevel, final double mod, final List<MapleCharacter> party) {
		for (final MapleCharacter chr : party) {
			final int amount = (int) Math
					.round(GameConstants
							.getExpNeededForLevel(chr.getLevel() > maxLevel
									? (maxLevel + ((maxLevel - chr.getLevel()) / 10)) : chr.getLevel())
	...
	public final void gainExp_PQ(final int maxLevel, final double mod) {
		final int amount = (int) Math
				.round(GameConstants
						.getExpNeededForLevel(getPlayer().getLevel() > maxLevel
								? (maxLevel + (getPlayer().getLevel() / 10)) : getPlayer().getLevel())
	...
	public final void givePartyExp_PQ(final int maxLevel, final double mod) {
		...
		for (final MaplePartyCharacter chr : getPlayer().getParty().getMembers()) {
			...
			if (curChar != null
				...
				final int amount = (int) Math
						.round(GameConstants
								.getExpNeededForLevel(curChar.getLevel() > maxLevel
										? (maxLevel + (curChar.getLevel() / 10)) : curChar.getLevel())
	...
	public final void gainCloseness(final int closeness, final int index) {
		...
		if (pet != null) {
			...
			getClient().getSession().write(PetPacket.updatePet(pet,
	...
	public final void dojo_getUp() {
		...
		for (int i = 0; i < 3; i++) {
			c.getPlayer().updateInfoQuest(7281, "item=0;chk=0;cNum=0;sec=2;stage=0;lBonus=0"); // last
																								// stage
		}
		for (int i = 0; i < 2; i++) {
		...
		// c.getSession().write(InfoPacket.updateInfoQuest(1207,
		// "min=1;tuto=1")); //old - 1207, "pt=1;min=4;belt=1;tuto=1")); //todo
		// c.getSession().write(InfoPacket.updateInfoQuest(7281,
