...
public class FarmPacket {
	...
	public static byte[] enterFarm(MapleClient c) {
		...
		/*
		 * Farm House positions: 000 001 002 003 004 025 026 027 028 029 050 051
		 * 052 053 054 075 076 077 078 079 100 101 102 103 104 //104 is base
		 * position
		 */
		...
		for (int i = 0; i < 25 * 25; i++) { // 2D building at every position
			...
			mplew.writeInt(housePosition ? houseId : 0); // building that the
															// position contains
			mplew.writeInt(i == houseBase ? houseId : 0); // building that the
															// position bases
	...
	public static byte[] sendFarmRanking(MapleCharacter chr, List<Pair<MapleFarm, Integer>> rankings) {
		...
		for (Pair<MapleFarm, Integer> best : rankings) {
			mplew.writeInt(i); // Type; 0 = visitors 1 = playtime 2 =
								// combinations
	...
	public static byte[] updateAvatar(Pair<WorldOption, MapleCharacter> from, Pair<WorldOption, MapleCharacter> to,
			boolean change) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
