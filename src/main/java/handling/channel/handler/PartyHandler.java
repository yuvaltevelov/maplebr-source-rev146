...
public class PartyHandler {
	...
	public static final void DenyPartyRequest(LittleEndianAccessor slea, MapleClient c) {
		...
		if ((action == 50)) {
			...
			if ((chr != null) && (chr.getParty() == null) && (c.getPlayer().getParty() != null)
					&& (c.getPlayer().getParty().getLeader().getId() == c.getPlayer().getId())
					&& (c.getPlayer().getParty().getMembers().size() < 8)
					&& (c.getPlayer().getParty().getExpeditionId() <= 0)
					&& (chr.getQuestNoAdd(MapleQuest.getInstance(122901)) == null)
					&& (c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(122900)) == null)) {
				chr.setParty(c.getPlayer().getParty());
				World.Party.updateParty(c.getPlayer().getParty().getId(), PartyOperation.JOIN,
		...
		if ((c.getPlayer().getParty() == null)
				&& (c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(122901)) == null)) {
				...
				if (action == 35) {// was 31 // 35== inv
					...
					if (cfrom != null) {
						cfrom.getClient().getSession()
								.write(CWvsContext.PartyPacket.partyStatusMessage(23, c.getPlayer().getName()));
					}
				}
			} else {
	...
	public static void MemberSearch(LittleEndianAccessor slea, MapleClient c) {
		...
		c.getSession()
				.write(CWvsContext.PartyPacket.showMemberSearch(c.getPlayer().getMap().getCharactersThreadsafe()));
	}
	...
	public static final void PartyListing(LittleEndianAccessor slea, MapleClient c) {
		...
		case 159:
			...
			if ((c.getPlayer().getParty() == null) && (World.Party.searchParty(pst).size() < 10)) {
				...
				if (pst.exped) {
					c.getSession().write(CWvsContext.ExpeditionPacket
		...
		case 163:
			...
			if (party != null) {
				...
				if ((ps != null) && (c.getPlayer().getLevel() <= ps.getType().maxLevel)
						&& (c.getPlayer().getLevel() >= ps.getType().minLevel) && (party.getMembers().size() < 8)) {
					c.getPlayer().setParty(party);
					World.Party.updateParty(party.getId(), PartyOperation.JOIN, partyplayer);
					c.getPlayer().receivePartyMemberHP();
					c.getPlayer().updatePartyMemberHP();
			...
			if (theCh > 0) {
				...
				if ((invited != null) && (invited.getParty() == null) && (party != null)
					...
					if ((me != null) && (me.getAllMembers() < me.getType().maxMembers)
						...
						invited.getClient().getSession().write(
							...
							if ((expelled != null) && (expelled.isOnline())) {
								...
								if (partyIndexTo < exped.getParties().size()) {
									...
									chr.getClient().getSession()
											.write(CWvsContext.PartyPacket.partyCreated(party.getId()));
									chr.getClient().getSession()
											.write(CWvsContext.ExpeditionPacket.expeditionStatus(exped, true, false));
