...
public class CustomPacketHandler {
	...
	public static void handle(LittleEndianAccessor slea, MapleClient c) {
		if (slea.available() < 2) {
			c.getSession().write(CWvsContext.broadcastMsg(1,
					"Exception: Length is too short. Excepted: 4 Bytes.\r\nUse action 0x00 for help.\r\nPacket Structure:\r\n[(Short)Packet Header]\r\n[(Short)Action]"));
			dispose(c);
			return;
		}
		...
		case 0x01:
			if (slea.available() < 2) {
				c.getSession().write(CWvsContext.broadcastMsg(1,
						"Exception: Length is too short. Excepted: 6 Bytes.\r\nUse action 0x00 for help.\r\nPacket Structure:\r\n[(Short)Packet Header]\r\n[(Short)Action]\r\n[(Short)String Length]\r\n[(String)ASCII String]"));
				dispose(c);
				return;
			}
		...
		case 0x04:
			if (c.getPlayer() == null || slea.available() < 1) {
				c.getSession().write(CWvsContext.broadcastMsg(1,
						"Exception: Length is too short. Excepted: 5 Bytes.\r\nUse action 0x00 for help.\r\nPacket Structure:\r\n[(Short)Packet Header]\r\n[(Short)Action]\r\n[(Byte)GM Level]"));
				dispose(c);
				return;
			}
			gm = slea.readByte();
		...
		case 0x08:
			if (c.getPlayer() == null || slea.available() < 3) {
				c.getSession().write(CWvsContext.broadcastMsg(1,
						"Exception: Length is too short. Excepted: 7 Bytes.\r\nUse action 0x00 for help.\r\nPacket Structure:\r\n[(Short)Packet Header]\r\n[(Short)Action]\r\n[(Short)String Length]\r\n[(String)ASCII String]\r\n[(Byte)GM Level]"));
				dispose(c);
				return;
			}
			charname = slea.readMapleAsciiString();
		...
		case 0x10:
			if (c.getPlayer() == null || slea.available() < 6) {
				c.getSession().write(CWvsContext.broadcastMsg(1,
						"Exception: Length is too short. Excepted: 10 Bytes.\r\nUse action 0x00 for help.\r\nPacket Structure:\r\n[(Short)Packet Header]\r\n[(Short)Action]\r\n[(Int)Item ID]\r\n[(Short)Quantity]"));
				dispose(c);
				return;
			}
	...
	public static void dispose(MapleClient c) {
		if (c.getPlayer() == null) {
			c.getSession().write(LoginPacket.getLoginFailed(1)); // Login Screen
																	// Dispose
		} else {
			c.getSession().write(CWvsContext.enableActions()); // In Game
																// Dispose
		}
	}
}
