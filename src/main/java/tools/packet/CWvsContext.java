...
public class CWvsContext {
	...
	public static byte[] updateSkill(Map<Skill, SkillEntry> update, int skillid, int level, int masterlevel,
			long expiration) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] getLieDetector(byte type, String tester) {
		...
		mplew.writeShort(SendPacketOpcode.LIE_DETECTOR.getValue()); // 2A 00 01
																	// 00 00 00
		mplew.write(type); // 1 = not attacking, 2 = tested, 3 = going through,
							// 4 save screenshot
		switch (type) {
		...
		case 5:
			...
			break;
		case 6:
			mplew.write(4); // 2 or anything else, 2 = with maple admin picture,
							// basicaly manager's skill?
			mplew.write(1); // if > 0, then time = 60,000..maybe try < 0?
			// mplew.writeInt(size);
			// mplew.write(byte); // bytes
			break;
		case 7:// send this if failed
				// 2 = You have been appointed as a auto BOT program user and
				// will be restrained.
		...
		case 9:
			...
			mplew.write(0);
			break;
		case 8: // save screen shot.. it appears that you may be using a
				// macro-assisted program
			mplew.write(0); // 2 or anything else , 2 = show msg, 0 = none
			mplew.writeMapleAsciiString(""); // file name
			break;
		case 10: // no save
			mplew.write(0); // 2 or anything else, 2 = show msg
			mplew.writeMapleAsciiString(""); // ?? // hi_You have passed the lie
												// detector test
			break;
		default:
			mplew.write(0);
	...
	public static byte[] lieDetector(byte mode, byte action, byte[] image, String str1, String str2, String str3) {
		...
		mplew.writeShort(SendPacketOpcode.LIE_DETECTOR.getValue());
		mplew.write(mode);
		mplew.write(action); // 2 = show msg/save screenshot/maple admin
								// picture(mode 6)
		if (mode == 6) {
		...
		if (mode != 5) {
			if (mode == 10) {
				mplew.writeMapleAsciiString(str2); // passed lie detector
													// message
			} else {
				if (mode != 8) {
				}
				mplew.writeMapleAsciiString(str2); // failed lie detector, file
													// name (for screenshot)
			}
		}
	...
	public static byte[] updateExpPotion(int mode, int id, int itemId, boolean firstTime, int level,
			int potionDstLevel) {
		MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		...
		if (id != 0) {
			...
			if (mode == 2) {
				mplew.write(firstTime ? 1 : 0); // 1 on first time then it turns
												// 0
	...
	public static byte[] charInfo(MapleCharacter chr, boolean isSelf) {
		...
		if (marriage != null && marriage.getId() != 0) {
			...
			mplew.writeAsciiString(marriage.getHusbandName(), 13); // husband
																	// name
		...
		for (MaplePet pet : chr.getSummonedPets()) {
			if (index == 1) { // please test if this doesn't d/c when viewing
								// multipets
				mplew.write(index);
			}
			...
			Item inv = chr.getInventory(MapleInventoryType.EQUIPPED)
					.getItem((short) (byte) (index == 2 ? -130 : index == 1 ? -114 : -138));
		...
		/*
		 * if ((chr.getInventory(MapleInventoryType.EQUIPPED).getItem((byte)
		 * -18) != null) &&
		 * (chr.getInventory(MapleInventoryType.EQUIPPED).getItem((byte) -19) !=
		 * null)) { MapleMount mount = chr.getMount(); mplew.write(1);
		 * mplew.writeInt(mount.getLevel()); mplew.writeInt(mount.getExp());
		 * mplew.writeInt(mount.getFatigue()); } else { mplew.write(0); }
		 */
	...
	private static byte[] broadcastMessage(int type, int channel, String message, boolean megaEar) {
		...
		case 20:
			...
			break;
		/*
		 * case 24: mplew.writeShort(0); break;
		 */
		case 4:
		case 5:
		case 7:
	...
	// mark packet
	public static byte[] giveMarkOfTheif(int cid, int oid, int skillid, List<MapleMonster> monsters, Point p1, Point p2,
			int javelin) {
	...
	public static byte[] gmBoard(int increnement, String url) { // Test if it
																// DOES work
																// with the
																// open_UI
																// packet.
	...
	// public static byte[] getMulungRanking(MapleClient c,
	// List<DojoRankingInfo> all) {
	...
	public static class PartyPacket {
		...
		public static void addPartyStatus(int forchannel, MapleParty party, MaplePacketLittleEndianWriter lew,
				boolean leaving) {
	...
	public static class InfoPacket {
		...
		public static byte[] GainEXP_Monster(int gain, boolean white, int partyinc, int Class_Bonus_EXP,
	...
	public static class BuffPacket {
		...
		public static byte[] giveForeignPirate(Map<MapleBuffStat, Integer> statups, int duration, int cid,
				int skillid) {
		...
		public static byte[] giveBuff(int buffid, int bufflength, Map<MapleBuffStat, Integer> statups,
				...
				if (stat.getKey().canStack()) {
					if (!stacked) {
						...
						if (GameConstants.isSpecialStackBuff(stat.getKey())) {
							mplew.writeZeroBytes(1); // not sure where this part
														// comes
						}
						stacked = true;
					}
			...
			if (buffid == 32001003 || buffid == 32120013 || buffid == 32101003 || buffid == 32120014
					|| buffid == 32111012 || buffid == 32120015 || buffid == 2221054 || buffid == 36121003
					|| buffid == 11101022 || buffid == 11111022 || buffid == 2311012 || buffid == 100001263
					|| buffid == 100001264) {
				mplew.write(1);
			}
			if (!isAura) {
			...
			mplew.writeShort(0);
			if (buffid == 32110000 || buffid == 32111012 || buffid == 2221054 || buffid == 11101022
					|| buffid == 11111022 || buffid == 2311012) {
				mplew.write(1);
			...
			// mplew.writeZeroBytes((buffid != 36111003 && buffid != 101120109
			// && buffid != 27121005) ? 3 : 0); // 197
			...
			/*
			 * if (buffid == 27111004) { mplew.write(0); mplew.writeShort(1000);
			 * mplew.writeShort(0); }
			 */
			if (buffid == 27110007) {
				mplew.write(0);
			...
			if ((statups.containsKey(MapleBuffStat.JUMP)) || (statups.containsKey(MapleBuffStat.SPEED))
					|| (statups.containsKey(MapleBuffStat.MORPH))
					|| (statups.containsKey(
							MapleBuffStat.GHOST_MORPH)) /*
														 * ||
														 * (statups.containsKey(
														 * MapleBuffStat.
														 * MAPLE_WARRIOR))
														 */
					|| (statups.containsKey(MapleBuffStat.MONSTER_RIDING))
					|| (statups.containsKey(MapleBuffStat.DASH_SPEED)) || (statups.containsKey(MapleBuffStat.DASH_JUMP))
					|| (statups.containsKey(MapleBuffStat.SOARING)) || (statups.containsKey(MapleBuffStat.YELLOW_AURA))
					|| (statups.containsKey(MapleBuffStat.SNATCH)) || (statups.containsKey(MapleBuffStat.INDIE_SPEED))
					|| (statups.containsKey(MapleBuffStat.ANGEL_JUMP))
					|| (statups.containsKey(MapleBuffStat.ENERGY_CHARGE))
					|| (statups.containsKey(MapleBuffStat.MECH_CHANGE))) {
				mplew.write(4);
			}
			...
			mplew.writeZeroBytes(69); // make sure no dc incase not enough
										// length
			...
			mplew.write(0);
			mplew.write(0);
			// System.err.println("sent Buff with buffid: " + buffid +" as
			// packet: "+ mplew.toString());
			return mplew.getPacket();
		}
		 ...
		 * 
		 * PacketHelper.writeMask(mplew, statups); for (MapleBuffStat z :
		 * statups) { if (z.canStack()) { mplew.writeInt(0); //amount of buffs
		 * still in the stack? dunno mans } } mplew.write(3); mplew.write(1);
		 * mplew.writeLong(0); mplew.writeLong(0); mplew.writeLong(0);
		...
		public static byte[] giveAriaBuff(Map<MapleBuffStat, Integer> statups, int bufflevel, int buffid,
				int bufflength) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
			...
			// 00 80 00 00 00 00 00 00 00 00 00 00 40 00 00 00 00 00 00 00 00 00
			// 00 00 00 00"));
		...
		public static byte[] giveForeignBuff(int cid, Map<MapleBuffStat, Integer> statups, MapleStatEffect effect) {
			...
			for (Entry<MapleBuffStat, Integer> statup : statups.entrySet()) {
				if (statup.getKey() == MapleBuffStat.SHADOWPARTNER || statup.getKey() == MapleBuffStat.MECH_CHANGE
						...
						|| statup.getKey() == MapleBuffStat.WK_CHARGE || statup.getKey() == MapleBuffStat.DAMAGE_R
			...
			if (effect.getSourceId() == 13101024) {
				...
				mplew.writeZeroBytes(3);
						 ...
						 * effect.getSourceId() == 1101013 ||
						 * effect.getSourceId() == 1120003 ||
						 */ effect.getSourceId() == 11111001 || effect.getSourceId() == 11110005) { // 콤보
																									// 어택
				mplew.writeLong(1);
				mplew.writeLong(0);
				...
				mplew.writeShort(0);
					...
					|| effect.getSourceId() == 61121053) {// KAISER BUFFS!
				/*
				 * mplew.writeLong(0); // old kaiser mplew.writeLong(0);
				 * mplew.writeZeroBytes(5);
				 */
				mplew.writeInt(2);
				 ...
				 * else if (effect.getSourceId() == 3101004 ||
				 * effect.getSourceId() == 3201004 || effect.getSourceId() ==
				 * 13101003 || effect.getSourceId() == 33101003) {
				 * mplew.writeLong(0); mplew.writeLong(0); }
				 */ else if (effect.getSourceId() == 30001001 || effect.getSourceId() == 30011001
					|| effect.getSourceId() == 2311009) {
				mplew.writeLong(0);
				mplew.writeLong(0);
				mplew.write(0);
			} else if (effect.getSourceId() == 1221004 || effect.getSourceId() == 1211006
					|| effect.getSourceId() == 1211008 || effect.getSourceId() == 1211004) {
				mplew.writeShort(0);
				mplew.writeLong(4);
				mplew.writeLong(0);
				mplew.write(0);
				mplew.writeShort(602);
			} else if (effect.getSourceId() == 32120000 || effect.getSourceId() == 32001003
					|| effect.getSourceId() == 32110000 || effect.getSourceId() == 32111012
					|| effect.getSourceId() == 32120001 || effect.getSourceId() == 32101003) { // 오라
				mplew.writeLong(0);
				mplew.writeLong(0);
				mplew.writeZeroBytes(5);
			} else {
	...
	public static class InventoryPacket {
		...
		public static byte[] moveAndMergeWithRestInventoryItem(MapleInventoryType type, short src, short dst,
				short srcQ, short dstQ, boolean bag, boolean switchSrcDst, boolean bothBag) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		...
		public static byte[] updateSpecialItemUse(Item item, byte invType, short pos, boolean theShort,
				MapleCharacter chr) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
		...
		public static byte[] scrolledItem(Item scroll, MapleInventoryType inv, Item item, boolean destroyed,
				boolean potential, boolean equipped) {
			MaplePacketLittleEndianWriter mplew = new MaplePacketLittleEndianWriter();
	...
	public static byte[] magicWheel(int type, List<Integer> items, String data, int endSlot) {
		...
		case 3:
			...
			mplew.writeMapleAsciiString(data); // nexon encrypt the item and
			...
			case 9:
				...
				if (rewards.size() > 0) {
					for (int i = 0; i < rewards.size(); i++) {
						...
						mplew.writeInt(empty ? 0 : reward.getId()); // 0 = blank
																	// 1+ = gift
						if (!empty) {
							if ((option & 1) != 0) {
								mplew.writeLong(reward.getReceiveDate()); // start
																			// time
								mplew.writeLong(reward.getExpireDate()); // end
																			// time
								mplew.writeLong(reward.getReceiveDate()); // start
																			// time
								mplew.writeLong(reward.getExpireDate()); // end
							...
							if ((option & 2) != 0) { // nexon do here a3 & 2
														// when a3 is 9
								mplew.writeInt(0);
								mplew.writeInt(0);
								mplew.writeInt(0);
							...
							mplew.writeInt(reward.getType()); // type 3 = maple
																// point 4 =
																// mesos 5 = exp
							mplew.writeInt(reward.getItem()); // item id
							mplew.writeInt(/* itemQ */reward.getItem() > 0 ? 1 : 0); // item
																						// quantity
																						// (?)
							mplew.writeInt(0);
							mplew.writeLong(0L);
							mplew.writeInt(0);
							mplew.writeInt(reward.getMaplePoints()); // maple
																		// point
																		// amount
