...
public class InterServerHandler {
	...
	public static void EnterCS(final MapleClient c, final MapleCharacter chr) {
		if (chr.hasBlockedInventory() || chr.getMap() == null || chr.getEventInstance() != null
			...
			if (farmtransfer != null) {
				c.getSession()
						.write(FarmPacket.farmMessage("You cannot move into Cash Shop while visiting your farm, yet."));
			}
	...
	public static void Loggedin(final int playerid, final MapleClient c) {
		...
		try {
			...
			if (transfer == null) { // Player isn't in storage, probably isn't
									// CC
			...
			c.getSession().write(CWvsContext.updateSkills(c.getPlayer().getSkills(), false));// skill
																								// to
																								// 0
																								// "fix"
			...
			int[] warriorz = { 25121131, 20050012, 20050073, 80001155, 80000000, 80000001, 80000002, 80000005, 80000006,
					80001140, 80000050, 80000047, 80001040, 20051284, 20050285, 20051287, 25100010, 20050286, 20051251,
					25120113, 25121116, 25001000, 25111111, 25120214, 25111206, 25120133, 25121030, 25121131, 71000251,
					25110107, 25120112, 25110108, 25121108, 25120115, 25121211, 25120110, 25121209, 25100106, 25110210,
					25111209, 25100009, 25100108, 25101111, 25100107, 25001204, 25000105, 25101205, 25101000 };
			for (int i = 0; i < warriorz.length; i++) {
				c.getSession()
						.write(CWvsContext.updateSkill(list, warriorz[i],
								c.getPlayer().getOriginSkillLevel(warriorz[i]),
								c.getPlayer().getMasterLevel(warriorz[i]), -1L));
			}
			...
			if (player.getStat().equippedSummon > 0) {
				SkillFactory.getSkill(
						player.getStat().equippedSummon + (GameConstants.getBeginnerJob(player.getJob()) * 1000))
						.getEffect(1).applyTo(player);
			}
			...
			c.getSession().write(
					CField.quickSlot(stat != null && stat.getCustomData() != null ? stat.getCustomData() : null));
			...
			for (short slot : slots) {
				if (GameConstants.isIllegalItem(equipped.getItem(slot).getItemId())) {
					MapleInventoryManipulator.removeFromSlot(player.getClient(), MapleInventoryType.EQUIPPED, slot,
							(short) 1, false);
				}
			}
			...
			if (!GameConstants.isZero(player.getJob())) { // tell all players 2
															// login so u can
															// remove this from
															// ther
