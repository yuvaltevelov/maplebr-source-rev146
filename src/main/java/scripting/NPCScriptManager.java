...
public class NPCScriptManager extends AbstractScriptManager {
	...
	public final void start(final MapleClient c, final int npc, String script) {
		...
		try {
			if (!cms.containsKey(c) && c.canClickNPC()) {
				Invocable iv = getInvocable("npc/" + npc + ".js", c, true); // safe
																			// disposal
				if (script != null) {
					iv = getInvocable("npc/" + script + ".js", c, true); // safe
																			// disposal
				}
				if (iv == null) {
					iv = getInvocable("npc/notcoded.js", c, true); // safe
																	// disposal
					if (iv == null) {
						dispose(c);
						return;
	...
	public final void startItemScript(final MapleClient c, final int npc, final String script) {
		...
		try {
			...
			FileoutputUtil.log(FileoutputUtil.ScriptEx_Log,
					"Error executing Item script, SCRIPT : " + script + ". " + e);
			dispose(c);
		} finally {
			lock.unlock();
		}
