...
public class CashShopOperation {
	...
	public static void LeaveCS(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		...
		try {
			...
			c.getSession().write(CField.getChannelChange(c,
	...
	public static void CouponCode(final String code, final MapleClient c) {
		...
		if (info != null && info.left) {
			if (!CouponCodeAttempt(c)) {
				 ...
				 * different things!
				 *
				 * Type 1: A-Cash, Type 2: Maple Points Type 3: Item.. use SN
				 * Type 4: Mesos
				 */
	...
	public static void BuyCashItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		...
		if (action == 0) {
			...
			int sn = slea.readInt();
			// final CashItem item =
			// CashItemFactory.getInstance().getMenuItem(sn);
			...
			// System.out.println("action 101");//might be farm mesos? RITE NOW
			// IS FREEH
			slea.skip(1);
			int type = slea.readInt();// type
			int sn = slea.readInt();
			...
			if (item != null && chr.getCSPoints(toCharge) >= item.getPrice()) {
				if (!item.genderEquals(c.getPlayer()
						.getGender())/* && c.getPlayer().getAndroid() == null */) {
					c.getSession().write(CSPacket.sendCSFail(0xA7));
					doCSPackets(c);
					return;
				} else if (item.getId() == 5211046 || item.getId() == 5211047 || item.getId() == 5211048
						|| item.getId() == 5050100 || item.getId() == 5051001) {
					c.getSession()
							.write(CWvsContext.broadcastMsg(1, "You cannot purchase this item through cash shop."));
				...
				if (itemz != null && itemz.getUniqueId() > 0 && itemz.getItemId() == item.getId()
					...
					chr.getCashInventory().addToInventory(itemz);
					// c.getSession().write(CSPacket.confirmToCSInventory(itemz,
			...
			if (item == null || c.getPlayer().getCSPoints(1) < item.getPrice() || msg.length() > 73
					|| msg.length() < 1) { // dont want packet editors gifting
											// random stuff =P
				c.getSession().write(CSPacket.sendCSFail(0));
				doCSPackets(c);
				return;
			...
			if (info == null || info.getLeft().intValue() <= 0 || info.getLeft().intValue() == c.getPlayer().getId()
				...
				c.getSession().write(
						CSPacket.sendGift(item.getPrice(), item.getId(), item.getCount(), partnerName, action == 34));
			}
			...
			if (item == null || c.getPlayer().getCSPoints(toCharge) < item.getPrice() || slots > 15
					|| item.getId() != 5430000) {
				c.getSession().write(CSPacket.sendCSFail(0));
				doCSPackets(c);
				return;
			...
			if (marr != null && marr.getCustomData() != null
				...
				c.getPlayer().getQuestNAdd(MapleQuest.getInstance(GameConstants.PENDANT_SLOT)).setCustomData(
						String.valueOf(System.currentTimeMillis() + ((long) item.getPeriod() * 24 * 60 * 60000)));
				c.getPlayer().modifyCSPoints(1, -item.getPrice(), false);
			...
			// Item item =
			// MapleItemInformationProvider.getInstance().getEquipById(item);
			...
			if (item != null && item.getQuantity() > 0 && item.getUniqueId() > 0
				...
				// warning: this d/cs
				// c.getSession().write(CSPacket.confirmToCSInventory(item,
				// c.getAccID(), itemz.getSN()));
			} else {
				c.getSession().write(CSPacket.sendCSFail(0xB1));
			}
			...
			if (item == null || !GameConstants.isEffectRing(item.getId())
					|| c.getPlayer().getCSPoints(toCharge) < item.getPrice() || msg.length() > 73 || msg.length() < 1) {
				c.getSession().write(CSPacket.sendCSFail(0));
				doCSPackets(c);
				return;
			...
			c.getSession()
					.write(CSPacket.showBoughtCSQuestItem(item.getPrice(), (short) item.getCount(), pos, item.getId()));
		} else if (action == 48) {
			...
			c.getSession().write(CSPacket.sendRandomBox(uniqueid,
					new Item(1302000, (short) 1, (short) 1, (short) 0, 10), (short) 0));
	...
	public static void SwitchCategory(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (Scategory == 103) {
			...
			try (PreparedStatement ps = DatabaseConnection.getConnection()
					.prepareStatement("INSERT INTO `wishlist` VALUES (?, ?)")) {
