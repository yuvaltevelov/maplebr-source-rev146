...
public class LoginInformationProvider {
	...
	public enum JobType {
		...
		UltimateAdventurer(-1, 0, 100000000, true, true, false, false, true, false, false, false), Resistance(0, 3000,
				931000000, true, true, false, false, true, false, false, false), // tutorial
																					// works
																					// 100%
				Adventurer(1, 0, 4000000, false, true, false, false, false, false, false, false), Cygnus(2, 1000,
						130030000, false, true, false, false, false, true, false, false), Aran(3, 2000, 914000000, true,
								true, false, false, true, false, false,
								false), Evan(4, 2001, 900010000, true, true, false, false, true, false, false, false), // evan
																														// starter
																														// map
																														// -
																														// need
																														...
																														// test
																														// tutorial
								Mercedes(5, 2002, 910150000, false, false, false, false, false, false, false, false), // 101050000
																														// -
																														// 910150000
								Demon(6, 3001, 931050310, false, false, true, false, false, false, false,
										false), Phantom(7, 2003, 915000000, false, true, false, false, false, true,
												false, false), DualBlade(8, 0, 103050900, false, true, false, false,
														false, false, false, false), // tutorial
																						// fixed
														Mihile(9, 5000, 913070000, true, true, false, false, true,
																false, false, false), Luminous(10, 2004, 927020000,
																		false, true, false, false, false, true, false,
																		false), // Ellinia
																				// atm
																				// TODO
																				// tutorial
																		Kaiser(11, 6000, 940001000, false, true, false,
																				false, false, false, false,
																				false), AngelicBuster(12, 6001,
																						940011000, false, true, false,
																						false, false, false, false,
																						false), // 400000000
																								// -
																								// 940011000
		...
		private JobType(int type, int id, int map, boolean hairColor, boolean skinColor, boolean faceMark, boolean hat,
				boolean bottom, boolean cape, boolean ears, boolean tail) {
			this.type = type;
			this.id = id;
			this.map = map;
	...
	protected LoginInformationProvider() {
		...
		/*
		 * for (MapleData dat : data) { int val = -1; if
		 * (dat.getName().endsWith("Female")) { val = 1; } else if
		 * (dat.getName().endsWith("Male")) { val = 0; } final int job =
		 * JobType.getByJob(dat.getName()).type; for (MapleData da : dat) {
		 * final Triple<Integer, Integer, Integer> key = new Triple<>(val,
		 * Integer.parseInt(da.getName()), job); List<Integer> our =
		 * makeCharInfo.get(key); if (our == null) { our = new ArrayList<>();
		 * makeCharInfo.put(key, our); } for (MapleData d : da) {
		...
		for (MapleData dat : infoData) {
			try {
				...
				for (MapleData d : dat) {
					int val;
					/*
					 * if (d.getName().contains("info")) { for (MapleData da :
					 * d) { if
					 * (da.getName().equalsIgnoreCase("choosableGender")) { int
					 * choosable = Integer.parseInt(da.getName()); if (choosable
					 * > 1) { if (choosable == 2) { //val = 0; //Forced male }
					 * else if (choosable == 3) { //val = 1; //Forced Female } }
					 * } } }
					 */
		...
		for (MapleData dat : uA) {
			final Triple<Integer, Integer, Integer> key = new Triple<>(-1, Integer.parseInt(dat.getName()),
					JobType.UltimateAdventurer.type);
			List<Integer> our = makeCharInfo.get(key);
			if (our == null) {
				our = new ArrayList<>();
				makeCharInfo.put(key, our);
