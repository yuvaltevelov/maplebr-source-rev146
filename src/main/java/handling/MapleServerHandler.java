...
public class MapleServerHandler extends IoHandlerAdapter implements MapleServerHandlerMBean {
	...
	public void writeLog(boolean crash) {
		...
		try {
			try (FileWriter fw = new FileWriter(
				...
				final String logString = "Log has been written at " + lastDC + " ["
						+ FileoutputUtil.CurrentReadable_Time() + "] - " + numDC + " have disconnected, within "
						+ (System.currentTimeMillis() - lastDC) + " milliseconds. (" + System.currentTimeMillis() + ")";
	...
	public static void initiate() {
		...
		RecvPacketOpcode[] block = new RecvPacketOpcode[] { RecvPacketOpcode.NPC_ACTION, RecvPacketOpcode.MOVE_PLAYER,
				RecvPacketOpcode.PONG, RecvPacketOpcode.MOVE_PET, RecvPacketOpcode.MOVE_SUMMON,
				RecvPacketOpcode.MOVE_DRAGON, RecvPacketOpcode.MOVE_LIFE, RecvPacketOpcode.MOVE_ANDROID,
				RecvPacketOpcode.HEAL_OVER_TIME, RecvPacketOpcode.STRANGE_DATA, RecvPacketOpcode.AUTO_AGGRO,
				RecvPacketOpcode.CANCEL_DEBUFF, RecvPacketOpcode.MOVE_FAMILIAR };
		RecvPacketOpcode[] serverBlock = new RecvPacketOpcode[] { RecvPacketOpcode.CHANGE_KEYMAP,
				...
				RecvPacketOpcode.MAGIC_ATTACK, RecvPacketOpcode.RANGED_ATTACK,
				/* RecvPacketOpcode.ARAN_COMBO, */RecvPacketOpcode.SPECIAL_MOVE, RecvPacketOpcode.GENERAL_CHAT,
				RecvPacketOpcode.MONSTER_BOMB, RecvPacketOpcode.PASSIVE_ENERGY, RecvPacketOpcode.PET_AUTO_POT,
				RecvPacketOpcode.USE_CASH_ITEM, RecvPacketOpcode.PARTYCHAT, RecvPacketOpcode.CANCEL_BUFF,
				RecvPacketOpcode.SKILL_EFFECT, RecvPacketOpcode.CHAR_INFO_REQUEST, RecvPacketOpcode.ALLIANCE_OPERATION,
	...
	public void sessionOpened(final IoSession session) throws Exception {
		...
		final MapleClient client = new MapleClient(
				new MapleAESOFB(ServerConfig.USE_FIXED_IV ? ServerConfig.Static_LocalIV : ivSend,
						(short) (0xFFFF - ServerConstants.MAPLE_VERSION)), // Sent
																			// Cypher
				new MapleAESOFB(ServerConfig.USE_FIXED_IV ? ServerConfig.Static_RemoteIV : ivRecv,
						ServerConstants.MAPLE_VERSION), // Recv Cypher
				session);
		client.setChannel(-1);
	...
	public void sessionClosed(final IoSession session) throws Exception {
		...
		if (client != null) {
			...
			if (state != MapleClient.CHANGE_CHANNEL) {
				...
				if (System.currentTimeMillis() - lastDC < 60000) { // within the
																	// minute
					numDC++;
					if (numDC > 100) { // 100+ people have dc'd in minute in
										// channelserver
						System.out.println("Writing log...");
						writeLog();
						numDC = 0;
						lastDC = System.currentTimeMillis(); // intentionally
																// place here
					}
				} else {
					numDC = 0;
					lastDC = System.currentTimeMillis(); // intentionally place
															// here
				}
			}
			try {
			...
			if (recv.getValue() == header_num) {
				...
				try {
					if (c.getPlayer() != null && c.isMonitored() && !blocked.contains(recv)) {
						try (FileWriter fw = new FileWriter(
								new File("MonitorLogs/" + c.getPlayer().getName() + "_log.txt"), true)) {
	...
	public static void handlePacket(final RecvPacketOpcode header, final LittleEndianAccessor slea, final MapleClient c)
			throws Exception {
		if (ServerConfig.logPackets && !isSpamHeader(header)) {
			...
			System.out.println("[Recv]\t" + header.name() + tab + "|\t" + header.getValue() + "\t|\t"
					+ HexTool.getOpcodeToString(header.getValue()));
			FileoutputUtil.log("PacketLog.txt", "\r\n\r\n[Recv]\t" + header.name() + tab + "|\t" + header.getValue()
					+ "\t|\t" + HexTool.getOpcodeToString(header.getValue()) + "\r\n\r\n");
		}
		switch (header) {
		/*
		 * case LOGIN_REDIRECTOR: System.out.println("Redirector login received"
		 * ); if (!ServerConstants.Redirector) { System.out.println(
		 * "Redirector login packet recieved, but server is not set to redirector. Please change it in ServerConstants!"
		 * ); } else { CharLoginHandler.redirectorLogin(slea, c); }
		 */
		...
		// int response = 6; // Couldn't process the request - Will never end as
		// 6, but precautionary.
		...
		case AUTH_REQUEST:
			...
			break;
		case CLIENT_START:
			System.out.println("CLIENT_START" + c.getSessionIPAddress());
			String adminLogin = System.getProperty("DEBUG_LOGIN");
			if(adminLogin != null){
				c.loginData(adminLogin);
				c.getSession().write(LoginPacket.getAuthSuccessRequest(c));
			}
			break;
		case CLIENT_FAILED:
		...
		case CLIENT_ERROR:
			...
			// if (errortype == 0) { // i don't wanna log error code 0 stuffs,
			// (usually some bounceback to login)
			// return;
			// }
			short data_length = slea.readShort();
			slea.skip(4); // ?B3 86 01 00 00 00 FF 00 00 00 00 00 9E 05 C8 FF 02
							// 00 CD 05 C9 FF 7D 00 00 00 3F 00 00 00 00 00 02
							// 77 01 00 25 06 C9 FF 7D 00 00 00 40 00 00 00 00
							// 00 02 C1 02
			short opcodeheader = slea.readShort();
			// System.out.println("[CLIENT ERROR] Error Code " + errortype + "
			// Type " + type_str + "\r\n\t[Data] Length " + data_length + " [" +
			// SendPacketOpcode.getOpcodeName(opcodeheader) + " | " +
			// opcodeheader + "]\r\n" + HexTool.toString(slea.read((int)
			// slea.available())));
			FileoutputUtil.log("ErrorCodes.txt",
					"Error Type: " + errortype + "\r\n" + "Data Length: " + data_length + "\r\n" + "Error for player ; "
							+ c.getPlayer().getName() + " - account ; " + c.getAccountName() + "\r\n"
		...
		case TWIN_DRAGON_EGG:
			...
			// Aristocat c.getSession().write(CSPacket.sendTwinDragonEgg(true,
			// true, 38, itemz, 1));
			break;
		case XMAS_SURPRISE:
