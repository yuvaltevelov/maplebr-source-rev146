...
public class FamilyHandler {
	...
	public static final void UseFamily(final LittleEndianAccessor slea, MapleClient c) {
		...
		case Teleport: // teleport: need add check for if not a safe place
			...
			if (FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit())
				...
				c.getPlayer().dropMessage(5,
						"Summons failed. Your current location or state does not allow a summons.");
				success = false;
				...
				success = false;
					...
					&& victim.getId() != c.getPlayer().getId() && !victim.isInBlockedMap()) {
			...
			break;
		case Summon: // TODO give a check to the player being forced somewhere
						// else..
			victim = c.getChannelServer().getPlayerStorage().getCharacterByName(slea.readMapleAsciiString());
			if (FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit())
					|| c.getPlayer().isInBlockedMap()) {
				...
				c.getPlayer().dropMessage(1,
						"Another character has requested to summon this character. Please try again later.");
			} else if (victim.getFamilyId() == c.getPlayer().getFamilyId()
					&& !FieldLimitType.VipRock.check(victim.getMap().getFieldLimit())
					&& victim.getId() != c.getPlayer().getId() && !victim.isInBlockedMap()) {
				victim.getClient().getSession().write(
						FamilyPacket.familySummonRequest(c.getPlayer().getName(), c.getPlayer().getMap().getMapName()));
		...
		case Drop_15_15:
			// case Drop_15_30:
			// c.getSession().write(FamilyPacket.familyBuff(entry.type, type,
			// entry.effect, entry.duration*60000));
			entry.applyTo(c.getPlayer());
			break;
		case Bonding: // 6 family members in pedigree online Drop Rate & Exp
						// Rate + 100% 30 minutes
			...
			if (chrs.size() < 7) {
				...
				for (MapleFamilyCharacter chrz : chrs) {
					...
					entry.applyTo(chrr);
					// chrr.getClient().getSession().write(FamilyPacket.familyBuff(entry.type,
					// type, entry.effect, entry.duration*60000));
				}
			}
			break;
		/*
		 * case EXP_Party: case Drop_Party_12: // drop rate + 100% party 30 min
		 * case Drop_Party_15: // exp rate + 100% party 30 min
		 * entry.applyTo(c.getPlayer());
		 * //c.getSession().write(FamilyPacket.familyBuff(entry.type, type,
		 * entry.effect, entry.duration*60000)); if (c.getPlayer().getParty() !=
		 * null ) { for (MaplePartyCharacter mpc :
		 * c.getPlayer().getParty().getMembers()) { if (mpc.getId() !=
		 * c.getPlayer().getId()) { MapleCharacter chr =
		 * c.getPlayer().getMap().getCharacterById(mpc.getId()); if (chr !=
		 * null) { entry.applyTo(chr);
		 * //chr.getClient().getSession().write(FamilyPacket.familyBuff(entry.
		 * type, type, entry.effect, entry.duration*60000)); } } } } break;
		 */
		}
		if (success) { // again
	...
	public static final void FamilyOperation(final LittleEndianAccessor slea, MapleClient c) {
		...
		if (addChr == null) {
			...
			// c.getPlayer().getFamily().getGens() >= 1000) {
			// c.getPlayer().dropMessage(5, "Your family cannot extend more than
			// 1000 generations from above and below.");
	...
	public static final void FamilySummon(final LittleEndianAccessor slea, MapleClient c) {
		...
		if (c.getPlayer().getFamilyId() > 0 && tt != null && tt.getFamilyId() == c.getPlayer().getFamilyId()
				...
				&& !c.getPlayer().isInBlockedMap() && !tt.isInBlockedMap()) {
			// whew lots of checks
	...
	public static final void AcceptFamily(LittleEndianAccessor slea, MapleClient c) {
		...
		if (inviter != null && c.getPlayer().getSeniorId() == 0 && (c.getPlayer().isGM() || !inviter.isHidden())
				...
				&& inviter.getName().equals(slea.readMapleAsciiString()) && inviter
						.getNoJuniors() < 2 /*
											 * && inviter.getFamily().getGens()
											 * < 1000
											 */
			...
			if (accepted) {
				...
				if (inviter.getFamilyId() > 0 && World.Family.getFamily(inviter.getFamilyId()) != null) {
					...
					// if old isn't null, don't set the familyid yet,
					// mergeFamily will take care of it
					...
					if (old > 0 && World.Family.getFamily(old) != null) { // has
																			// junior
					...
					if (fam != null) {
						if (inviter.getNoJuniors() == 1 || old > 0) {// just got
																		// their
																		// first
																		// junior
																		// whoopee
					...
					if (id > 0) {
						...
						MapleFamily.setOfflineFamilyStatus(id, inviter.getId(), oldj1 <= 0 ? 0 : oldj1,
								...
								c.getPlayer().getId());
						inviter.setFamily(id, 0, c.getPlayer().getId(), 0); // load
																			// the
																			// family
