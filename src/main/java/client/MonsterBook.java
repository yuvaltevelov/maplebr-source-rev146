...
public final class MonsterBook implements Serializable {
	...
	public MonsterBook(Map<Integer, Integer> cards, MapleCharacter chr) {
		...
		if ((stat != null) && (stat.getCustomData() != null)) {
			...
			if ((!this.sets.containsKey(Integer.valueOf(this.currentSet)))
					|| (!((Boolean) ((Pair) this.sets.get(Integer.valueOf(this.currentSet))).right).booleanValue())) {
				this.currentSet = -1;
			}
		}
		applyBook(chr, true);
	...
	public void writeCharInfoPacket(MaplePacketLittleEndianWriter mplew) {
		...
		for (Iterator i$ = this.cardItems.iterator(); i$.hasNext();) {
			...
			cardSize.set(x / 1000 % 10 + 1,
					Integer.valueOf(((Integer) cardSize.get(x / 1000 % 10 + 1)).intValue() + 1));
		}
	...
	public boolean changeSet(int c) {
		if ((this.sets.containsKey(Integer.valueOf(c)))
				&& (((Boolean) ((Pair) this.sets.get(Integer.valueOf(c))).right).booleanValue())) {
			this.currentSet = c;
			return true;
		}
		return false;
	...
	public final int getLevelByCard(int cardid) {
		return this.cards.get(Integer.valueOf(cardid)) == null ? 0
				: (this.cards.get(Integer.valueOf(cardid))).intValue();
	}
	...
	public final boolean monsterCaught(MapleClient c, int cardid, String cardname) {
		if ((!this.cards.containsKey(Integer.valueOf(cardid)))
				|| ((this.cards.get(Integer.valueOf(cardid))).intValue() < 2)) {
			this.changed = true;
			c.getPlayer().dropMessage(-6,
					new StringBuilder().append("Book entry updated - ").append(cardname).toString());
