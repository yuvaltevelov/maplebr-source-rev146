...
public class PlayerInteractionHandler {
	...
	public enum Interaction {
		...
		SET_ITEMS(0), SET_ITEMS2(1), SET_ITEMS3(2), SET_ITEMS4(3), UPDATE_MESO(4), SET_MESO2(5), SET_MESO3(
				6), SET_MESO4(7), CONFIRM_TRADE(8), CONFIRM_TRADE2(9), CONFIRM_TRADE_MESO(10), CONFIRM_TRADE_MESO2(
						11), CREATE(16), VISIT(19), INVITE_TRADE(21), DENY_TRADE(22), CHAT(24), EXIT(28), OPEN1(
								25), OPEN2(26), OPEN3(80), ADD_ITEM1(33), ADD_ITEM2(34), ADD_ITEM3(35), ADD_ITEM4(
										36), BUY_ITEM_HIREDMERCHANT(37), BUY_ITEM_STORE1(38), BUY_ITEM_STORE2(
												39), BUY_ITEM_STORE3(40), MAINTANCE_OFF(50), // 41
												REMOVE_ITEM(49), // +10
		...
		START_ROCK_PAPER_SCISSORS1(96), START_ROCK_PAPER_SCISSORS2(97), START_ROCK_PAPER_SCISSORS3(
				98), INVITE_ROCK_PAPER_SCISSORS(112), FINISH_ROCK_PAPER_SCISSORS(113),
				//
				// REQUEST_TIE(0x30),
				// ANSWER_TIE(0x31),
		GIVE_UP(114),
	...
	public static final void PlayerInteraction(final LittleEndianAccessor slea, final MapleClient c,
		...
		case CREATE: {
			...
			if (createType == 4) { // trade//was 3
				...
				// if (createType == 4 && !chr.isIntern()) { //not hired
				// merch... blocked playershop
				// c.getSession().write(CWvsContext.enableActions());
				// return;
				// }
				if (!chr.getMap()
				...
				if (createType == 1 || createType == 2) {
					...
					if (createType == 3) {
						// MaplePlayerShop mps = new MaplePlayerShop(chr,
						// shop.getItemId(), desc);
						// chr.setPlayerShop(mps);
						// chr.getMap().addMapObject(mps);
						// c.getSession().write(PlayerShopPacket.getPlayerStore(chr,
						// true));
			...
			if (chr.getTrade() != null && chr.getTrade().getPartner() != null && !chr.getTrade().inTrade()) {
				...
				if (ob instanceof IMaplePlayerShop && chr.getPlayerShop() == null) {
					...
					if (ob instanceof HiredMerchant) {
						...
						/*
						 * if (merchant.isOwner(chr) && merchant.isOpen() &&
						 * merchant.isAvailable()) { merchant.setOpen(false);
						 * merchant.removeAllVisitors((byte) 16, (byte) 0);
						 * chr.setPlayerShop(ips);
						 * c.getSession().write(PlayerShopPacket.getHiredMerch(
						 * chr, merchant, false)); } else {
						 */
		...
		case CHAT: {
			...
			if (chr.getTrade() != null) {
				...
				if (chr.getClient().isMonitored()) { // Broadcast info even if
														// it was a command.
		...
		case EXIT: {
			if (chr.getTrade() != null) {
				...
				final IMaplePlayerShop ips = chr.getPlayerShop();
				if (ips == null) { // should be null anyway for owners of hired
									// merchants (maintenance_off)
					return;
				}
				if (ips.isOwner(chr) && ips.getShopType() != 1) {
					ips.closeShop(false, ips.isAvailable()); // how to return
																// the items?
				} else {
			...
			if (ivItem != null) {
				...
				if (ivItem.getQuantity() >= bundles_perbundle) {
					...
					if (GameConstants.getLowestPrice(ivItem.getItemId()) > price) {
						c.getPlayer().dropMessage(1, "The lowest you can sell this for is "
								+ GameConstants.getLowestPrice(ivItem.getItemId()));
						c.getSession().write(CWvsContext.enableActions());
						return;
					}
		...
		case BUY_ITEM_HIREDMERCHANT: { // Buy and Merchant buy
			...
			if (check <= 0 || check2 > 9999999999L || check2 <= 0 || check3 > 32767 || check3 < 0) { // This
																										// is
																										// the
																										// better
																										// way
			...
			if (tobuy.bundles < quantity
					...
					|| chr.getMeso() - (check2) < 0 || chr.getMeso() - (check2) > 9999999999L
					|| shop.getMeso() + (check2) < 0 || shop.getMeso() + (check2) > 9999999999L) {
				return;
			}
		...
		case START: {
			...
			// if (slea.readByte() > 0) {
			// game.broadcastToVisitors(PlayerShopPacket.getMiniGameResult(game,
			// 1, game.getRequestedTie()));
			// game.nextLoser();
			// game.setOpen(true);
			// game.update();
			// game.checkExitAfterGame();
			...
			// ips.broadcastToVisitors(PlayerShopPacket.shopChat("Turn could not
			// be skipped by " + chr.getName() + ". Loser: " + game.getLoser() +
			// " Visitor: " + ips.getVisitorSlot(chr),
			...
			// game.broadcastToVisitors(PlayerShopPacket.shopChat("Omok could
			// not be placed by " + chr.getName() + ". Loser: " +
			// game.getLoser() + " Visitor: " + game.getVisitorSlot(chr),
			// game.getVisitorSlot(chr)));
			// return;
			// }
			// game.setPiece(slea.readInt(), slea.readInt(), slea.readByte(),
			// chr);
			// }
			// break;
			// }
			...
			// }
			// if (game.getLoser() != game.getVisitorSlot(chr)) {
			// game.broadcastToVisitors(PlayerShopPacket.shopChat("Card could
			// not be placed by " + chr.getName() + ". Loser: " +
			// game.getLoser() + " Visitor: " + game.getVisitorSlot(chr),
			// game.getVisitorSlot(chr)));
			// return;
			...
			// not be placed by " + chr.getName() + ". Loser: " +
			// game.getLoser() + " Visitor: " + game.getVisitorSlot(chr) + "
			// Turn: " + game.getTurn(), game.getVisitorSlot(chr)));
			// return;
			// }
			...
			// if (game.isOwner(chr)) {
			// game.broadcastToVisitors(PlayerShopPacket.getMatchCardSelect(turn,
			// slot, fs, turn), false);
			// } else {
			// game.getMCOwner().getClient().getSession().write(PlayerShopPacket.getMatchCardSelect(turn,
			// slot, fs, turn));
			// }
			// game.setTurn(0); //2nd turn nao
			// return;
			// } else if (fs > 0 && game.getCardId(fs + 1) ==
			// game.getCardId(slot + 1)) {
			// game.broadcastToVisitors(PlayerShopPacket.getMatchCardSelect(turn,
			// slot, fs, game.isOwner(chr) ? 2 : 3));
			// game.setPoints(game.getVisitorSlot(chr)); //correct.. so still
			// same loser. diff turn tho
			// } else {
			// game.broadcastToVisitors(PlayerShopPacket.getMatchCardSelect(turn,
			// slot, fs, game.isOwner(chr) ? 0 : 1));
