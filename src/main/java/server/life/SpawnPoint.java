...
public class SpawnPoint extends Spawns {
	...
	public final boolean shouldSpawn(long time) {
		...
		// maximum of 1
		if (((mobTime != 0 || !monster.getMobile()) && spawnedMonsters.get() > 0)
				|| spawnedMonsters.get() > 1 && !MoonlightRevamp.MoonlightRevamp
				|| spawnedMonsters.get() > MoonlightRevamp.monsterSpawn && MoonlightRevamp.MoonlightRevamp) {
			return false;
		}
	...
	public final MapleMonster spawnMonster(final MapleMap map) {
		...
		if (carnivalTeam > -1) {
			for (MapleReactor r : map.getAllReactorsThreadsafe()) { // parsing
																	// through
																	// everytime
																	// a monster
																	// is
				...
				if (r.getName().startsWith(String.valueOf(carnivalTeam)) && r.getReactorId() == (9980000 + carnivalTeam)
						&& r.getState() < 5) {
					final int num = Integer.parseInt(r.getName().substring(1, 2)); // 00,
																					// 01,
																					// etc
			...
			if (s.getSkill() == 35111005) {
				...
				for (Map.Entry<MonsterStatus, Integer> stat : effect.getMonsterStati().entrySet()) {
					mob.applyStatus(s.getOwner(),
							new MonsterStatusEffect(stat.getKey(), stat.getValue(), s.getSkill(), null, false), false,
