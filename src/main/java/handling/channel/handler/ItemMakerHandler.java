...
public class ItemMakerHandler {
	...
	public static enum CraftRanking {
		...
		SOSO(19, 30), GOOD(20, 40), COOL(21, 50);
		public int i, craft;
		...
		case 1: { // Gem
			...
			if (GameConstants.isGem(toCreate)) {
				...
				c.getPlayer().getMap().broadcastMessage(c.getPlayer(),
						EffectPacket.ItemMaker_Success_3rdParty(c.getPlayer().getId()), false);
	...
	private static boolean hasSkill(final MapleClient c, final int reqlvl) {
		return c.getPlayer().getSkillLevel(
				SkillFactory.getSkill(PlayerStats.getSkillByJob(1007, c.getPlayer().getJob()))) >= reqlvl;
	}
	...
	public static final void MakeExtractor(final LittleEndianAccessor slea, final MapleClient c,
		...
		chr.setExtractor(new MapleExtractor(chr, itemId, fee, chr.getFH())); // no
																				// clue
																				// about
																				// time
																				// left
		...
		// expiry date ..
		// MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.SETUP,
		// toUse.getPosition(), (short) 1, false);
	}
	...
	public static final void StartHarvest(final LittleEndianAccessor slea, final MapleClient c,
			final MapleCharacter chr) {
		// its ok if a hacker bypasses this as we do everything in the reactor
		// anyway
		final MapleReactor reactor = c.getPlayer().getMap().getReactorByOid(slea.readInt());
		if (reactor == null || !reactor.isAlive() || reactor.getReactorId() > 200011
				|| chr.getStat().harvestingTool <= 0
				|| reactor.getTruePosition().distanceSq(chr.getTruePosition()) > 10000
				|| c.getPlayer().getFatigue() >= 200) {
			return;
		}
		Item item = c.getPlayer().getInventory(MapleInventoryType.EQUIP)
				.getItem((short) c.getPlayer().getStat().harvestingTool);
		...
		if (marr.getCustomData() == null) {
			...
			c.getSession().write(CField.harvestMessage(reactor.getObjectId(), 13)); // ok
																					// to
																					// harvest,
																					// gogo
	...
	public static final void StopHarvest(final LittleEndianAccessor slea, final MapleClient c,
			final MapleCharacter chr) {
		// its ok if a hacker bypasses this as we do everything in the reactor
		// anyway
		/*
		 ...
		 * c.getPlayer().getStats().harvestingTool); if (item == null ||
		 * ((Equip) item).getDurability() == 0) {
		 * c.getPlayer().getStats().handleProfessionTool(c.getPlayer()); return;
		 * } c.getPlayer().getMap().destroyReactor(reactor.getObjectId());
		 * ReactorScriptManager.getInstance().act(c, reactor);
		 */
	}

	public static void ProfessionInfo(final LittleEndianAccessor slea, final MapleClient c) { // so
																								// pointless
		try {
			...
			if (skillid.startsWith("9200") || skillid.startsWith("9201")) {
				...
				c.getSession().write(
						CWvsContext.updateSpecialStat(skillid, level1, level2, c.getPlayer().getHonourNextExp()));
				return;
				...
				rate = Math.max(0,
						100 - ((level1 + 1) - c.getPlayer().getProfessionLevel(Integer.parseInt(skillid))) * 20);
			}
	...
	public static final void CraftEffect(final LittleEndianAccessor slea, final MapleClient c,
			final MapleCharacter chr) {
		...
		if (profession != null && (c.getPlayer().getProfessionLevel(profession.intValue()) > 0
				|| (profession == 92040000 && chr.getMap().getExtractorSize() > 0))) {
			int time = slea.readInt();
			if (time > 6000 || time < 3000) {
				time = 4000;
			}
			c.getSession().write(EffectPacket.showOwnCraftingEffect(effect, (byte) chr.getDirection(), time,
					effect.endsWith("Extract") ? 1 : 0));
			chr.getMap().broadcastMessage(chr, EffectPacket.showCraftingEffect(chr.getId(), effect,
					(byte) chr.getDirection(), time, effect.endsWith("Extract") ? 1 : 0), false);
		}
	}
	...
	public static final void CraftMake(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		...
		final int something = slea.readInt(); // no clue what it is, but its
												// between 288 and 305..
	...
	public static final void CraftComplete(final LittleEndianAccessor slea, final MapleClient c,
			final MapleCharacter chr) {
		...
		if (craftID == 92049000) { // disassembling
			...
			toGet = 4031016;
			quantity = (short) Randomizer.rand(3,
			...
			fatigue = 3;
			MapleInventoryManipulator.addById(c, toGet, quantity,
					"Made by disassemble " + itemId + " on " + FileoutputUtil.CurrentReadable_Date());
			...
			for (Entry<Integer, Integer> e : ce.reqItems.entrySet()) {
				MapleInventoryManipulator.removeById(c, GameConstants.getInventoryType(e.getKey()), e.getKey(),
						e.getValue(), false, false);
			}
					...
					for (Triple<Integer, Integer, Integer> i : ce.targetItems) {
						if (Randomizer.nextInt(100) < i.right) {
							...
							if (GameConstants.getInventoryType(toGet) == MapleInventoryType.EQUIP) {
								...
								receive = new Item(toGet, (short) 0, quantity,
										(short) (ItemFlag.CRAFTED_USE.getValue()));
							}
							if (ce.period > 0) {
								receive.setExpiration(System.currentTimeMillis() + (ce.period * 60000)); // period
																											// is
																											// in
																											// minutes
							}
							...
							receive.setGMLog(
									"Crafted from " + craftID + " on " + FileoutputUtil.CurrentReadable_Date());
							...
							if (ce.needOpenItem) {
								...
								if (mLevel == 1) {
									...
									sa.put(ce,
											new SkillEntry(Integer.MAX_VALUE, (byte) (chr.getMasterLevel(craftID) - 1),
													SkillFactory.getDefaultSExpiry(ce)));
								}
							}
							fatigue = ce.incFatigability;
							expGain = ce.incSkillProficiency == 0
									? (((fatigue * 20) - (ce.reqSkillLevel - theLevl) * 2) * 2)
									: ce.incSkillProficiency;
		...
		MapleQuest.getInstance(2550).forceStart(c.getPlayer(), 9031000, "1"); // removes
																				// tutorial
																				// stuff
	...
	public static final void UsePot(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (slot == null || slot.getQuantity() <= 0 || slot.getItemId() != itemid || itemid / 10000 != 244
		...
		for (int i = 0; i < c.getPlayer().getImps().length; i++) {
			if (c.getPlayer().getImps()[i] == null) {
				...
				c.getSession().write(
						CWvsContext.updateImp(c.getPlayer().getImps()[i], ImpFlag.SUMMONED.getValue(), i, false));
				MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot.getPosition(), (short) 1,
						false, false);
				return;
			}
		}
	...
	public static final void ClearPot(final LittleEndianAccessor slea, final MapleClient c) {
		...
		c.getSession()
				.write(CWvsContext.updateImp(c.getPlayer().getImps()[index], ImpFlag.REMOVED.getValue(), index, false));
		c.getPlayer().getImps()[index] = null;
	}

	public static final void FeedPot(final LittleEndianAccessor slea, final MapleClient c) {
		final int itemid = slea.readInt();
		final Item slot = c.getPlayer().getInventory(GameConstants.getInventoryType(itemid))
				.getItem((short) slea.readInt());
		if (slot == null || slot.getQuantity() <= 0 || slot.getItemId() != itemid) {
			c.getSession().write(CWvsContext.enableActions());
			return;
		}
		final int level = GameConstants.getInventoryType(itemid) == MapleInventoryType.ETC
		...
		if (level <= 0 || level < (Math.min(120, c.getPlayer().getLevel()) - 50)
		...
		final int index = slea.readInt() - 1;
		if (index < 0 || index >= c.getPlayer().getImps().length || c.getPlayer().getImps()[index] == null
				|| c.getPlayer().getImps()[index].getLevel() >= (MapleItemInformationProvider.getInstance()
						.getPot(c.getPlayer().getImps()[index].getItemId()).right - 1)
		...
		c.getPlayer().getImps()[index].setFullness(c.getPlayer().getImps()[index].getFullness()
				+ (100 * (GameConstants.getInventoryType(itemid) == MapleInventoryType.EQUIP ? 2 : 1)));
		if (Randomizer.nextBoolean()) {
			mask |= ImpFlag.CLOSENESS.getValue();
			c.getPlayer().getImps()[index].setCloseness(c.getPlayer().getImps()[index].getCloseness() + 1 + (Randomizer
					.nextInt(5 * (GameConstants.getInventoryType(itemid) == MapleInventoryType.EQUIP ? 2 : 1))));
		...
		if (c.getPlayer().getImps()[index].getFullness() >= 1000) {
			...
			if (c.getPlayer().getImps()[index].getLevel() >= (MapleItemInformationProvider.getInstance()
					.getPot(c.getPlayer().getImps()[index].getItemId()).right - 1)) {
				c.getPlayer().getImps()[index].setState(5);
			}
		}
		MapleInventoryManipulator.removeFromSlot(c, GameConstants.getInventoryType(itemid), slot.getPosition(),
				(short) 1, false, false);
	...
	public static final void CurePot(final LittleEndianAccessor slea, final MapleClient c) {
		...
		c.getPlayer().getImps()[index].setState(1);
		c.getSession()
				.write(CWvsContext.updateImp(c.getPlayer().getImps()[index], ImpFlag.STATE.getValue(), index, false));
		MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.ETC, slot.getPosition(), (short) 1, false,
				false);
	}
	...
	public static final void RewardPot(final LittleEndianAccessor slea, final MapleClient c) {
		...
		final int itemid = GameConstants.getRewardPot(c.getPlayer().getImps()[index].getItemId(),
		...
		MapleInventoryManipulator.addById(c, itemid, (short) 1, "Item Pot from "
				+ c.getPlayer().getImps()[index].getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
		c.getSession()
				.write(CWvsContext.updateImp(c.getPlayer().getImps()[index], ImpFlag.REMOVED.getValue(), index, false));
		c.getPlayer().getImps()[index] = null;
	}
