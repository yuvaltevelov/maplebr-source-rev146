...
public class CashShop implements Serializable {
	...
	public Item toItem(CashItemInfo cItem, int uniqueid, String gift) {
		...
		if (cItem.getId() >= 5000100 && cItem.getId() < 5000200) { // permanent
																	// pet
	...
	public Item toItem(CashItem cItem, int uniqueid, String gift) {
		...
		if ((period <= 0 && GameConstants.getInventoryType(cItem.getItemId()) != MapleInventoryType.EQUIP)
				|| GameConstants.isPet(cItem.getItemId())) {
			period = GameConstants.GMS ? 90 : 45;
		}
		if (cItem.getItemId() >= 5000100 && cItem.getItemId() < 5000200) { // permanent
																			// pet
			period = 20000; // permanent time millis
		}
		Item ret;
