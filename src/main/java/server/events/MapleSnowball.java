...
public class MapleSnowball extends MapleEvent {
	...
	public static class MapleSnowballs {
		...
		public int getRightX() {
			return getLeftX() + 275; // exact pos where you cant hit it, as it
										// should knockback
		}
		...
		public static void hitSnowball(final MapleCharacter chr) {
			...
			final MapleSnowball sb = ((MapleSnowball) chr.getClient().getChannelServer()
					.getEvent(MapleEventType.Snowball));
			...
			if (ball != null && !ball.isInvis()) {
				...
				if (!snowman) {
					int damage = (Math.random() < 0.01 || (chr.getTruePosition().x > ball.getLeftX()
							&& chr.getTruePosition().x < ball.getRightX())) && ball.isHittable() ? 10 : 0;
					...
					if (damage == 0) {
						...
						// System.out.println("pos: " + chr.getPosition().x + ",
						// ballpos: " + ball.getPosition().x + ", hittable: " +
						// ball.isHittable() + ", startPoints: " +
						// startPoints[0] + "," + startPoints[1] + ", damage: "
						// + damage + ", snowmens: " + snowmens[0] + "," +
						// snowmens[1] + ", extraDistances: " +
						// extraDistances[0] + "," + extraDistances[1] + ", HP:
						// " + ball.getHP());
						if (ball.getPosition() == 255 || ball.getPosition() == 511 || ball.getPosition() == 767) { // Going
																													// to
																													// stage
							ball.setStartPoint(chr.getMap());
							chr.getMap().broadcastMessage(CField.rollSnowball(4, sb.getSnowBall(0), sb.getSnowBall(1)));
						} else if (ball.getPosition() == 899) { // Crossing the
																// finishing
																// line
							...
							chr.getMap().broadcastMessage(CWvsContext.broadcastMsg(6, "Congratulations! Team "
									+ (team == 0 ? "Story" : "Maple") + " has won the Snowball Event!"));

							for (MapleCharacter chrz : chr.getMap().getCharactersThreadsafe()) {
								if ((team == 0 && chrz.getTruePosition().y > -80)
										|| (team == 1 && chrz.getTruePosition().y <= -80)) { // winner
					...
					chr.getMap().broadcastMessage(CField.hitSnowBall(team + 2, damage, 0, 0)); // Hitting
																								// the
																								// snowman
					ball.setSnowmanHP(ball.getSnowmanHP() - damage);
					if (damage > 0) {
						chr.getMap().broadcastMessage(CField.rollSnowball(0, sb.getSnowBall(0), sb.getSnowBall(1))); // not
																														// sure
						if (ball.getSnowmanHP() <= 0) {
							...
							for (MapleCharacter chrz : chr.getMap().getCharactersThreadsafe()) {
								if ((ball.getTeam() == 0 && chr.getTruePosition().y < -80)
										|| (ball.getTeam() == 1 && chr.getTruePosition().y > -80)) {
									chrz.giveDebuff(MapleDisease.SEDUCE, MobSkillFactory.getMobSkill(128, 1)); // go
																												// left
								}
							}
						}
