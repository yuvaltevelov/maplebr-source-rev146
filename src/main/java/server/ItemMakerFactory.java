...
public class ItemMakerFactory {
		...
		for (MapleData dataType : info.getChildren()) {
			...
			case 0: { // Caching of gem
				for (MapleData itemFolder : dataType.getChildren()) {
					...
					// totalupgrades = MapleDataTool.getInt("tuc", itemFolder,
					// 0); // Gem is always 0
	...
	public static class ItemMakerCreateEntry {
		...
		private final List<Pair<Integer, Integer>> reqItems = new ArrayList<>(); // itemId
																					// /
																					// amount
		private final List<Integer> reqEquips = new ArrayList<>();

		public ItemMakerCreateEntry(int cost, int reqLevel, byte reqMakerLevel, int quantity, byte tuc,
				int stimulator) {
			this.cost = cost;
			this.tuc = tuc;
			this.reqLevel = reqLevel;
