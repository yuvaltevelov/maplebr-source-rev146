...
public class CustomPlayerRankings {
	...
	public void reload() {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				final CustomRankingInfo rankfield = new CustomRankingInfo(rank, rs.getString("name"),
						rs.getInt("level"));
				ranks.add(rankfield);
				rank++;
			}
	...
	/*
	 * public static byte[] customRanks(int npcid,
	 * List<CustomPlayerRankings.CustomRankingInfo> all) {
	 * MaplePacketLittleEndianWriter mplew = new
	 * MaplePacketLittleEndianWriter();
	 * 
	 * mplew.writeShort(SendPacketOpcode.GUILD_OPERATION.getValue());
	 * mplew.write(80); mplew.writeInt(npcid); mplew.writeInt(all.size()); for
	 * (CustomPlayerRankings.CustomRankingInfo info : all) {
	 * mplew.writeShort(info.getRank());
	 * mplew.writeMapleAsciiString(info.getName());
	 * mplew.writeInt(info.getLevel()); mplew.writeInt(0); mplew.writeInt(0);
