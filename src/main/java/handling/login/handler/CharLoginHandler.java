...
public class CharLoginHandler {
	...
	public static void login(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (AutoRegister.autoRegister && !AutoRegister.getAccountExists(login)
			...
			if (pwd.equalsIgnoreCase("disconnect") || pwd.equalsIgnoreCase("fixme")) {
				...
				c.getSession().write(LoginPacket.getLoginFailed(1)); // Shows no
																		// message,
																		// used
																		// for
																		// unstuck
		...
		if (!c.isGm() && !c.isLocalhost() && ServerConstants.Use_Localhost) {
			c.getSession().write(CWvsContext.broadcastMsg(1,
					"We are sorry, but the server is under a maintenance, please check the forums for more information."));
			c.getSession().write(LoginPacket.getLoginFailed(1)); // Shows no
																	// message,
																	// used for
																	// unstuck
		...
		if (loginok == 0 && (ipBan || macBan) && !c.isGm()) {
			loginok = 3;
			if (macBan) {
				// this is only an ipban o.O" - maybe we should refactor this a
				// bit so it's more readable
		...
		if (loginok != 0) {
			if (!loginFailCount(c)) {
				...
				if (loginok == 3) {
					...
					c.getSession().write(LoginPacket.getLoginFailed(1)); // Shows
																			// no
																			// message,
																			// used
																			// for
			...
			if (!loginFailCount(c)) {
				c.clearInformation();
				c.getSession().write(LoginPacket.getTempBan(PacketHelper.getTime(tempbannedTill.getTimeInMillis()),
						c.getBanReason()));
			} else {
				c.getSession().close();
			}
	...
	public static void ServerListRequest(final MapleClient c) {
		if (ServerConstants.TESPIA) {
			for (TespiaWorldOption tespiaservers : TespiaWorldOption.values()) {
				if (TespiaWorldOption.getById(tespiaservers.getWorld()).show()
						&& TespiaWorldOption.getById(tespiaservers.getWorld()) != null) {
					c.getSession().write(LoginPacket.getServerList(
							Integer.parseInt(tespiaservers.getWorld().replace("t", "")), LoginServer.getLoad()));
				}
			}
		} else {
			for (WorldOption servers : WorldOption.values()) {
		...
		if (ServerConstants.TESPIA) {
			for (TespiaWorldOption value : TespiaWorldOption.values()) {
				...
				// final List<MapleCharacter> chars =
				// c.loadTespiaCharacters(world);
				// if (chars != null) {
				// hasCharacters = true;
				// break;
				// }
	...
	public static void CharlistRequest(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (!World.isChannelAvailable(channel, server) || !WorldOption.isExists(server)) {
			c.getSession().write(LoginPacket.getLoginFailed(10)); // cannot
																	// process
																	// so many
			return;
		}
		...
		if (!WorldOption.getById(server).isAvailable() && !(c.isGm() && server == WorldConstants.gmserver)) {
			c.getSession().write(CWvsContext.broadcastMsg(1, "We are sorry, but " + WorldConstants.getNameById(server)
		...
		// c.getSession().getRemoteAddress().toString().split(":")[0] + " is
		// connecting to server " + server + " channel " + channel + "");
		final List<MapleCharacter> chars = c.loadCharacters(server);
		if (chars != null && ChannelServer.getInstance(channel) != null) {
			c.setWorld(server);
			c.setChannel(channel);
			// this shit aint needed.
	...
	public static void updateCCards(final LittleEndianAccessor slea, final MapleClient c) {
		...
		for (int i = 1; i <= 6; i++) { // 6 chars
			...
			if ((!c.login_Auth(charId) && charId != 0) || ChannelServer.getInstance(c.getChannel()) == null
					|| !WorldOption.isExists(c.getWorld())) {
				c.getSession().close();
				return;
			}
			cids.put(i, charId);
	...
	public static void CreateUltimate(final LittleEndianAccessor slea, final MapleClient c) {
		if (!c.getPlayer().isGM() && (!c.isLoggedIn() || c.getPlayer() == null || c.getPlayer().getLevel() < 120
				|| c.getPlayer().getMapId() != 130000000 || c.getPlayer().getQuestStatus(20734) != 0
				|| c.getPlayer().getQuestStatus(20616) != 2 || !GameConstants.isKOC(c.getPlayer().getJob())
				|| !c.canMakeCharacter(c.getPlayer().getWorld()))) {
			c.getSession().write(CField.createUltimate(2));
			// Character slots are full. Please purchase another slot from the
			// Cash Shop.
			return;
		}
		...
		// if (!LoginInformationProvider.getInstance().isEligibleItem(gender, 0,
		// errorCheck.type, face)) {
		...
		newchar.setRemainingSp(job / 100 == 2 ? 128 : 122); // 2 from job
															// advancements. 120
															// from leveling.
															// (mages get +6)
		...
		if (MapleCharacterUtil.canCreateChar(name, c.isGm())
			...
			c.getSession().write(CField.createUltimate(3)); // "You cannot use
															// this name."
		} else {
	...
	public static void DeleteChar(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (Secondpw_Client == null) {
			if (slea.readByte() > 0) { // Specific if user have second password
										// or not
		...
		if (c.getSecondPassword() != null) { // On the server, there's a second
												// password
	...
	public static void Character_login_noPIC(final LittleEndianAccessor slea, final MapleClient c, final boolean view,
		...
		if (haspic) {
			...
			c.getSession().write(CField.getServerIP(c,
	...
	public static void Character_WithoutSecondPassword(final LittleEndianAccessor slea, final MapleClient c,
			final boolean haspic, final boolean view) {
		...
		if (!c.isLoggedIn() || loginFailCount(c) || (currentpw != null && (!currentpw.equals("") || haspic))
				|| !c.login_Auth(charId) || ChannelServer.getInstance(c.getChannel()) == null
				|| !WorldOption.isExists(c.getWorld())) {
			c.getSession().close();
			return;
		}
	...
	public static void Character_WithSecondPassword(final LittleEndianAccessor slea, final MapleClient c,
			final boolean view) {
		if (constants.ServerConfig.DISABLE_PIC) {
		...
		if (!c.isLoggedIn() || loginFailCount(c) || c.getSecondPassword() == null || !c.login_Auth(charId)
				|| ChannelServer.getInstance(c.getChannel()) == null || !WorldOption.isExists(c.getWorld())) {
			c.getSession().close();
			return;
		}
		c.updateMacs(slea.readMapleAsciiString());
