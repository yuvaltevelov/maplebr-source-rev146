...
public class NPCConversationManager extends AbstractPlayerInteraction {
	...
	public static boolean hairExists(int hair) {
		MapleDataProvider data = MapleDataProviderFactory
				.getDataProvider(new File(System.getProperty("wzpath") + "/Character.wz/Hair"));
	...
	public static boolean faceExists(int face) {
		MapleDataProvider data = MapleDataProviderFactory
				.getDataProvider(new File(System.getProperty("wzpath") + "/Character.wz/Face"));
		final MapleDataDirectoryEntry root = data.getRoot();
		for (MapleDataFileEntry topDir : root.getFiles()) {
			int id = Integer.parseInt(topDir.getName().substring(0, 8));
			if (id == face) {
	...
	public int gainGachaponItemp(int id, int quantity, String msg, byte rareness) {
		try {
			...
			if (rareness > 0) {
				World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(this.c.getPlayer().getName(),
						" : got a(n)", item, rareness, "from The Great Gachapierrot!"));
			}
			World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(this.c.getPlayer().getName(), " : got a(n)",
					item, rareness, "from The Great Gachapierrot!"));
	...
	public int gainGachaponItem(int id, int quantity, String msg, byte rareness) {
		try {
			...
			if (rareness > 0) {
				World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(this.c.getPlayer().getName(),
						" : got a(n)", item, rareness, "from Gachapon!"));
			}
			World.Broadcast.broadcastMessage(CWvsContext.getGachaponMega(this.c.getPlayer().getName(), " : got a(n)",
					item, rareness, "from Gachapon!"));
			this.c.getSession().write(CWvsContext.InfoPacket.getShowItemGain(item.getItemId(), (short) quantity, true));
			return item.getItemId();
		} catch (Exception e) {
			e.printStackTrace();
	...
	public final boolean checkSkillForJob(int skillid) {
		if (GameConstants.isApplicableSkill(skillid) && skillCanBeLearnedByJob(getPlayer().getJob(), skillid)) { // no
																													// db/additionals/resistance
																													// skills
			return true;
		}
		...
		CloneTimer.getInstance().schedule(new Runnable() {
			...
			public void run() {
				if (chr == null || player == null) {
					...
					chr.getMap()
							.broadcastMessage(CWvsContext.yellowChat(chr.getName() + ", do you take " + player
									.getName()
	...
	public Triple<Integer, Integer, Integer> getCompensation() {
		...
		try {
			try (PreparedStatement ps = DatabaseConnection.getConnection()
					.prepareStatement("SELECT * FROM compensationlog_confirmed WHERE chrname LIKE ?")) {
	...
	/*
	 * public void LumiMove927020000() { try {
	 * c.getPlayer().getClient().getSession().write(CField.UIPacket.
	 * getDirectionInfo((byte) 3, 2));
	 * c.getPlayer().getClient().getSession().write(CField.UIPacket.
	 * getDirectionInfo((byte) 1, 6000)); Thread.sleep(5000);
	 * c.getPlayer().getClient().getSession().write(CField.UIPacket.
	 * getDirectionInfo((byte) 3, 0)); NPCScriptManager.getInstance().start(c,
	 * 2159353, "Lumi_tut2"); } catch (InterruptedException e) { } }
	 */
	...
	public void showBeastTamerTutScene1() {
		try {
			...
			c.getSession().write(CField.UIPacket.getDirectionInfo(1, 1000));
			Thread.sleep(2000);
			c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction14.img/effect/ShamanBT/balloonMsg/10", 2000,
					0, -120, 1, 0));
			...
			Thread.sleep(1200);
			c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction14.img/effect/ShamanBT/BalloonMsg1/7", 2000,
					571, -120, 1, 0));
	...
	public boolean skillCanBeLearnedByJob(int job, int skillid) { // test
		// if (GameConstants.getBeginnerJob((short) (id / 10000)) ==
		// GameConstants.getBeginnerJob((short) job))
		// return true;
		int jid = job;
		...
		if (skillForJob == 2001) {
			...
			return GameConstants.isAdventurer(job); // special exception for
													// beginner
			...
			return GameConstants.isResistance(job); // special exception for
													// beginner
		} else if (skillForJob == 1) {
			return GameConstants.isCannon(job); // special exception for
												// beginner
		} else if (skillForJob == 3001) {
			return GameConstants.isDemonSlayer(job) || GameConstants.isDemonAvenger(job); // special
																							// exception
																							// for
																							// beginner
		} else if (skillForJob == 2002) {
			return GameConstants.isMercedes(job); // special exception for
													// beginner
		} else if (skillForJob == 508) {
			return GameConstants.isJett(job); // special exception for beginner
		} else if (skillForJob == 2003) {
			return GameConstants.isPhantom(job); // special exception for
													// beginner
		} else if (skillForJob == 5000) {
			return GameConstants.isMihile(job); // special exception for
												// beginner
		} else if (skillForJob == 2004) {
			return GameConstants.isLuminous(job); // special exception for
													// beginner
		} else if (skillForJob == 6000) {
			return GameConstants.isKaiser(job); // special exception for
												// beginner
		} else if (skillForJob == 6001) {
			return GameConstants.isAngelicBuster(job); // special exception for
														// beginner
			...
			return false;
		} else if ((job / 10) % 10 == 0 && (skillForJob / 10) % 10 > (job / 10) % 10) { // wrong
																						// 2nd
																						// job
			return false;
		} else if ((skillForJob / 10) % 10 != 0 && (skillForJob / 10) % 10 != (job / 10) % 10) { // wrong
																									// 2nd
																									// job
			return false;
