...
public class ShopAnalyzer {
	...
	public static void main(String[] args) throws IOException {
		// System.out.println("Length:" + HexTool.getByteArrayFromHexString("40
		// E0 FD 3B 37 4F 01 00 80 05 BB 46 E6 17 02 00 00 00 00 00 00 00 00 00
		// 01 00 64 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 75 96
		// 8F 00 00 00 00 00 76 96 8F 00 00 00 00 00 77 96 8F 00 00 00 00 00 78
		// 96 8F 00 00 00 00 00").length);
		String data = "";
		// Loading Data

		/*
		 * FileInputStream in = new FileInputStream("ShopData.txt"); while
		 * (in.available() > 0) { System.out.println(in.available()); data +=
		 * (char) in.read(); }
		 */
		/*
		 * try (SeekableByteChannel ch =
		 * Files.newByteChannel(Paths.get("ShopData.txt"))) { long remaining =
		 * ch.size(); ByteBuffer bb = ByteBuffer.allocateDirect(1000); int n =
		 * 0; while (n != -1) { n = ch.read(bb);
		 * System.out.println(--remaining); data += (char) n; } }
		 */
		...
		// System.out.println("\r\nPlease enter the file name of the text file
		// the shop item data will be saved into: \r\n");
		// FileOutputStream out = new FileOutputStream(input.next() + ".txt",
		// false);
		System.out.println("\r\n\r\n");
		// Parsing Data
		...
		for (int size = 1; size < data.split("@").length; size++) {
			...
			sb.append("#SQL:\r\n");
			sb.append("#shops:\r\n\r\n");
			sb.append("INSERT INTO shops (`shopid`, `npcid`) VALUES (").append(shopid).append(", ").append(shopid)
					.append(");\r\n\r\n");
			sb.append("#shopitems:\r\n\r\n");
			...
			for (int i = 1; i <= itemsize; i++) {
				...
				/*
				 * slea.readInt(); //0 slea.readInt(); //0 if
				 * ((!GameConstants.isThrowingStar(itemid)) &&
				 * (!GameConstants.isBullet(itemid))) { slea.readShort();
				 * //always 1 slea.readShort(); //always 1000 (might be recharge
				 * price) } else { slea.readInt(); //0 slea.readShort(); //0
				 * slea.readShort(); //price? might be recharge price short
				 * itemsPerSlot = slea.readShort(); }
				 */
				...
				// sb.append("#SQL:\r\n");
				// sb.append("#shops:\r\n\r\n");
				// sb.append("INSERT INTO shops (`shopid`, `npcid`) VALUES
				...
				sb.append(
						...
						.append(shopid).append(", ").append(itemid).append(", ").append(price).append(", ").append(i)
						.append(", ").append(reqItem).append(", ").append(reqItemQuantity).append(", ").append(rank)
						.append(", ").append(quantity).append(", ").append(buyable).append(", ").append(category)
						.append(");\r\n");
				// }
			}
