...
public class PlayerCommand {
	 ...
	 * (!ServerConstants.AntiKS) { return 0; } MapleQuestStatus stat =
	 * c.getPlayer().getQuestNoAdd(MapleQuest.getInstance(732648172)); boolean
	 * antiks; if (stat == null || stat.getCustomData() == null) { antiks =
	 * false; stat.setCustomData(!antiks + ";" + 0); } else { String[] stats =
	 * stat.getCustomData().split(";"); try { antiks =
	 * Boolean.parseBoolean(stats[0]); } catch (Exception ex) { antiks = false;
	 * } stat.setCustomData(!antiks + ";" + stats[1]); }
	 * c.getPlayer().dropMessage(5, "AntiKS " + (antiks ? "activated" :
	 ...
	 * SkillFactory.getSkill(1142).getEffect(1).applyTo(c.getPlayer()); } else {
	 ...
	 * c.getPlayer().resetStats(4, 4, 4, 4); return 1; } }
	 * 
	 * public static class SellItems extends CommandExecute {
	 * 
	 * @Override public int execute(MapleClient c, String[] splitted) {
	 * MapleCharacter player = c.getPlayer(); if (splitted.length < 3 ||
	 * player.hasBlockedInventory()) { c.getPlayer().dropMessage(5,
	 * "@sellitems <eq/use/setup/etc> <starting slot> <ending slot>"); return 0;
	 * } else { MapleInventoryType type; if (splitted[1].equalsIgnoreCase("eq"))
	 * { type = MapleInventoryType.EQUIP; } else if
	 * (splitted[1].equalsIgnoreCase("use")) { type = MapleInventoryType.USE; }
	 * else if (splitted[1].equalsIgnoreCase("setup")) { type =
	 * MapleInventoryType.SETUP; } else if (splitted[1].equalsIgnoreCase("etc"))
	 * { type = MapleInventoryType.ETC; } else { c.getPlayer().dropMessage(5,
	 * "Invalid. @sellitems <eq/use/setup/etc>"); return 0; } MapleInventory inv
	 * = c.getPlayer().getInventory(type); byte start =
	 * Byte.parseByte(splitted[2]); byte end = Byte.parseByte(splitted[3]); int
	 * totalMesosGained = 0; for (byte i = start; i <= end; i++) { if
	 * (inv.getItem(i) != null) { MapleItemInformationProvider iii =
	 * MapleItemInformationProvider.getInstance(); int itemPrice = (int)
	 * iii.getPrice(inv.getItem(i).getItemId()); totalMesosGained += itemPrice;
	 * player.gainMeso(itemPrice < 0 ? 0 : itemPrice, true);
	 * MapleInventoryManipulator.removeFromSlot(c, type, i,
	 * inv.getItem(i).getQuantity(), true); } } c.getPlayer().dropMessage(5,
	 * "You sold slots " + start + " to " + end + ", and gained " +
	 * totalMesosGained + " mesos."); } return 1; } }
	 */
	public static class maxSkills extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			for (Skill skil : SkillFactory.getAllSkills()) {
				if (GameConstants.isApplicableSkill(skil.getId()) && skil.canBeLearnedBy(c.getPlayer().getJob())
						&& !skil.isInvisible()) { // no
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			if (eem.getProperty("entryPossible").equals("false")) {
				c.getPlayer().dropMessage(5,
						"Entry to the [Pink Zakum] Event is currently closed please wait patiently for the next entrance!");
			}
			return 1;
		}
	}
	...
	public static class ExpFix extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			c.getPlayer()
					.setExp(c.getPlayer().getExp() - GameConstants.getExpNeededForLevel(c.getPlayer().getLevel()) >= 0
							? GameConstants.getExpNeededForLevel(c.getPlayer().getLevel()) : 0);
			return 1;
		}
	}
	...
	public abstract static class DistributeStatCommands extends CommandExecute {
		...
		private void setStat(MapleCharacter player, int current, int amount) {
			...
			case MAXHP:
				...
				// player.getStat().setMaxHp((short) (current + amount * 30),
				// player);
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			if (stat == MapleStat.MAXHP) {
				c.getPlayer().dropMessage(5,
						StringUtil.makeEnumHumanReadable(stat.name()) + " has been raised by " + change * 30 + ".");
				c.getPlayer().fakeRelog();
			} else {
				c.getPlayer().dropMessage(5,
						StringUtil.makeEnumHumanReadable(stat.name()) + " has been raised by " + change + ".");
			}
			return 1;
		}
	}
	 ...
	 * 
	 * protected int npc = -1; private static int[] npcs = { //Ish yur job to
	 * make sure these are in order and correct ;( 9270035, 9900000, 9000000,
	 * 9000030, 9010000, 9000085, 9000018, 9900000, // yes, i know, again but
	 * for other reason. 9010000 // again too, but used for opening a scriptname
	 * };
	 * 
	 * @Override public int execute(MapleClient c, String[] splitted) { if (npc
	 * != 6 && npc != 5 && npc != 4 && npc != 3 && npc != 1 &&
	 * c.getPlayer().getMapId() != 910000000) { //drpcash can use anywhere if
	 * (c.getPlayer().getLevel() < 10 && c.getPlayer().getJob() != 200) {
	 * c.getPlayer().dropMessage(5,
	 * "You must be over level 10 to use this command."); return 0; } if
	 * (c.getPlayer().isInBlockedMap()) { c.getPlayer().dropMessage(5,
	 * "You may not use this command here."); return 0; } } else if (npc == 1) {
	 * if (c.getPlayer().getLevel() < 70) { c.getPlayer().dropMessage(5,
	 * "You must be over level 70 to use this command."); return 0; } } if
	 * (c.getPlayer().hasBlockedInventory()) { c.getPlayer().dropMessage(5,
	 * "You may not use this command here."); return 0; }
	 * NPCScriptManager.getInstance().start(c, npcs[npc], npc == 1 ? "CashDrop"
	 * : npc == 8 ? "BossWarp" : null); return 1; } }
	 * 
	 * public static class Style extends Stylist { }
	 * 
	 * public static class Stylist extends OpenNPCCommand {
	 * 
	 * public Stylist() { npc = 7; } }
	 * 
	 * public static class Npc extends Maple { }
	 * 
	 * public static class Maple extends OpenNPCCommand {
	 * 
	 * public Crescent() { npc = 0; } }
	 * 
	 * public static class BossWarp extends OpenNPCCommand {
	 * 
	 * public BossWarp() { npc = 8; } }
	 * 
	 * public static class Advance extends CommandExecute {
	 * 
	 * @Override public int execute(MapleClient c, String[] splitted) { if
	 * (c.getPlayer().isInBlockedMap()) { c.getPlayer().dropMessage(5,
	 * "You may not use this command here."); return 0; } if
	 * (c.getPlayer().getLevel() < 10) { c.getPlayer().dropMessage(5,
	 * "You must be over level 10 to use this command."); return 0; }
	 * NPCScriptManager.getInstance().start(c, 9900002, null); return 1; } }
	 */
	 ...
	 * player.hasBlockedInventory()) { c.getPlayer().dropMessage(5,
	 * "@clearslot <eq / use / setup / etc / cash / all >"); return 0; } else {
	 * MapleInventoryType type; if (splitted[1].equalsIgnoreCase("eq")) { type =
	 ...
	 * c.getPlayer().dropMessage(5,
	 * "Invalid. @clearslot <eq / use / setup / etc / cash / all >"); return 0;
	 * } if (type == null) { //All, a bit hacky, but it's okay for
	 * (MapleInventoryType t : invs) { type = t; MapleInventory inv =
	 * c.getPlayer().getInventory(type); byte start = -1; for (byte i = 0; i <
	 * inv.getSlotLimit(); i++) { if (inv.getItem(i) != null) { start = i;
	 * break; } } if (start == -1) { c.getPlayer().dropMessage(5,
	 * "There are no items in that inventory."); return 0; } int end = 0; for
	 * (byte i = start; i < inv.getSlotLimit(); i++) { if (inv.getItem(i) !=
	 * null) { MapleInventoryManipulator.removeFromSlot(c, type, i,
	 ...
	 * inv.getSlotLimit(); i++) { if (inv.getItem(i) != null) { start = i;
	 * break; } } if (start == -1) { c.getPlayer().dropMessage(5,
	 * "There are no items in that inventory."); return 0; } byte end = 0; for
	 * (byte i = start; i < inv.getSlotLimit(); i++) { if (inv.getItem(i) !=
	 * null) { MapleInventoryManipulator.removeFromSlot(c, type, i,
	 * inv.getItem(i).getQuantity(), true); } else { end = i; break;//Break at
	 * first empty space. } } c.getPlayer().dropMessage(5, "Cleared slots " +
	...
	public static class home extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			if (c.getPlayer().hasBlockedInventory() || c.getPlayer().getMap().getSquadByMap() != null
	 ...
	 * 
	 * public int execute(MapleClient c, String[] splitted) {
	 * c.removeClickedNPC(); NPCScriptManager.getInstance().dispose(c);
	 * c.getSession().write(CWvsContext.enableActions()); return 1; } }
	 * 
	 * public static class TSmega extends CommandExecute {
	 * 
	 * public int execute(MapleClient c, String[] splitted) {
	 * c.getPlayer().setSmega(); return 1; } }
	 * 
	 * public static class Ranking extends CommandExecute {
	 * 
	 * public int execute(MapleClient c, String[] splitted) { if
	 * (splitted.length < 4) { //job start end c.getPlayer().dropMessage(5,
	 * "Use @ranking [job] [start number] [end number] where start and end are ranks of the players"
	 * ); final StringBuilder builder = new StringBuilder("JOBS: "); for (String
	 * b : RankingWorker.getJobCommands().keySet()) { builder.append(b);
	 * builder.append(" "); } c.getPlayer().dropMessage(5, builder.toString());
	 * } else { int start = 1, end = 20; try { start =
	 * Integer.parseInt(splitted[2]); end = Integer.parseInt(splitted[3]); }
	 * catch (NumberFormatException e) { c.getPlayer().dropMessage(5,
	 * "You didn't specify start and end number correctly, the default values of 1 and 20 will be used."
	 * ); } if (end < start || end - start > 20) { c.getPlayer().dropMessage(5,
	 * "End number must be greater, and end number must be within a range of 20 from the start number."
	 * ); } else { final Integer job = RankingWorker.getJobCommand(splitted[1]);
	 * if (job == null) { c.getPlayer().dropMessage(5,
	 * "Please use @ranking to check the job names."); } else { final
	 * List<RankingInformation> ranks =
	 * RankingWorker.getRankingInfo(job.intValue()); if (ranks == null ||
	 * ranks.size() <= 0) { c.getPlayer().dropMessage(5,
	 * "Please try again later."); } else { int num = 0; for (RankingInformation
	 * rank : ranks) { if (rank.rank >= start && rank.rank <= end) { if (num ==
	 * 0) { c.getPlayer().dropMessage(6, "Rankings for " + splitted[1] +
	 * " - from " + start + " to " + end); c.getPlayer().dropMessage(6,
	 * "--------------------------------------"); } c.getPlayer().dropMessage(6,
	 * rank.toString()); num++; } } if (num == 0) { c.getPlayer().dropMessage(5,
	 * "No ranking was returned."); } } } } } return 1; } }
	 */
	...
	public static class Check extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			c.getPlayer().dropMessage(6,
					"You currently have " + c.getPlayer().getCSPoints(1) + " Cash, " + c.getPlayer().getEPoints()
							+ " Event Points, " + c.getPlayer().getDPoints() + " Donation Points, "
							+ c.getPlayer().getVPoints() + " voting points and "
							+ c.getPlayer().getIntNoRecord(GameConstants.BOSS_PQ) + " Boss Party Quest points.");
			c.getPlayer().dropMessage(6,
					"The time is currently " + FileoutputUtil.CurrentReadable_TimeGMT() + " GMT. | EXP "
							+ (Math.round(c.getPlayer().getEXPMod()) * 100)
									* Math.round(c.getPlayer().getStat().expBuff / 100.0)
					+ "%, Drop "
					+ (Math.round(c.getPlayer().getDropMod()) * 100)
							* Math.round(c.getPlayer().getStat().dropBuff / 100.0) + "%, Meso "
	 ...
	 * 
	 * @Override public int execute(MapleClient c, String[] splitted) { if
	 * (c.getPlayer().isInBlockedMap() || c.getPlayer().hasBlockedInventory()) {
	 * c.getPlayer().dropMessage(5, "You may not use this command here.");
	 * return 0; } NPCScriptManager.getInstance().start(c, 9000021, null);
	 * return 1; } }
	 * 
	 ...
	 * "[System] : <@offerequip, @offeruse, @offersetup, @offeretc, @offercash> <quantity> <name of the item>"
	 * ); return 1; } }
	 * 
	 ...
	 * 
	 * @Override public int execute(MapleClient c, String[] splitted) { if
	 * (splitted.length < 3) { c.getPlayer().dropMessage(-2,
	 * "[Error] : <quantity> <name of item>"); } else if
	 * (c.getPlayer().getLevel() < 70) { c.getPlayer().dropMessage(-2,
	 * "[Error] : Only level 70+ may use this command"); } else { int quantity =
	 * 1; try { quantity = Integer.parseInt(splitted[1]); } catch
	 * (NumberFormatException e) { } String search =
	 * StringUtil.joinStringFrom(splitted, 2).toLowerCase(); Item found = null;
	 * final MapleItemInformationProvider ii =
	 * MapleItemInformationProvider.getInstance(); for (Item inv :
	 * c.getPlayer().getInventory(MapleInventoryType.getByType((byte) invType)))
	 * { if (ii.getName(inv.getItemId()) != null &&
	 * ii.getName(inv.getItemId()).toLowerCase().contains(search)) { found =
	 * inv; break; } } if (found == null) { c.getPlayer().dropMessage(-2,
	 * "[Error] : No such item was found (" + search + ")"); return 0; } if
	 * (GameConstants.isPet(found.getItemId()) ||
	 * GameConstants.isRechargable(found.getItemId())) {
	 * c.getPlayer().dropMessage(-2,
	 * "[Error] : You may not trade this item using this command"); return 0; }
	 * if (quantity > found.getQuantity() || quantity <= 0 || quantity >
	 * ii.getSlotMax(found.getItemId())) { c.getPlayer().dropMessage(-2,
	 * "[Error] : Invalid quantity"); return 0; } if
	 * (!c.getPlayer().getTrade().setItems(c, found, (byte) -1, quantity)) {
	 * c.getPlayer().dropMessage(-2, "[Error] : This item could not be placed");
	 * return 0; } else { c.getPlayer().getTrade().chatAuto("[System] : " +
	 * c.getPlayer().getName() + " offered " + ii.getName(found.getItemId()) +
	 * " x " + quantity); } } return 1; } }
	 * 
	 * public static class OfferEquip extends OfferCommand {
	 * 
	 * public OfferEquip() { invType = 1; } }
	 * 
	 * public static class OfferUse extends OfferCommand {
	 * 
	 * public OfferUse() { invType = 2; } }
	 * 
	 * public static class OfferSetup extends OfferCommand {
	 * 
	 * public OfferSetup() { invType = 3; } }
	 * 
	 * public static class OfferEtc extends OfferCommand {
	 * 
	 * public OfferEtc() { invType = 4; } }
	 * 
	 * public static class OfferCash extends OfferCommand {
	 * 
	 * public OfferCash() { invType = 5; } }
	 */
	/*
	 ...
	 * 100000000) { if (c.getChannelServer().getWaiting() ||
	 * c.getPlayer().isGM()) { //TOD: test
	 ...
	 * 80001005).getEffect(1).applyTo(c.getPlayer());
	 ...
	 * } else { c.getPlayer().dropMessage(0,
	 * "There is no event currently taking place."); return 0; } } else {
	 * c.getPlayer().dropMessage(0, "You are not at Henesys."); return 0; } }
	 * else { c.getPlayer().dropMessage(0, "You have already joined this race."
	 * ); return 0; } return 1; } }
	 * 
	...
	public static class SpawnBomb extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			c.getPlayer().getMap().spawnMonsterOnGroudBelow(MapleLifeFactory.getMonster(9300166),
	...
	public static class CallGM extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			for (final ChannelServer cserv : ChannelServer.getAllInstances()) {
				cserv.broadcastGMMessage(tools.packet.CField.multiChat("[GM Help] " + c.getPlayer().getName(),
						StringUtil.joinStringFrom(splitted, 1), 6));
			}
