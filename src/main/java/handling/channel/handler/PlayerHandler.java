...
public class PlayerHandler {
	...
	public static void DressUpTime(LittleEndianAccessor slea, final MapleClient c, MapleCharacter chr) {
		...
		if (type == 1) {
			...
			if (GameConstants.isAngelicBuster(c.getPlayer().getJob())) {
				...
				chr.getMap().broadcastMessage(chr, CField.updateCharLook(chr, true), false);// PLZ
																							// TEST
																							// ANGELIC
																							// CHANGE!
				// }
		...
		if (damage > 0) {
			...
			if (chr.getTotalSkillLevel(SkillFactory.getSkill(27000003)) > 0) {
				...
				if (isDeadlyAttack) {
					...
					double lost = SkillFactory.getSkill(27000003)
							.getEffect(chr.getTotalSkillLevel(SkillFactory.getSkill(27000003))).getX() / 100.0D;
	...
	public static void closeRangeAttack(LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr,
		...
		case 1201011:
			...
			DamageParse.applyAttack(attack, skill, chr, skillLevel, maxdamage, effect, AttackType.NON_RANGED);// applyAttack(attack,
																												// skill,
																												// chr,
																												// bulletCount,
																												// effect,
	...
	public static void rangedAttack(LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
		...
		boolean noBullet = (chr.getJob() >= 300 && chr.getJob() <= 322)
				...
				|| GameConstants.isZero(chr.getJob()) || GameConstants.isBeastTamer(chr.getJob())
			...
			if ((chr.getJob() >= 410 && chr.getJob() <= 412)) {
				...
				if (c.getPlayer().getSkillLevel(SkillFactory.getSkill(4100011)) > 0) {
					...
					final List<MapleMapObject> objs = c.getPlayer().getMap().getMapObjectsInRange(
							c.getPlayer().getPosition(), 500000, Arrays.asList(MapleMapObjectType.MONSTER));
					...
					for (AttackPair at : attack.allDamage) {
						...
						if (Randomizer.nextInt(100) < percent) {
							if (mob != null) {
								final Item star = c.getPlayer().getInventory(MapleInventoryType.USE)
										.getItem(attack.slot);
								...
								c.getPlayer().getMap().broadcastMessage(AdventurerPacket.AssassinPacket.giveMarkOfTheif(
										c.getPlayer().getId(), source.getObjectId(), (skill.getId() + 1), monsters,
										c.getPlayer().getPosition(), monsters.get(0).getPosition(), star.getItemId()));
	...
	public static void MagicDamage(LittleEndianAccessor slea, MapleClient c, final MapleCharacter chr) {
		...
		case 27121100: // Reflection
			if (chr.getLuminousState() <= 20040000 && chr.getLevel() > 30) {
				chr.getClient().getSession().write(JobPacket.LuminousPacket.giveLuminousState(20040216,
						chr.getLightGauge(), chr.getDarkGauge(), 2000000000));
		...
		case 27121202:
			if (chr.getLuminousState() <= 20040000 && chr.getLevel() > 30) {
				chr.getClient().getSession().write(JobPacket.LuminousPacket.giveLuminousState(20040217,
						chr.getLightGauge(), chr.getDarkGauge(), 2000000000));
		...
		/*
		 * if (skill.getId() >= 27100000 && skill.getId() < 27120400 &&
		 * attack.targets > 0 && chr.getLuminousState() < 20040000) {
		 * //chr.changeSkillLevel(SkillFactory.getSkill(20040216), (byte) 1,
		 * (byte) 1); //chr.changeSkillLevel(SkillFactory.getSkill(20040217),
		 * (byte) 1, (byte) 1);
		 * //chr.changeSkillLevel(SkillFactory.getSkill(20040220), (byte) 1,
		 * (byte) 1); //chr.changeSkillLevel(SkillFactory.getSkill(20041239),
		 * (byte) 1, (byte) 1);
		 ...
		 * GameConstants.getLuminousSkillMode(skill.getId()),
		 * chr.getLightGauge(), chr.getDarkGauge(), 10000));
		 * SkillFactory.getSkill(GameConstants.getLuminousSkillMode(skill.getId(
		 * ))).getEffect(1).applyTo(chr); }
		 */
		...
		// double maxdamage = chr.getStat().getCurrentMaxBaseDamage() *
		// (effect.getDamage() + chr.getStat().getDamageIncrease(attack.skill))
		// / 100.0D;
								...
								if (clone.getMap() == map) {
									if (clone.isHidden()) {
										map.broadcastGMMessage(clone,
												CField.movePlayer(clone.getId(), res3, Original_Pos), false);
									} else {
	...
	public static void ChangeMapSpecial(String portal_name, MapleClient c, MapleCharacter chr) {
		if(MapleServerConfig.isDebugMode){
			System.out.println("Trying to change map special using script " + portal_name + " map: " + chr.getMap().getId());
		}
		if ((chr == null) || (chr.getMap() == null)) {
			return;
		}
		...
		if (slea.available() != 0L) {
			...
			if ((targetid != -1) && (!chr.isAlive())) {
				...
				if (!wheel) {
					...
					c.getSession().write(CField.EffectPacket
