	...
	public static DojoRankingsData loadLeaderboard() {
		...
		try {
			PreparedStatement ps = con
					.prepareStatement("SELECT `name`, `time` FROM `dojorankings` ORDER BY `time` ASC LIMIT " + limit);
			ResultSet rs = ps.executeQuery();

			int i = 0;
			while (rs.next()) {
				if (rs.getInt("time") != 0) {
					// long time = (rs.getLong("endtime") -
					// rs.getLong("starttime")) / 1000;
					ret.ranks[i] = (i + 1);
			...
			// PreparedStatement pss = con.prepareStatement("INSERT INTO
			// dojodata VALUES(DEFAULT, DEFAULT, DEFAULT, ?, DEFAULT,
			// DEFAULT)");
			// pss.setLong(1, time);
