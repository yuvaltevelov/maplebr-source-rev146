...
public class CrusaderCodex {
	...
	public static void main(String[] args) {
		...
		MapleDataProvider data = MapleDataProviderFactory
				.getDataProvider(MapleDataProviderFactory.fileInWZPath("Item.wz"));
		try {
			String s;
			...
			try (PrintWriter writer = new PrintWriter(new FileOutputStream(text))) {
				while ((s = b.readLine()) != null) {
					...
					SeekableLittleEndianAccessor slea = new GenericSeekableLittleEndianAccessor(
							new ByteArrayByteStream(bArray));
					...
					// Don't forget to insert the card, along with the rest of
					// the drops.
					sb.append("INSERT INTO `drop_data` (`dropperid`, `itemid`, `questid`, `chance`) VALUES (")
							.append(monsterid).append(", ").append(card).append(", 0, 400);\r\n");
					for (int i = 0; i < size; i++) {
						int itemid = slea.readInt();
						sb.append("INSERT INTO `drop_data` (`dropperid`, `itemid`, `questid`, `chance`) VALUES ");
						sb.append("(").append(monsterid).append(", ").append(itemid).append(", 0, ")
								.append(getChance(itemid) == -1 ? -1 : getChance(itemid) * 10).append(");\r\n");
					}
