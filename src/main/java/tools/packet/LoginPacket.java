...
public class LoginPacket {
	...
	public static byte[] getAuthSuccessRequest(MapleClient client) {
		...
		mplew.write(JobConstants.jobOrder); // Job Order (orders are located in
											// wz)
	 ...
	 * maintenance check site for updates 35 - your computer is running thirdy
	 * part programs close them and play again 36 - due to high population char
	 * creation has been disabled 43 - revision needed your ip is temporary
	 * blocked 75-78 are cool for auto register
	 * 
	 */
	...
	public static byte[] changeBackground(List<Triple<String, Integer, Boolean>> backgrounds) {
		 ...
		 */
		/*
		 * Background are like layers, backgrounds in the packets are removed,
		 * so the background which was hiden by the last one is shown.
		 */
	...
	public static byte[] getCharList(String secondpw, List<MapleCharacter> chars, int charslots) {
		...
		if (constants.ServerConfig.DISABLE_PIC) {
			...
			mplew.write((secondpw != null) && (secondpw.length() <= 0) ? 2
					: (secondpw != null) && (secondpw.length() > 0) ? 1 : 0);
		}
		mplew.write(0);
