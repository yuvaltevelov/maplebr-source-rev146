...
public class DatabaseConnection {
	...
	public static Connection getConnection() {
		...
		try {
			c.getMetaData();
		} catch (SQLException e) { // connection is dead, therefore discard old
									// object
			con.remove();
			c = con.get();
		}
		...
		protected Connection initialValue() {
			try {
				return DriverManager
						.getConnection(
								"jdbc:mysql://127.0.0.1:" + ServerConstants.SQL_PORT + "/"
										+ ServerConstants.SQL_DATABASE + "?autoReconnect=true",
