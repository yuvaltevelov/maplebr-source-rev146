...
public class InventoryHandler {
	...
	private static List<Item> sortItems(final List<Item> passedMap) {
		...
		final List<Item> sortedList = new LinkedList<>(); // ordered list pl0x
															// <3.
		...
		if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && !chr.hasBlockedInventory()) {
			if (chr.getInventory(MapleInventoryType.EQUIP).getNextFreeSlot() > -1
				...
				case 2291020:
					...
					// World.Broadcast.broadcastMessage(CField.getGameMessage("SMB.",
					// (short) 8));
				...
				if (rewards != null && rewards.getLeft() > 0) {
					...
					if (itemId == 2028162) { // custom test
						List<Integer> items;
						Integer[] itemArray = { 1302000, 1302001, 1302002, 1302003, 1302004, 1302005, 1302006,
								1302007 };
						...
						if (unseal) {
							...
							c.getSession().write(EffectPacket.showRewardItemAnimation(2028162, "")); // sealed
																										// box
							c.getSession().write(InfoPacket.getShowItemGain(item.getItemId(), (short) 1, true));
						} else {
							c.getSession().write(CField.sendSealedBox(slot, 2028162, items)); // sealed
																								// box
						}
					}
					switch (itemId) {
	...
	public static final void UseItem(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		...
		if (!FieldLimitType.PotionUse.check(chr.getMap().getFieldLimit())) { // cwk
																				// quick
																				// hack
	...
	public static final void UseAlienSocket(final LittleEndianAccessor slea, final MapleClient c) {
		...
		// c.getPlayer().dropMessage(1, "Added 1 socket successfully to " +
		// toMount);
	}
	...
	public static final void UseNebulite(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (eqq.getSocket1() == 0/*
									 * || eqq.getSocket2() == 0 || eqq.getSocket3()
									 * == 0
									 */) { // GMS removed 2nd and 3rd sockets, we
										// can put into npc.
	...
	public static final void UseNebuliteFusion(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (grade1 == -1 || grade2 == -1 || (highestRank == 3 && premiumQuantity != 2)
				...
				|| grade1 >= 4 || grade2 >= 4
				|| (c.getPlayer().getInventory(MapleInventoryType.SETUP).getNumFreeSlot() < 1)) { // 4000
																									// +
																									// =
																									// S,
																									// 3000
			...
			return; // Most of them were done in client, so we just send the
					// unsuccessfull packet, as it is only here when they packet
					// edit.
		}
		final int avg = (grade1 + grade2) / 2; // have to revise more about
												// grades.
		final int rank = Randomizer.nextInt(100) < 4
				? (Randomizer.nextInt(100) < 70 ? (avg != 3 ? (avg + 1) : avg) : (avg != 0 ? (avg - 1) : 0)) : avg;
		// 4 % chance to up/down 1 grade, (70% to up, 30% to down), cannot up to
		// S grade. =)
		final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
	...
	public static void UseMagnify(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (eqq.getState() == 1 && (src == 0x7F && price != -1 && c.getPlayer().getMeso() >= price || insight
			...
			while (eqq.getState() != new_state) {
				// 31001 = haste, 31002 = door, 31003 = se, 31004 = hb, 41005 =
				// combat orders, 41006 = advanced blessing, 41007 = speed
				// infusion
				for (int i = 0; i < lines; i++) { // minimum 2 lines, max 5
					boolean rewarded = false;
					while (!rewarded) {
						StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
						if (pot != null && pot.reqLevel / 1 <= reqLevel
			...
			c.getPlayer().getTrait(MapleTraitType.insight).addExp(
					(src == 0x7F && price != -1 ? 10 : insight ? 10 : ((magnify.getItemId() + 2) - 2460000)) * 2,
					c.getPlayer());
			c.getPlayer().getMap()
					.broadcastMessage(CField.showMagnifyingEffect(c.getPlayer().getId(), eqq.getPosition()));
			if (!insight && src != 0x7F) {
				c.getSession()
						.write(InventoryPacket.scrolledItem(magnify,
								equipped ? MapleInventoryType.EQUIPPED : MapleInventoryType.EQUIP, toReveal, false,
								true, equipped));
				...
				c.getPlayer().forceReAddItem(toReveal,
						eqSlot >= 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED);
	...
	public static boolean magnifyEquip(final MapleClient c, Item magnify, Item toReveal, byte eqSlot) {
		...
		if (eqq.getState() == 1
			...
			if (!insight) {
				c.getSession()
						.write(InventoryPacket.scrolledItem(magnify,
								eqSlot >= 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED, toReveal, false,
								true, false));
			} else {
				c.getPlayer().forceReAddItem(toReveal,
						eqSlot >= 0 ? MapleInventoryType.EQUIP : MapleInventoryType.EQUIPPED);
	...
	public static boolean isAllowedPotentialStat(Equip eqq, int opID) { // For
																		// now
		// if (GameConstants.isWeapon(eqq.getItemId())) {
		// return !(opID > 60000) || (opID >= 1 && opID <= 4) || (opID >= 9 &&
		// opID <= 12) || (opID >= 10001 && opID <= 10006) || (opID >= 10011 &&
		// opID <= 10012) || (opID >= 10041 && opID <= 10046) || (opID >= 10051
		// && opID <= 10052) || (opID >= 10055 && opID <= 10081) || (opID >=
		// 10201 && opID <= 10291) || (opID >= 210001 && opID <= 20006) || (opID
		// >= 20011 && opID <= 20012) || (opID >= 20041 && opID <= 20046) ||
		// (opID >= 20051 && opID <= 20052) || (opID >= 20055 && opID <= 20081)
		// || (opID >= 20201 && opID <= 20291) || (opID >= 30001 && opID <=
		// 30006) || (opID >= 30011 && opID <= 30012) || (opID >= 30041 && opID
		// <= 30046) || (opID >= 30051 && opID <= 30052) || (opID >= 30055 &&
		// opID <= 30081) || (opID >= 30201 && opID <= 30291) || (opID >= 40001
		// && opID <= 40006) || (opID >= 40011 && opID <= 40012) || (opID >=
		// 40041 && opID <= 40046) || (opID >= 40051 && opID <= 40052) || (opID
		// >= 40055 && opID <= 40081) || (opID >= 40201 && opID <= 40291);
		// }
		return opID < 60000;
	}
	...
	public static boolean UseUpgradeScroll(final short slot, final short dst, final short ws, final MapleClient c,
		...
		} else /* if (legendarySpirit) */ {// may want to create a boolean for
											// strengthen ui? lol
			toScroll = (Equip) chr.getInventory(MapleInventoryType.EQUIP).getItem(dst);
		}
		// if (toScroll == null || c.getPlayer().hasBlockedInventory())
		// {//removed just in case :P
		// c.getSession().write(CWvsContext.enableActions());
		// return false;
		// }
		...
		if (GameConstants.isAzwanScroll(scroll.getItemId())) {
			if (toScroll.getUpgradeSlots() < MapleItemInformationProvider.getInstance()
					.getEquipStats(scroll.getItemId()).get("tuc")) {
				c.getSession().write(InventoryPacket.getInventoryFull());
				c.getSession().write(CWvsContext.enableActions());
				return false;
			}
		...
		if (!GameConstants.isSpecialScroll(scroll.getItemId()) && !GameConstants.isCleanSlate(scroll.getItemId())
			...
			if ((!isEpic && !isUnique && toScroll.getState() >= 1) || (isEpic && toScroll.getState() >= 18)
					|| (isUnique && toScroll.getState() >= 19)
					|| (toScroll.getLevel() == 0 && toScroll.getUpgradeSlots() == 0
							&& toScroll.getItemId()
									/ 10000 != 135/* && !isEpic && !isUnique */)
		...
		if (scrollSuccess == Equip.ScrollResult.CURSE) {
			c.getSession().write(
					InventoryPacket.scrolledItem(scroll, MapleInventoryType.EQUIP, toScroll, true, false, false));
			if (dst < 0) {
				chr.getInventory(MapleInventoryType.EQUIPPED).removeItem(toScroll.getPosition());
			} else {
			...
			c.getSession().write(
					InventoryPacket.scrolledItem(scroll, MapleInventoryType.EQUIP, scrolled, false, false, false));
		}

		chr.getMap().broadcastMessage(chr, CField.getScrollEffect(c.getPlayer().getId(), scrollSuccess, legendarySpirit,
				toScroll.getItemId(), scroll.getItemId()), vegas == 0);
		// toscroll
		// scroll
		c.getSession().write(CField.enchantResult(
				scrollSuccess == ScrollResult.SUCCESS ? 1 : scrollSuccess == ScrollResult.CURSE ? 2 : 0));
		// addToScrollLog(chr.getAccountID(), chr.getId(), scroll.getItemId(),
		// itemID, oldSlots, (byte)(scrolled == null ? -1 :
		// scrolled.getUpgradeSlots()), oldVH, scrollSuccess.name(),
		// whiteScroll, legendarySpirit, vegas);
	...
	public static void UseExpPotion(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		...
		c.getSession()
				.write(CWvsContext.updateExpPotion(last ? 0 : 2, chr.getId(), itemid, first, level, potionDstLevel));
		if (first) {
	...
	public static void UseCarvedSeal(final LittleEndianAccessor slea, final MapleClient c) {
		...
		final Integer success = MapleItemInformationProvider.getInstance().getEquipStats(toUse.getItemId())
				.get("success");
		if (success == null || Randomizer.nextInt(100) <= success) {
			if (item != null) {
				final Equip eq = (Equip) item;
				...
				while (eq.getBonusState() != new_state) {
					...
					// 41005 = combat orders, 41006 = advanced blessing, 41007 =
					// speed infusion
					for (int i = 0; i < lines; i++) { // minimum 2 lines, max 3
						boolean rewarded = false;
						while (!rewarded) {
							StructItemOption pot = pots.get(Randomizer.nextInt(pots.size())).get(reqLevel);
							if (pot != null && pot.reqLevel / 10 <= reqLevel
									&& GameConstants.optionTypeFits(pot.optionType, eq.getItemId())
									&& GameConstants.potentialIDFits(pot.opID, new_state, i)) { // optionType
				...
				c.getPlayer().getMap()
						.broadcastMessage(CField.showPotentialReset(c.getPlayer().getId(), true, toUse.getItemId()));
				c.getSession()
						.write(InventoryPacket.scrolledItem(toUse, MapleInventoryType.EQUIP, item, false, true, false));
				c.getPlayer().forceReAddItem_NoUpdate(item, MapleInventoryType.EQUIP);
			}
		} else {
			c.getPlayer().getMap()
					.broadcastMessage(CField.showPotentialReset(c.getPlayer().getId(), false, toUse.getItemId()));
		}
		c.getSession().write(CWvsContext.enableActions());
	}
	...
	public static void UseCube(LittleEndianAccessor slea, MapleClient c) {
		...
		if (toUse.getItemId() / 10000 != 271 || item == null || toUse == null
				...
				|| ii.getEquipStats(toUse.getItemId()).containsKey("success")) {
			c.getPlayer().getMap()
					.broadcastMessage(CField.showPotentialReset(c.getPlayer().getId(), false, item.getItemId()));
			c.getSession().write(CField.enchantResult(0));
			return;
		}
		final Equip eq = (Equip) item;
		if (eq.getState() >= 17 && eq.getState() <= 20) {
			...
			c.getPlayer().getMap()
					.broadcastMessage(CField.showPotentialReset(c.getPlayer().getId(), true, item.getItemId()));
			c.getSession()
					.write(InventoryPacket.scrolledItem(toUse, MapleInventoryType.EQUIP, item, false, true, false));
			c.getPlayer().forceReAddItem_NoUpdate(item, MapleInventoryType.EQUIP);
	...
	public static void UseScriptedNPCItem(final LittleEndianAccessor slea, final MapleClient c,
		...
		int mountid = 0;
		// int npc =
		// MapleItemInformationProvider.getInstance().getEquipStats(itemId).get("npc").intValue();
		// String script =
		// MapleItemInformationProvider.getInstance().getEquipStats(itemId).get("script").toString();
		final MapleItemInformationProvider ii = MapleItemInformationProvider.getInstance();
		...
		if (toUse != null && toUse.getQuantity() >= 1 && toUse.getItemId() == itemId && !chr.hasBlockedInventory()
			...
			case 2431789:
			case 2431935: {
				Integer[] itemArray = { 2290000, 2290002, 2290004, 2290006, 2290008, 2290010, 2290012, 2290014, 2290016,
						2290018, 2290019, 2290020, 2290022, 2290024, 2290026, 2290028, 2290030, 2290032, 2290034,
						2290036, 2290038, 2290040, 2290042, 2290044, 2290046, 2290048, 2290050, 2290052, 2290054,
						2290056, 2290058, 2290060, 2290062, 2290064, 2290066, 2290068, 2290070, 2290072, 2290074,
						2290076, 2290078, 2290080, 2290082, 2290084, 2290086, 2290088, 2290090, 2290092, 2290094,
						2290096, 2290097, 2290099, 2290101, 2290102, 2290104, 2290106, 2291022, 2291013, 2291011,
						2291010, 2291008, 2291007, 2291005, 2291003, 2291002, 2291000, 2290998, 2290995, 2290996,
						2290993, 2290979, 2290977, 2290975, 2290973, 2290971, 2290969, 2290967, 2290966, 2290964,
						2290962, 2290960, 2290959, 2290957, 2290955, 2290953, 2290951, 2290949, 2290947, 2290945,
						2290943, 2290941, 2290939, 2290928, 2290798, 2290808, 2290809, 2290810, 2290811, 2290812,
						2290813, 2290814, 2290815, 2290816, 2290817, 2290818, 2290819, 2290820, 2290821, 2290822,
						2290823, 2290824, 2290825, 2290826, 2290827, 2290828, 2290829, 2290830, 2290831, 2290832,
						2290833, 2290834, 2290835, 2290836, 2290837, 2290838, 2290839, 2290840, 2290841, 2290842,
						2290843, 2290844, 2290845, 2290846, 2290847, 2290848, 2290849, 2290850, 2290851, 2290852,
						2290853, 2290854, 2290855, 2290856, 2290857, 2290858, 2290859, 2290860, 2290861, 2290862,
						2290863, 2290864, 2290865, 2290866, 2290867, 2290794, 2290795, 2290796, 2290791, 2290792,
						2290789, 2290787, 2290777, 2290775, 2290773, 2290771, 2290769, 2290767, 2290765, 2290751,
						2290752, 2290753, 2290763, 2290749, 2290747, 2290745, 2290743, 2290741, 2290739, 2290734,
						2290732, 2290729, 2290730, 2290727, 2290725, 2290706, 2290707, 2290704, 2290689, 2290689,
						2290651, 2290649, 2290647, 2290645, 2290639, 2290641, 2290637, 2290635, 2290633, 2290631,
						2290629, 2290627, 2290624, 2290625, 2290622, 2290619, 2290620, 2290617, 2290615, 2290612,
						2290613, 2290610, 2290608, 2290599, 2290597, 2290595, 2290593, 2290591, 2290580, 2290589,
						2290523, 2290521, 2290519, 2290516, 2290517, 2290514, 2290512, 2290446, 2290445, 2290443,
						2290441, 2290438, 2290439, 2290436, 2290434, 2290432, 2290430, 2290426, 2290427, 2290428,
						2290424, 2290422, 2290420, 2290418, 2290416, 2290414, 2290412, 2290378, 2290370, 2290366,
						2290363, 2290364, 2290361, 2290358, 2290359, 2290356, 2290349, 2290354, 2290333, 2290331,
						2290328, 2290329, 2290326, 2290324, 2290322, 2290292, 2290290, 2290284, 2290281, 2290282,
						2290279, 2290277, 2290275, 2290246, 2290244, 2290242, 2290240, 2290238, 2290236, 2290234,
						2290232, 2290230, 2290228, 2290226, 2290206, 2290204, 2290205, 2290153, 2290154, 2290155,
						2290156, 2290151, 2290150, 2290148, 2290146, 2290144, 2290142, 2290140, 2290138, 2290136,
						2290134, 2290132, 2290130, 2290128, 2290126, 2290123, 2290124, 2290121, 2290119, 2290117,
						2290115, 2290114, 2290112, 2290110 };
				...
				if (MapleItemInformationProvider.getInstance().itemExists(reward)) {
					MapleInventoryManipulator.removeById(c, GameConstants.getInventoryType(itemId), itemId, 1, false,
							false);
					MapleInventoryManipulator.addById(c, reward, (short) 1,
							"Reward item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
				}
				break;
			}
			...
			case 2431938: {
				Integer[] itemArray = { 1212063, 1222058, 1232057, 1242060, 1242061, 1252015, 1302275, 1312153, 1322203,
						1332225, 1342082, 1362090, 1372177, 1382208, 1402196, 1412135, 1422140, 1432167, 1442223,
						1452205, 1462193, 1472214, 1482168, 1492179, 1522094, 1532098, 1542063, 1552063 };
				int randomizer = Randomizer.nextInt(itemArray.length);
				int reward = itemArray[randomizer];
				if (MapleItemInformationProvider.getInstance().itemExists(reward)) {
					MapleInventoryManipulator.removeById(c, GameConstants.getInventoryType(itemId), itemId, 1, false,
			...
			case 2430130:
				if (GameConstants.isResistance(c.getPlayer().getJob())) {
					...
					c.getPlayer()
							.gainExp(
									20000 + (c.getPlayer().getLevel() * 50
											* c.getChannelServer().getExpRate(c.getPlayer().getWorld())),
									true, true, false);
				} else {
			...
			case 2430979:
				MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, slot, (byte) 1, false);
				c.getPlayer().gainExp(
						GameConstants.getExpNeededForLevel(c.getPlayer().getLevel()) - c.getPlayer().getExp(), true,
						true, false);
				break;
				...
				if (c.getPlayer().getInventory(MapleInventoryType.CASH).getNumFreeSlot() >= 1
					...
					if (Randomizer.nextInt(100) < 30) { // 30% for Hilla's Pet
						if (MapleInventoryManipulator.checkSpace(c, 5000217, 1, "") && MapleInventoryManipulator
								.removeById(c, MapleInventoryType.USE, toUse.getItemId(), 1, true, false)) {
							MapleInventoryManipulator.addById(c, 5000217, (short) 1, "",
									MaplePet.createPet(5000217, "Blackheart", 1, 0, 100,
											MapleInventoryIdentifier.getInstance(), 0, (short) 0),
									45, false, "Scripted item: " + toUse.getItemId() + " on "
											+ FileoutputUtil.CurrentReadable_Date());
						} else {
							c.getPlayer().dropMessage(0, "Please make more space");
						}
					} else { // 70% for Hilla's Pet's earrings
						if (MapleInventoryManipulator.checkSpace(c, 1802354, 1, "") && MapleInventoryManipulator
								.removeById(c, MapleInventoryType.USE, toUse.getItemId(), 1, true, false)) {
							MapleInventoryManipulator.addById(c, 1802354, (short) 1, "Scripted item: "
									+ toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
						} else {
							c.getPlayer().dropMessage(0, "Please make more space");
						}
			...
			case 2431855: {// First Explorer Gift Box
				if (c.getPlayer().getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() >= 2
					...
					if (MapleInventoryManipulator.checkSpace(c, 1052646, 1, "") && MapleInventoryManipulator
							.removeById(c, MapleInventoryType.USE, toUse.getItemId(), 1, true, false)) {
						MapleInventoryManipulator.addById(c, 1052646, (short) 1,
								"Scripted item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
						MapleInventoryManipulator.addById(c, 1072850, (short) 1,
								"Scripted item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
						MapleInventoryManipulator.addById(c, 2000013, (short) 50,
								"Scripted item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
						MapleInventoryManipulator.addById(c, 2000014, (short) 50,
								"Scripted item: " + toUse.getItemId() + " on " + FileoutputUtil.CurrentReadable_Date());
					} else {
						c.getPlayer().dropMessage(0, "Please make more space");
					}
			...
			case 5680021:
				...
				// MapleInventoryManipulator.addById(c, chair, (short) 1, null,
				// null, expiration, "Chair Gachapon");
				break;
			default:
				NPCScriptManager.getInstance().startItemScript(c, npc, script); // maple
																				// admin
																				// as
																				// default
																				// npc
		...
		if (mountid > 0) {
			...
			if (c.getPlayer().getSkillLevel(mountid) > 0) {
				...
				if (SkillFactory.getSkill(mountid) == null || GameConstants.getMountItem(mountid, c.getPlayer()) == 0) {
	...
	public static final void useInnerCirculator(LittleEndianAccessor slea, MapleClient c) {
		...
		if (item.getItemId() == itemid) {
			...
			for (InnerSkillValueHolder isvh : c.getPlayer().getInnerSkills()) {
				if (!isvh.isLocked()) {
					if (i == 0 && c.getPlayer().getInnerSkills().size() > 1 && itemid == 2702000) { // Ultimate
																									// Circulator
					...
					// c.getPlayer().changeSkillLevel(SkillFactory.getSkill(isvh.getSkillId()),
					// (byte) 0, (byte) 0);
				} else {
					newValues.add(isvh);
				}
			...
			for (InnerSkillValueHolder isvh : newValues) {
				...
				c.getSession().write(
						CField.updateInnerPotential(ability, isvh.getSkillId(), isvh.getSkillLevel(), isvh.getRank()));
				ability++;
				// c.getPlayer().changeSkillLevel(SkillFactory.getSkill(isvh.getSkillId()),
			...
			// c.getSession().write(CField.gameMsg("Inner Potential has been
			// reconfigured.")); //not sure if it's working
	...
	public static final void UseCashItem(final LittleEndianAccessor slea, final MapleClient c) {
		...
		case 5050000: { // AP Reset
			...
			if (used) {
				...
				case 0x2000: // mp
					...
					if (GameConstants.isDemonSlayer(job) || GameConstants.isAngelicBuster(job)
		...
		case 5220083: {// starter pack
			used = true;
			for (Entry<Integer, StructFamiliar> f : MapleItemInformationProvider.getInstance().getFamiliars()
					.entrySet()) {
				if (f.getValue().itemid == 2870055 || f.getValue().itemid == 2871002 || f.getValue().itemid == 2870235
						|| f.getValue().itemid == 2870019) {
		...
		case 5050009: {
			...
			// GameConstants.getJobNumber(skill2 / 10000)) { //putting 3rd job
			// skillpoints into 4th job for example
			// c.getPlayer().dropMessage(1, "You may not add skillpoints to a
			// higher job.");
			// break;
			// }
			if ((c.getPlayer().getSkillLevel(skillSPTo) + 1 <= skillSPTo.getMaxLevel())
				...
				if (itemId >= 5050005) {
					...
					if (theJob != itemId - 5050000) { // you may only subtract
														// from the skill if the
														// ID matches Sp reset
		...
		case 5534000: { // tims lab
			...
			if (item != null) {
				...
				if (eq.getState() == 0) {
					eq.resetPotential();
					c.getPlayer().getMap()
							.broadcastMessage(CField.showPotentialReset(c.getPlayer().getId(), true, itemId));
					c.getSession().write(
							InventoryPacket.scrolledItem(toUse, MapleInventoryType.EQUIP, item, false, true, false));
					c.getPlayer().forceReAddItem_NoUpdate(item, MapleInventoryType.EQUIP);
					c.getSession().write(CField.enchantResult(1));
					used = true;
				} else {
				...
				c.getPlayer().getMap()
						.broadcastMessage(CField.showPotentialReset(c.getPlayer().getId(), false, itemId));
				c.getSession().write(CField.enchantResult(0));
			}
			break;
		}
				...
				if (eq.getState() >= 17 && eq.getState() != 20) {
					...
					if (tofind != 0) {
						...
						if (magnifyEquip(c, magnify, item, (byte) item.getPosition())) {
							MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE, magnify.getPosition(),
									(short) 1, false);
							c.getSession().write(
									CField.getGameMessage("A Magnifying Glass (Premium) has been used.", (short) 7));
						} else {
							c.getSession()
									.write(CField.getGameMessage(
											"A Magnifying Glass was not found. The equipment will stay as Hidden Potential.",
											(short) 7));
						}
					} else {
						c.getSession()
			...
			if (c.getPlayer().getLevel() < 10) {
				...
				if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
					...
					if (eq.getState() >= 17 && eq.getState() != 20) {
						...
						c.getPlayer().getMap()
								.broadcastMessage(CField.showPotentialReset(c.getPlayer().getId(), true, itemId));
						c.getSession().write(InventoryPacket.scrolledItem(toUse, MapleInventoryType.EQUIP, item, false,
								true, false));
						c.getPlayer().forceReAddItem_NoUpdate(item, MapleInventoryType.EQUIP);
						MapleCharacter chr = c.getPlayer();
						int tofind = 0;
						...
						if (tofind != 0) {
							...
							if (magnifyEquip(c, magnify, item, (byte) item.getPosition())) {
								MapleInventoryManipulator.removeFromSlot(c, MapleInventoryType.USE,
										magnify.getPosition(), (short) 1, false);
								c.getSession().write(CField
										.getGameMessage("A Magnifying Glass (Premium) has been used.", (short) 7));
							} else {
								c.getSession()
										.write(CField.getGameMessage(
												"A Magnifying Glass was not found. The equipment will stay as Hidden Potential.",
		...
		case 5062005: { // enlightening cube
			...
			if (item != null && c.getPlayer().getInventory(MapleInventoryType.USE).getNumFreeSlot() >= 1) {
				...
				if (eq.getState() >= 17) {
					...
					MapleInventoryManipulator.addById(c, 2430759, (short) 1,
							"Cube" + " on " + FileoutputUtil.CurrentReadable_Date());
					c.getSession().write(CField.enchantResult(tofind == 0 ? 1 : 1));// 3
					used = true;
				} else {
		...
		case 5062300: { // white awakening stamp
			...
			if (item != null) {
				...
				while (!rewarded) {
					...
					if (pot != null && pot.reqLevel / 10 <= reqLevel
							&& GameConstants.optionTypeFits(pot.optionType, eq.getItemId())
							&& GameConstants.potentialIDFits(pot.opID, new_state, 3)) { // optionType
		...
		case 5521000: { // Karma
			...
			if (item != null && !ItemFlag.KARMA_ACC.check(item.getFlag())
					&& !ItemFlag.KARMA_ACC_USE.check(item.getFlag()) && GameConstants.isEquip(item.getItemId())
		...
		case 5520000: { // Karma
			...
			if (item != null && !ItemFlag.KARMA_EQ.check(item.getFlag()) && !ItemFlag.KARMA_USE.check(item.getFlag())
					&& GameConstants.isEquip(item.getItemId()) && ((Equip) item).getKarmaCount() != 0) {
				Equip eq = (Equip) item;
		...
		case 5610000: { // Vega 30
			...
			used = UseUpgradeScroll(src, dst, (short) 2, c, c.getPlayer(), itemId, false); // cannot
																							// use
																							// ws
																							// with
																							// vega
		...
		case 5079002: {
			...
			if (!c.getChannelServer().getMegaphoneMuteState()) {
				...
				World.Broadcast
						.broadcastSmega(CWvsContext.broadcastMsg(24 + itemId % 10, c.getChannel(), sb.toString(), ear));
				used = true;
			} else {
				c.getPlayer().dropMessage(5, "The usage of Megaphone is currently disabled.");
			}
		...
		case 5075005: {
			...
			World.Broadcast.broadcastSmega(
					CWvsContext.broadcastMsg(3, c.getChannel(), c.getPlayer().getName() + " : " + message, ear));
			used = true;
			break;
		}
		...
		case 5700000: {
			...
			if (MapleCharacterUtil.canChangePetName(nName)) {
				...
				c.getPlayer().setAndroid(c.getPlayer().getAndroid()); // respawn
																		// it
				used = true;
			}
			break;
		...
		default:
			if (itemId / 10000 == 524 || itemId / 10000 == 546) { // Pet food &
																	// snacks
			...
			if (itemId / 10000 == 512) {
				...
				if (mesars > 0 && c.getPlayer().getMeso() < (Integer.MAX_VALUE - mesars)) {
					...
					if (Math.random() > 0.1) {
						...
						c.getSession().write(CSPacket.sendMesobagFailed(false)); // not
																					// random
					}
				}
			...
			if (mapitem.getMeso() > 0) {
				...
				if (MapleItemInformationProvider.getInstance().isPickupBlocked(mapitem.getItemId())) {
					...
					if (!GameConstants.isPet(mapitem.getItemId())) {
						...
						MapleInventoryManipulator.addById(c, mapitem.getItemId(), (short) 1, "",
								MaplePet.createPet(mapitem.getItemId(),
										MapleItemInformationProvider.getInstance().getName(mapitem.getItemId()), 1, 0,
										100, MapleInventoryIdentifier.getInstance(), 0, (short) 0),
								90, false, null);
	...
	/*
	 * public static final boolean UseTeleRock(LittleEndianAccessor slea,
	 * MapleClient c, int itemId) { boolean used = false; if (itemId == 5041001
	 * || itemId == 5040004) { slea.readByte(); //useless } if (slea.readByte()
	 * == 0) { // Rocktype final MapleMap target =
	 * c.getChannelServer().getMapFactory().getMap(slea.readInt()); if ((itemId
	 * == 5041000 && c.getPlayer().isRockMap(target.getId())) || (itemId !=
	 * 5041000 && c.getPlayer().isRegRockMap(target.getId())) || ((itemId ==
	 * 5040004 || itemId == 5041001) &&
	 * (c.getPlayer().isHyperRockMap(target.getId()) ||
	 * GameConstants.isHyperTeleMap(target.getId())))) { if
	 * (!FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit()) &&
	 * !FieldLimitType.VipRock.check(target.getFieldLimit()) &&
	 * !c.getPlayer().isInBlockedMap()) { //Makes sure this map doesn't have a
	 * forced return map c.getPlayer().changeMap(target, target.getPortal(0));
	 * used = true; } } } else { c.getPlayer().dropMessage(1,
	 * "You cannot go to that place."); final MapleCharacter victim =
	 * c.getChannelServer().getPlayerStorage().getCharacterByName(slea.
	 * readMapleAsciiString()); if (victim != null && !victim.isIntern() &&
	 * c.getPlayer().getEventInstance() == null && victim.getEventInstance() ==
	 * null) { if
	 * (!FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit()) &&
	 * !FieldLimitType.VipRock.check(c.getChannelServer().getMapFactory().getMap
	 * (victim.getMapId()).getFieldLimit()) && !victim.isInBlockedMap() &&
	 * !c.getPlayer().isInBlockedMap()) { if (itemId == 5041000 || itemId ==
	 * 5040004 || itemId == 5041001 || (victim.getMapId() / 100000000) ==
	 * (c.getPlayer().getMapId() / 100000000)) { // Viprock or same continent
	 * c.getPlayer().changeMap(victim.getMap(),
	 * victim.getMap().findClosestPortal(victim.getTruePosition())); used =
	 * true; } } } } return used && itemId != 5041001 && itemId != 5040004; }
	 */
	...
	public static boolean checkPotentialLock(MapleCharacter chr, Equip eq, int line, int potential) {
		...
		if (line < 0 || line > 3) {
			System.out.println("[Hacking Attempt] " + MapleCharacterUtil.makeMapleReadable(chr.getName())
					+ " Tried to lock potential line which does not exists.");
			return false;
		}
		if (line == 1 && eq.getPotential1() != potential - line * 100000
			...
			System.out.println("[Hacking Attempt] " + MapleCharacterUtil.makeMapleReadable(chr.getName())
					+ " Tried to lock potential which equip doesn't have.");
			return false;
		}
		return true;
	}
	...
	public static final boolean UseTeleRock(LittleEndianAccessor slea, MapleClient c, int itemId) {
		...
		if ((itemId == 5040004) || itemId == 5041001) {
			if (slea.readByte() == 0) {
				...
				if (target != null) { // Premium and Hyper rocks are allowed to
										// go anywhere. Blocked maps are checked
										// below.
					if (!FieldLimitType.VipRock.check(c.getPlayer().getMap().getFieldLimit())
							&& !FieldLimitType.VipRock.check(target.getFieldLimit())
							&& !c.getPlayer().isInBlockedMap()) { // Makes sure
																	// this map
																	// doesn't
																	// have a
																	// forced
				...
				if (victim != null && !victim.isIntern() && c.getPlayer().getEventInstance() == null
					...
					if (victim == null) {
						...
						c.getPlayer().dropMessage(1, "(" + name
								+ ") is currently difficult to locate, so the teleport will not take place.");
					}
				}
			}
		} else {
