...
public class PackageHandler {
	...
	public static void handleAction(final LittleEndianAccessor slea, final MapleClient c) {
			...
			if (mesos >= 0 && mesos <= 100000000 && c.getPlayer().getMeso() >= finalcost) {
				...
				if (accid != -1) {
					if (accid != c.getAccID()) {
						...
						try {
							// int channel =
							// c.getChannelServer().getWorldInterface().find(recipient);
							int channel = c.getChannelServer().getPlayerStorage().getCharacterByName(recipient)
						...
						c.getSession().write(CField.sendPackageMSG((byte) 15, null)); // Same
																						// acc
																						// error
					}
				} else {
					c.getSession().write(CField.sendPackageMSG((byte) 14, null)); // Name
																					// does
																					// not
																					// exist
				}
			} else {
				c.getSession().write(CField.sendPackageMSG((byte) 12, null)); // Not
																				// enough
																				// mesos
			}
			break;
		...
		case 5: { // Recieve Package
			...
			if (dp.getItem() != null && !MapleInventoryManipulator.checkSpace(c, dp.getItem().getItemId(),
				...
				c.getSession().write(CField.sendPackageMSG((byte) 16, null)); // Not
																				// enough
																				// Space
				return;
	...
	private static boolean addItemToDB(final Item item, final int quantity, final int mesos, final String sName,
		...
		try {
			try (PreparedStatement ps = con.prepareStatement(
				...
				try (ResultSet rs = ps.getGeneratedKeys()) {
					if (rs.next()) {
						ItemLoader.PACKAGE.saveItems(
								Collections.singletonList(
										new Pair<>(item, GameConstants.getInventoryType(item.getItemId()))),
								rs.getInt(1));
					}
				}
			}
