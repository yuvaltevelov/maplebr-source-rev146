...
public class ShopCreator {
	...
	public static void main(String[] args) throws FileNotFoundException, IOException {
		...
		while (!"done".equals(input.next())) {
			if (position >= 250) {
				System.out
						.println("Warning: you better stop adding items, as too many items will cause lags and crash.");
			}
			System.out.println("Please enter the next values: {item id, price}");
			sb.append("\r\nINSERT INTO shopitems (`shopid`, `itemid`, `price`, `position`) VALUES(").append(shopid)
					.append(",").append(input.next()).append(",").append(position).append(");");
			position++;
		}
