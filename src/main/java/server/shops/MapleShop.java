...
public class MapleShop {
	...
	public void buy(MapleClient c, short slot, int itemId, short quantity) {
		...
		if (index >= 0) {
			...
			int price = (int) Math.max(
					Math.ceil(ii.getPrice(itemId) * (GameConstants.isRechargable(itemId) ? 1 : i.getQuantity())), 0.0D);
		...
		if ((item != null) && (item.getPrice() > 0) && (item.getReqItem() == 0)) {
			if (item.getRank() >= 0) {
				...
				int y = 0;
				for (Pair i : getRanks()) {
					if ((c.getPlayer().haveItem(((Integer) i.left).intValue(), 1, true, true))
							&& (item.getRank() >= y)) {
						passed = true;
						break;
					}
			...
			if ((price >= 0) && (c.getPlayer().getMeso() >= price)) {
				if (MapleInventoryManipulator.checkSpace(c, itemId, quantity, "")) {
					...
					if (GameConstants.isPet(itemId)) {
						MapleInventoryManipulator.addById(c, itemId, (short) (quantity * item.getQuantity()), "",
								MaplePet.createPet(itemId, MapleInventoryIdentifier.getInstance()), -1, false,
						...
						MapleInventoryManipulator.addById(c, itemId, quantity, "Bought from shop " + this.id + ", "
								+ this.npcId + " on " + FileoutputUtil.CurrentReadable_Date());
					}
				} else {
					c.getPlayer().dropMessage(1, "Your Inventory is full");
				}
	...
	public void recharge(MapleClient c, byte slot) {
		...
		if (item.getQuantity() < slotMax) {
			...
			if (c.getPlayer().getMeso() >= price) {
				...
				c.getSession()
						.write(CWvsContext.InventoryPacket.updateInventorySlot(MapleInventoryType.USE, item, false));
			...
			while (rs.next()) {
				if (ii.itemExists(rs.getInt("itemid"))) {
					if ((GameConstants.isThrowingStar(rs.getInt("itemid")))
						...
						MapleShopItem starItem = new MapleShopItem((short) rs.getShort("buyable"),
						...
						ret.addItem(new MapleShopItem((short) rs.getShort("buyable"), rs.getShort("quantity"),
								...
								rs.getInt("minLevel"), rs.getInt("expiration"), false)); // todo
																							// potential
					}
				}
			}
			for (Integer recharge : recharges) {
				ret.addItem(new MapleShopItem((short) 1, ii.getSlotMax(recharge.intValue()), recharge.intValue(), 0,
						(short) 0, 0, 0, (byte) 0, 0, 0, 0, false));
			}
			rs.close();
			ps.close();
	...
	public static MapleShop moonlightShop(int npc) { // No ranks/rechargable
														// items for now
		MapleShop moonlight = new MapleShop(MoonlightRevamp.shopId, npc);
		for (MoonlightShop item : MoonlightShop.values()) {
			moonlight.addItem(new MapleShopItem((short) item.getBuyable(), (short) 1, item.getItemId(), item.getPrice(),
					(short) 0, item.getReqItem(), item.getReqItemQ(), item.getRank(), item.getCategory(),
					item.getMinLevel(), item.getExpiration(), false));
		}
		return moonlight;
	}
