...
public enum MapleStatInfo {
	...
	cr(0), // Critical %
	actionSpeed(0), criticaldamageMax(0), // Critical Maximum Damage Increase +
	...
	MDF(0), powerCon(0), // Surplus Energy Cost
	...
	indieBDR(0), indieAcc(0), // Accuracy +
	...
	indiePdd(0), // ??
	indieMdd(0), // MDEF?
	indieTerR(0), indieAsrR(0), indieMhpR(0), // Max HP Increase %
	...
	itemConsume(0), // Uses certain item to cast that attack, the itemid doesn't
					// need to be in inventory, just the effect.
	...
	mdd2pdx(0), // When hit with a physical attack, damage equal to #mdd2pdx% of
				// Magic DEF is ignored
	mddR(0), // Magic DEF %
	mddX(0), // Magic DEF
	...
	nbdR(0), // Increases damage by a set percentage when attacking a normal
				// monster.
	nocoolProp(0), // When using a skill, Cooldown is not applied at
					// #nocoolProp% chance. Has no effect on skills without
					// Cooldown.
	...
	pdd2mdx(0), // When hit with a magical attack, damage equal to #pdd2mdx% of
				// Weapon DEF is ignored
	pddR(0), // Weapon DEF %
	pddX(0), // Weapon DEF
