...
public class MaplePortal {
	...
	public final void enterPortal(final MapleClient c) {
		...
		if (!c.getPlayer().hasBlockedInventory() && (portalState || c.getPlayer().isGM())) {
			if (getScriptName() != null) {
				...
				try {
					...
					System.out.println("Error while entering portal: " + e.getMessage());
				}
			} else if (getTargetMapId() != 999999999) {
				final MapleMap oldto = ChannelServer.getInstance(c.getChannel()).getMapFactory()
						.getMap(getTargetMapId());
				final MapleMap to = ChannelServer.getInstance(c.getChannel()).getMapFactory()
						.getMap(GameConstants.getSpecialMapTarget(getTargetMapId()));
				if (to == null) {
					c.getSession().write(CWvsContext.enableActions());
					return;
				}
				...
				c.getPlayer().changeMapPortal(to,
						to.getPortal(GameConstants.getSpecialPortalTarget(oldto.getId(), getTarget())) == null
								? to.getPortal(0)
								: to.getPortal(GameConstants.getSpecialPortalTarget(oldto.getId(), getTarget()))); // late
																													// resolving
																													// makes
																													// this
																													// harder
		...
		if (c != null && c.getPlayer() != null && c.getPlayer().getMap() == currentmap) { // Character
																							// is
																							// still
																							// on
																							// the
