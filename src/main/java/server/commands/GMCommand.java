...
public class GMCommand {
	...
	public static class KillNear extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			List<MapleMapObject> players = map.getMapObjectsInRange(c.getPlayer().getPosition(), (double) 25000,
		...
		public int execute(MapleClient c, String[] splitted) {
			if (c.getChannelServer().manualEvent(c.getPlayer())) {
				for (MapleCharacter chrs : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
					// chrs.dropMessage(0, "MapleGM is hosting an event! Use the
					// @joinevent command to join the event!");
					// chrs.dropMessage(0, "Event Map: " +
					// c.getPlayer().getMap().getMapName());
					// World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(25,
					// 0, "MapleGM is hosting an event! Use the @joinevent
					// command to join the event!"));
					// World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(26,
					// 0, "Event Map: " + c.getPlayer().getMap().getMapName()));
					chrs.getClient().getSession()
							.write(CWvsContext.broadcastMsg(
									GameConstants.isEventMap(chrs.getMapId()) ? 0 : 25, c
											.getChannel(),
							"Event : MapleGM is hosting an event! Use the @joinevent command to join the event!"));
					chrs.getClient().getSession()
							.write(CWvsContext.broadcastMsg(GameConstants.isEventMap(chrs.getMapId()) ? 0 : 26,
									c.getChannel(), "Event : Event Channel: " + c.getChannel() + " Event Map: "
											+ c.getPlayer().getMap().getMapName()));
				}
			} else {
				for (MapleCharacter chrs : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
					// World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(22,
					// 0, "Enteries to the GM event are closed. The event has
					// began!"));
					chrs.getClient().getSession()
							.write(CWvsContext.broadcastMsg(GameConstants.isEventMap(chrs.getMapId()) ? 0 : 22,
									c.getChannel(),
					...
					if (ch < 0) {
						...
						if (splitted.length > 2) {
							try {
								...
								// noop, assume the gm didn't know how many
								// portals there are
								c.getPlayer().dropMessage(5, "Invalid portal selected.");
							} catch (NumberFormatException a) {
								// noop, assume that the gm is drunk
	...
	public static class TempBan extends CommandExecute {
		...
		private final String[] types = { "HACK", "BOT", "AD", "HARASS", "CURSE", "SCAM", "MISCONDUCT", "SELL", "ICASH",
				"TEMP", "GM", "IPROGRAM", "MEGAPHONE" };
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			victim.tempban("Temp banned by " + c.getPlayer().getName() + " for " + types[reason] + " reason", cal,
					reason, ipBan);
		...
		public int execute(MapleClient c, String[] splitted) {
			if (splitted.length < 3) {
				...
				c.getPlayer().dropMessage(5,
						"If you want to consider this ban as an autoban, set the reason \"AutoBan\"");
				return 0;
			}
			StringBuilder sb = new StringBuilder();
			if (hellban) {
				sb/* .append("Banned ").append(splitted[1]).append(": ") */.append(
						StringUtil.joinStringFrom(splitted, 2));
			} else {
				sb/*
					 * .append(c.getPlayer().getName()).append(" banned "
					 * ).append(splitted[1]).append(": ")
			...
			if (target != null) {
				if ((c.getPlayer().getGMLevel() > target.getGMLevel() || c.getPlayer().isAdmin())
					...
					// sb.append(" (IP:
					// ").append(target.getClient().getSessionIPAddress()).append(")");
				...
				if (MapleCharacter.ban(splitted[1], sb.toString(), false,
						c.getPlayer().isAdmin() ? 250 : c.getPlayer().getGMLevel(), hellban)) {
					c.getPlayer().dropMessage(6,
		...
		public int execute(MapleClient c, String[] splitted) {
			if (splitted.length < 6) {
				c.getPlayer().dropMessage(0,
						splitted[0] + " <character name> <petid> <petname> <petlevel> <petcloseness> <petfullness>");
				return 0;
			}
			...
			try {
				MapleInventoryManipulator.addById(petowner.getClient(), id, (short) 1,
						"", MaplePet.createPet(id, name, level, closeness, fullness,
								MapleInventoryIdentifier.getInstance(), id == 5000054 ? (int) period : 0, flags),
						45, false, null);
	...
	public static class SP2 extends CommandExecute {
		...
		@Override
		public int execute(MapleClient c, String[] splitted) {
			c.getPlayer().setRemainingSp(CommandProcessorUtil.getOptionalIntArg(splitted, 2, 1),
	...
	public static class LevelUpTill extends CommandExecute {
		...
		@Override
		public int execute(MapleClient c, String[] splitted) {
			// for (int i = 0; i < Integer.parseInt(splitted[1]) -
			// c.getPlayer().getLevel(); i++) {
	...
	public static class LevelUpPersonTill extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			MapleCharacter victim = c.getChannelServer().getPlayerStorage().getCharacterByName(splitted[1]);
			// for (int i = 0; i < Integer.parseInt(splitted[2]) -
			// victim.getLevel(); i++) {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			for (Item item : chr.getInventory(type).listById(itemid)) {
				...
				chr.getClient().getSession().write(
						InventoryPacket.updateSpecialItemUse(item, type.getType(), item.getPosition(), true, chr));
			}
			if (type == MapleInventoryType.EQUIP) {
				type = MapleInventoryType.EQUIPPED;
				for (Item item : chr.getInventory(type).listById(itemid)) {
					item.setFlag((byte) (item.getFlag() | ItemFlag.LOCK.getValue()));
					// chr.getClient().getSession().write(CField.updateSpecialItemUse(item,
					// type.getType()));
				}
			}
			c.getPlayer().dropMessage(6, "All items with the ID " + splitted[2]
	...
	public static class Smega extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			World.Broadcast
					.broadcastSmega(
							CWvsContext.broadcastMsg(3,
									c.getPlayer() == null ? c.getChannel() : c.getPlayer().getClient().getChannel(),
									c.getPlayer() == null ? c.getPlayer().getName()
											: c.getPlayer().getName() + " : " + StringUtil.joinStringFrom(splitted, 1),
									true));
			/*
			 * if (splitted.length < 2) { c.getPlayer().dropMessage(0,
			 * "!smega <itemid> <message>"); return 0; } final List<String>
			 * lines = new LinkedList<>(); for (int i = 0; i < 4; i++) { final
			 * String text = StringUtil.joinStringFrom(splitted, 2); if
			 * (text.length() > 55) { continue; } lines.add(text); } final
			 * boolean ear = true;
			 * World.Broadcast.broadcastSmega(CWvsContext.getAvatarMega(c.
			 * getPlayer(), c.getChannel(), Integer.parseInt(splitted[1]),
			 * lines, ear));
			 */
			return 1;
		}
	...
	public static class SpeakMega extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			World.Broadcast.broadcastSmega(CWvsContext.broadcastMsg(3, victim.getClient().getChannel(),
					victim.getName() + " : " + StringUtil.joinStringFrom(splitted, 2), true));
			/*
			 * if (splitted.length < 2) { c.getPlayer().dropMessage(0,
			 * "!smega <itemid> <victim> <message>"); return 0; } final
			 * List<String> lines = new LinkedList<>(); for (int i = 0; i < 4;
			 * i++) { final String text = StringUtil.joinStringFrom(splitted,
			 * 3); if (text.length() > 55) { continue; } lines.add(text); }
			 * final boolean ear = true;
			 * World.Broadcast.broadcastSmega(CWvsContext.getAvatarMega(victim,
			 * victim.getClient().getChannel(), Integer.parseInt(splitted[1]),
			 * lines, ear));
			 */
			return 1;
		}
		...
		public int execute(MapleClient c, String[] splitted) {
			for (MapleCharacter mch : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
				if (mch == null) {
					...
					mch.getMap().broadcastMessage(
							CField.getChatText(mch.getId(), StringUtil.joinStringFrom(splitted, 1), mch.isGM(), 0));
				}
			}
			return 1;
		}
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			if (victim == null) {
				...
				victim.getMap().broadcastMessage(
						CField.getChatText(victim.getId(), StringUtil.joinStringFrom(splitted, 2), victim.isGM(), 0));
			}
			return 1;
		}
	}
	...
	public static class gmtext extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			if (splitted[1].equalsIgnoreCase("normal")) {
				...
				c.getPlayer().dropMessage(6,
						"Wrong syntax: use !gmtext normal/orange/pink/purple/green/blue/red/whitebg/lightinggreen/yellow/spouse");
				return 1;
			}
			try {
	...
	public static class DiseaseMap extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			if (splitted.length < 3) {
				c.getPlayer().dropMessage(6,
						"!disease <type> <level> where type = SEAL/DARKNESS/WEAKEN/STUN/CURSE/POISON/SLOW/SEDUCE/REVERSE/ZOMBIFY/POTION/SHADOW/BLIND/FREEZE/POTENTIAL");
				return 0;
			}
			int type;
			if (splitted[1].equalsIgnoreCase("SEAL")) {
				...
				type = 126;
			} else if (splitted[1].equalsIgnoreCase("SEDUCE")) { // 24, 289 and
																	// 29 are
																	// cool.
				type = 128;
				...
				c.getPlayer().dropMessage(6,
						"!disease <type> <level> where type = SEAL/DARKNESS/WEAKEN/STUN/CURSE/POISON/SLOW/SEDUCE/REVERSE/ZOMBIFY/POTION/SHADOW/BLIND/FREEZE/POTENTIAL/SLOW2/TORNADO/FLAG");
				return 0;
			}
			for (MapleCharacter mch : c.getChannelServer().getPlayerStorage().getAllCharacters()) {
	...
	public static class Disease extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			if (splitted.length < 3) {
				c.getPlayer().dropMessage(6,
						"!disease <type> [charname] <level> where type = SEAL/DARKNESS/WEAKEN/STUN/CURSE/POISON/SLOW/SEDUCE/REVERSE/ZOMBIFY/POTION/SHADOW/BLIND/FREEZE/POTENTIAL");
				return 0;
			}
			int type;
			if (splitted[1].equalsIgnoreCase("SEAL")) {
				...
				c.getPlayer().dropMessage(6,
						"!disease <type> [charname] <level> where type = SEAL/DARKNESS/WEAKEN/STUN/CURSE/POISON/SLOW/SEDUCE/REVERSE/ZOMBIFY/POTION/SHADOW/BLIND/FREEZE/POTENTIAL/SLOW2/TORNADO/FLAG");
				return 0;
			}
			if (splitted.length == 4) {
	...
	public static class Jail extends CommandExecute {
		...
		public int execute(MapleClient c, String[] splitted) {
			...
			if (victim != null && c.getPlayer().getGMLevel() >= victim.getGMLevel()) {
				...
				victim.getQuestNAdd(MapleQuest.getInstance(GameConstants.JAIL_QUEST))
						.setCustomData(String.valueOf(minutes * 60));
	 ...
	 * 
	 * @Override public int execute(MapleClient c, String[] splitted) { // int
	 * clock = 1; MapleMonster mob = MapleLifeFactory.getMonster(891000);
	 * c.getSession().write(CField.spawnClockMist(this)); return 0; } }
	 */
