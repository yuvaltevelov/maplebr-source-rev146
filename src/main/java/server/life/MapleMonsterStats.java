...
public class MapleMonsterStats {
	...
	private byte cp, selfDestruction_action, tagColor, tagBgColor, rareItemDropLevel, HPDisplayType, summonType, PDRate,
			MDRate, category;
	private short level, PhysicalDefense, MagicDefense, charismaEXP;
	private long hp, finalmaxhp;
	private int id, exp, mp, removeAfter, buffToGive, fixedDamage, selfDestruction_hp, dropItemPeriod, point, eva, pad,
			mad, acc, PhysicalAttack, MagicAttack, speed, partyBonusR, pushed;
	private boolean boss, undead, ffaLoot, firstAttack, isExplosiveReward, mobile, fly, onlyNormalAttack, friendly,
			noDoom, invincible, partyBonusMob, changeableMob, escort;
	...
	public int dropsMeso() {
		if (getRemoveAfter() != 0 || isInvincible() || getOnlyNoramlAttack() || getDropItemPeriod() > 0 || getCP() > 0
				|| getPoint() > 0 || getFixedDamage() > 0 || getSelfD() != -1 || getPDRate() <= 0 || getMDRate() <= 0) {
			return 0;
		}
		...
		if (GameConstants.getPartyPlayHP(getId()) > 0 || mobId == 97 || mobId == 95 || mobId == 93 || mobId == 91
				|| mobId == 90) {
			return 0;
		}
