...
public class SummonHandler {
	...
	public static final void MoveDragon(final LittleEndianAccessor slea, final MapleCharacter chr) {// MIXTAMAL6
																									// +Mally
						...
						public void run() {
							try {
								if (clone.getMap() == map && clone.getDragon() != null) {
									...
									if (!clone.isHidden()) {
										map.broadcastMessage(clone, CField.moveDragon(clone.getDragon(), startPos, res),
	...
	public static final void MoveSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
		...
		if (res.size() > 0) {
			chr.getMap().broadcastMessage(chr, SummonPacket.moveSummon(chr.getId(), sum.getObjectId(), pos, res),
					sum.getTruePosition());
		}
	}
	...
	public static final void DamageSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
		...
		try {
			while (iter.hasNext()) {
				summon = iter.next();
				if (summon.isPuppet() && summon.getOwnerId() == chr.getId() && damage > 0) { // We
																								// can
																								// only
																								// have
																								// one
	...
	public static void SummonAttack(final LittleEndianAccessor slea, final MapleClient c, final MapleCharacter chr) {
		...
		map.broadcastMessage(chr, SummonPacket.summonAttack(summon.getOwnerId(), summon.getObjectId(), animation,
				allDamage, chr.getLevel(), false), summon.getTruePosition());
		// }
		...
		for (Pair<Integer, Integer> attackEntry : allDamage) {
			...
			if (sse != null && sse.delay > 0 && summon.getMovementType() != SummonMovementType.STATIONARY
				...
				if (summonEffect.makeChanceResult()) {
					for (Map.Entry<MonsterStatus, Integer> z : summonEffect.getMonsterStati().entrySet()) {
						mob.applyStatus(chr,
								...
								summonEffect.isPoison(), 4000, true, summonEffect);
					}
				}
			}
			if (chr.isGM()
					|| toDamage < (chr.getStat().getCurrentMaxBaseDamage() * 5.0 * (summonEffect.getSelfDestruction()
				...
				// AutobanManager.getInstance().autoban(c, "High Summon Damage
				// (" + toDamage + " to " + attackEntry.right + ")");
	...
	public static final void RemoveSummon(final LittleEndianAccessor slea, final MapleClient c) {
		...
		if (summon.getSkill() == 35111002 || summon.getSkill() == 35121010) { // rock
																				// n
																				// shock,
																				// amp
			return;
	...
	public static final void SubSummon(final LittleEndianAccessor slea, final MapleCharacter chr) {
		...
		case 35111011: // healing
			...
			chr.getClient().getSession()
					.write(EffectPacket.showOwnBuffEffect(sum.getSkill(), 2, chr.getLevel(), sum.getSkillLevel()));
			chr.getMap().broadcastMessage(chr,
					EffectPacket.showBuffeffect(chr.getId(), sum.getSkill(), 2, chr.getLevel(), sum.getSkillLevel()),
					false);
			break;
		...
		case 1311013: // Evil Eye of Domination
			...
			chr.getMap().broadcastMessage(chr,
					EffectPacket.showBuffeffect(chr.getId(), sum.getSkill(), 2, chr.getLevel(), bHealingLvl), false);
			break;
		}
		if (GameConstants.isAngel(sum.getSkill())) {
			...
			chr.getMap().broadcastMessage(chr, EffectPacket.showBuffeffect(chr.getId(), sum.getSkill(), 2, 2, 1),
					false);
		}
	}
	...
	public static final void SummonPVP(final LittleEndianAccessor slea, final MapleClient c) {
		...
		for (MapleMapObject mo : chr.getMap().getCharactersIntersect(box)) {
			...
			if (attacked.getId() != chr.getId() && attacked.isAlive() && !attacked.isHidden()
				...
				double rawDamage = maxdamage / Math.max(0, ((magic ? attacked.getStat().mdef : attacked.getStat().wdef)
				...
				for (int i = 0; i < attackCount; i++) {
					...
					if (attacked.getStat().dodgeChance > 0
						...
						// } else if (attacked.getLevel() > chr.getLevel() &&
						// Randomizer.nextInt(100) < (attacked.getLevel() -
						// chr.getLevel())) {
						// ourDamage = 0;
					}
					if (attacked.getBuffedValue(MapleBuffStat.MAGIC_GUARD) != null) {
						mploss = (int) Math.min(attacked.getStat().getMp(),
								(ourDamage * attacked.getBuffedValue(MapleBuffStat.MAGIC_GUARD).doubleValue() / 100.0));
					}
					ourDamage -= mploss;
				...
				chr.getClient().getSession().write(CField.getPVPHPBar(attacked.getId(), attacked.getStat().getHp(),
						attacked.getStat().getCurrentMaxHp()));
				addedScore += (totalHPLoss / 100) + (totalMPLoss / 100); // ive
																			// NO
																			// idea
				if (!attacked.isAlive()) {
					killed = true;
