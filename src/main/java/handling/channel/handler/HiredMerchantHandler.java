...
public class HiredMerchantHandler {
	...
	public static void displayMerch(MapleClient c) {
		...
		if (merch) {
			...
			if (pack == null) {
				c.getSession()
						.write(NPCPacket.getNPCTalk(9030000, (byte) 0,
								...
								"00 00", (byte) 0));
				c.getPlayer().setConversation(0);
			} else if (pack.getItems().size() <= 0) { // error fix for
														// complainers.
				...
				if (deletePackage(c.getPlayer().getAccountID(), pack.getPackageid(), c.getPlayer().getId())) {
					...
					c.getSession().write(NPCPacket.getNPCTalk(9030000, (byte) 0,
							"I see that you forgot something here right?\r\nHere is your money sir " + pack.getMesos(),
							"00 00", (byte) 0));
					c.getPlayer().setConversation(0);
				} else {
					c.getPlayer().dropMessage(1, "An unknown error occured.");
				...
				for (final Item item : pack.getItems()) {
					if (c.getPlayer().getInventory(GameConstants.getInventoryType(item.getItemId())).isFull()) {
						c.getSession()
								.write(NPCPacket.getNPCTalk(9030000, (byte) 0,
										"Sir, if you want your items back please clean up your inventory before you come here!",
										"00 00", (byte) 0));
						c.getPlayer().setConversation(0);
						break;
					}
					...
					c.getSession()
							.write(NPCPacket.getNPCTalk(9030000, (byte) 0,
									"I saved your items sir, next time don't forget them, have a nice day.", "00 00",
									(byte) 0));
					c.getPlayer().setConversation(0);
				}
	...
	public static void displayMerch2(MapleClient c) {
		...
		if (merch) {
			...
			if (pack == null) {
				c.getSession()
						.write(NPCPacket.getNPCTalk(9030000, (byte) 0,
								"I don't think you have any items or money to retrive here. This is where you retrieve the items and mesos that you couldn't get from your Hired Merchant. You'll also need to see me as the character that opened the Personal Store.",
								"00 00", (byte) 0));
				c.getPlayer().setConversation(0);
			} else if (pack.getItems().size() <= 0) { // error fix for
														// complainers.
				...
				for (final Item item : pack.getItems()) {
					if (c.getPlayer().getInventory(GameConstants.getInventoryType(item.getItemId())).isFull()) {
						c.getSession().write(NPCPacket.getNPCTalk(9030000, (byte) 0, "Please clean up your inventory.",
								"00 00", (byte) 0));
						c.getPlayer().setConversation(0);
						break;
					}
					...
					deletePackage(c.getPlayer().getAccountID(), pack.getPackageid(), c.getPlayer().getId());
					// c.getPlayer().fakeRelog();
					c.getSession().write(NPCPacket.getNPCTalk(9030000, (byte) 0, "Your items have been claimed.",
							"00 00", (byte) 0));
					c.getPlayer().setConversation(0);
				}
	...
	private static void requestItems(final MapleClient c, final boolean request) {
		...
		final int days = StringUtil.getDaysAmount(pack.getSavedTime(), System.currentTimeMillis()); // max
																									// 100%
		final double percentage = days / 100.0;
		final int fee = (int) Math.ceil(percentage * pack.getMesos()); // if no
																		// mesos
																		// = no
																		// tax
