...
public class MapleClient implements Serializable {
	...
	public final List<MapleCharacter> loadCharacters(final int serverId) { // TODO
		...
		for (final CharNameAndId cni : loadCharactersInternal(serverId)) {
			...
			chars.add(chr);
			charInfo.put(chr.getId(), new Pair<>(chr.getLevel(), chr.getJob())); // to
																					// be
																					// used
																					// to
																					// update
		...
		try {
			...
			try (PreparedStatement psu = con.prepareStatement(
				...
				for (final Entry<Integer, Integer> ii : cids.entrySet()) {
					final Pair<Short, Short> info = charInfo.get(ii.getValue()); // charinfo
																					// we
																					// can
																					// use
																					// here
					...
					psu.setInt(4, ii.getKey()); // position shouldn't matter
												// much, will reset upon login
					...
					if (loginstate > MapleClient.LOGIN_NOTLOGGEDIN) { // already
						...
						if (pwd.equalsIgnoreCase("fixme") || pwd.equalsIgnoreCase("unstuckme")) {
							try {
								...
								getSession()
										.write(CWvsContext.broadcastMsg(1, "Account has been successfully unstuck!"));
	...
	public final int deleteCharacter(final int cid) {
		try {
			...
			// MapleCharacter.deleteWhereCharacterId(con, "DELETE FROM cheatlog
	...
	public MapleFarm getFarm() {
		if (farm == null) {
			...
			// MapleFarm farm2 = MapleFarm.getDefault(35549721, this,
			// "AstralMS");
			// farm2.setLevel(1);
			// return farm2;
		}
