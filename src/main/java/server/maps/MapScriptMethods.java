...
public class MapScriptMethods {
	...
	public static void startScript_FirstUser(final MapleClient c, String scriptName) {
		...
		case Ranmaru_Before: {
			if (c.getPlayer().getMap().getMobsSize() == 0) {
				c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9421581),
						new Point(109, 123));
				break;
			}
		}
		case pierre_Summon: {
			if (c.getPlayer().getMap().getMobsSize() == 0) {
				c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8900000),
						new Point(497, 551));
				c.getPlayer().getMap().startMapEffect("From the bottom of my heart, welcome to the tea party!",
						5120098);
		...
		case queen_summon0: {
			...
			if (c.getPlayer().getMap().getMobsSize() == 0) {
				...
				// c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8920000),
				// new Point(4, 135));
				break;
			}
		}
		...
		case sao_enterF01: {
			...
			c.getPlayer().getMap().resetFully();
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390706),
					new Point(-615, 336));
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390706),
					new Point(-364, 336));
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390706), new Point(78, 336));
			// c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(2600208),
			// new Point(736, 336));
			...
			oms.setOHp((long) Math.ceil(theMob.getMobMaxHp() * 15.0)); // 10k to
																		// 4m
		...
		case sao_enterF02: {
			...
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390700),
					new Point(-454, 336));
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390700),
					new Point(-119, 336));
		...
		case sao_enterF03: {
			...
			c.getPlayer().getMap().resetFully();
			// c.getPlayer().getMap().startMapEffect("Floor 2.", 5120107);
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390701),
					new Point(-593, 336));
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390701),
					new Point(-205, 336));
		...
		case sao_enterF04: {
			...
			c.getPlayer().getMap().resetFully();
			// c.getPlayer().getMap().startMapEffect("Floor 2.", 5120107);
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390702),
					new Point(-631, -204));
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390702),
					new Point(-14, -204));
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9390702),
		...
		case sao_enterF05: {
			...
			oms.setOMp(theMob.getMobMaxMp());
			oms.setOExp(theMob.getMobExp() * 0);
			oms.setOHp((long) Math.ceil(theMob.getMobMaxHp() * 1500.0)); // 10k
																			// to
																			// 4m
			theMob.setOverrideStats(oms);
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(theMob, new Point(-12, 336));
		...
		case sao_enterF06: {
			...
			oms.setOMp(theMob.getMobMaxMp());
			oms.setOExp(theMob.getMobExp() * 0);
			oms.setOHp((long) Math.ceil(theMob.getMobMaxHp() * 25.0)); // 10k to
																		// 4m
			theMob.setOverrideStats(oms);
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(theMob, new Point(736, 336));
			break;
		...
		case sao_enterF07: {
			...
			oms.setOMp(theMob.getMobMaxMp());
			oms.setOExp(theMob.getMobExp() * 0);
			oms.setOHp((long) Math.ceil(theMob.getMobMaxHp() * 3.0)); // 10k to
																		// 4m
			theMob.setOverrideStats(oms);
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(theMob, new Point(1225, 51));
			break;
		...
		case sao_enterF08: {
			...
			oms.setOMp(theMob.getMobMaxMp());
			oms.setOExp(theMob.getMobExp() * 0);
			oms.setOHp((long) Math.ceil(theMob.getMobMaxHp() * 5.0)); // 10k to
																		// 4m
			theMob.setOverrideStats(oms);
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(theMob, new Point(-430, -410));
			break;
		...
		case sao_enterF10: {
			...
			oms.setOMp(theMob.getMobMaxMp());
			oms.setOExp(theMob.getMobExp() * 0);
			oms.setOHp((long) Math.ceil(theMob.getMobMaxHp() * 20.0)); // 10k to
																		// 4m
			theMob.setOverrideStats(oms);
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(theMob, new Point(-12, 336));
			break;
		...
		case abysscave_ent: {
			// c.getPlayer().getMap().resetFully();
			if (c.getPlayer().getMap().getMobsSize() == 0) {
				c.getPlayer().getMap().startMapEffect("Vellum is nowhere to be seen. Take a look around the altar",
						5120107);
				// c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8930000),
				// new Point(256, 443));
				break;
			}
		}
		case banban_Summon: {
			...
			if (c.getPlayer().getMap().getMobsSize() == 0) {
				...
				// c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8910000),
				// new Point(256, 443));
				break;
			}
		}
		...
		case StageMsg_davy: {
			...
			if (c.getPlayer().getMapId() == 925100500 && em != null && em.getProperty("stage5") != null) {
				int mobId = Randomizer.nextBoolean() ? 9300107 : 9300119; // lord
																			// pirate
		...
		case boss_Ravana: { // event handles this so nothing for now until i
							// find out something to do with it
		...
		case shammos_Fenter: {
			if (c.getPlayer().getMapId() >= 921120100 && c.getPlayer().getMapId() < 921120300) {
				 ...
				 * (c.getPlayer().getEventInstance() != null) { int averageLevel
				 * = 0, size = 0; for (MapleCharacter pl :
				 * c.getPlayer().getEventInstance().getPlayers()) { averageLevel
				 * += pl.getLevel(); size++; } if (size <= 0) { return; }
				 * averageLevel /= size; shammos.changeLevel(Math.max(120,
				 * Math.min(200, averageLevel))); }
				 * c.getPlayer().getMap().spawnMonsterOnGroundBelow(shammos, new
				 * Point(350, 170));
				 */
			}
			break;
		}
			// 5120038 = dr bing. 5120039 = visitor lady. 5120041 = unknown dr
			// bing.
		case iceman_FEnter: {
		...
		case magnus_summon: {
			...
			c.getPlayer().getMap().resetFully();
			// c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(8880000),
			// c.getPlayer().getPosition());
			break;
		}
		default: {
	...
	public static void startScript_User(final MapleClient c, String scriptName) {
		...
		case direction_59061: {
			if (c.getPlayer().getQuestStatus(59061) == 1) {
				...
				// MapleQuest.getInstance(59063).forceStart(c.getPlayer(), 0,
				// null);
				// MapleMap mapto =
				// c.getChannelServer().getMapFactory().getMap(866000230);
				// c.getPlayer().changeMap(mapto, mapto.getPortal(0));
			}
			// c.getSession().write(UIPacket.getDirectionStatus(true));
			// c.getSession().write(UIPacket.getDirectionInfo(3, 0));
		...
		case onUserEnter_866135000: {
			...
			if (c.getPlayer().haveItem(2500004, 0)) {
				c.getPlayer().gainItem(2500004, 1);// Animal SP Reset Scroll
													// (Beast Tamer only)
		...
		case Advanture_tuto00: {
			try {
				...
				Thread.sleep(30);
				c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction3.img/effect/tuto/BalloonMsg0/0", 2100,
						0, -120, 0, 0));
				...
				Thread.sleep(420);
				c.getSession().write(UIPacket.getDirectionInfo(3, 0));
				c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction12.img/effect/tuto/BalloonMsg0/1", 2100,
						0, -120, 0, 0));
				c.getSession().write(UIPacket.getDirectionInfo(1, 1800));
				Thread.sleep(2800);
				c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction3.img/effect/tuto/BalloonMsg0/1", 2100,
						0, -120, 0, 0));
				c.getSession().write(UIPacket.getDirectionInfo(1, 2100));
				Thread.sleep(2800);
				c.getSession().write(
						UIPacket.getDirectionEffect("Effect/Direction3.img/effect/tuto/key/0", 3000000, -300, 0));
				c.getSession()
						.write(UIPacket.getDirectionEffect("Effect/Direction3.img/effect/tuto/key/0", 3000000, 0, 0));
				c.getSession()
						.write(UIPacket.getDirectionEffect("Effect/Direction3.img/effect/tuto/key/0", 3000000, 300, 0));
				c.getSession().write(
						UIPacket.getDirectionEffect("Effect/Direction3.img/effect/tuto/key/1", 3000000, 540, 70));
				c.getSession().write(UIPacket.getDirectionInfo(1, 1200));
				Thread.sleep(1800);
				c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction3.img/effect/tuto/BalloonMsg0/2", 2100,
						0, -120, 0, 0));
				c.getSession().write(UIPacket.getDirectionInfo(1, 2100));
				Thread.sleep(2800);
				...
				Thread.sleep(3000);
				c.getSession().write(CWvsContext
						.getTopMsg("Go to where the portal is and press the up key to move to the next map."));

				// MapleQuest.getInstance(32200).forceStart(c.getPlayer(), 0,
				// null);
				// MapleQuest.getInstance(32200).forceComplete(c.getPlayer(),
				// 0);
				// Thread.sleep(6000);
			} catch (InterruptedException e) {
			}
		...
		case Advanture_tuto01: {
			try {
				...
				Thread.sleep(3000);
				c.getSession().write(UIPacket.getDirectionStatus(true));
				c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction3.img/effect/tuto/BalloonMsg0/3", 2100,
						0, -120, 0, 0));
				c.getSession().write(UIPacket.getDirectionInfo(1, 1800));
				Thread.sleep(1800);
		...
		case Advanture_tuto10: {
			try {
				...
				Thread.sleep(2100);
				c.getSession().write(
						UIPacket.getDirectionEffect("Effect/Direction3.img/effect/tuto/key/2", 3000000, -520, -740));
		...
		case Advanture_tuto11:
			...
			// Thread.sleep(1000);
			// c.getSession().write(UIPacket.getDirectionStatus(true));
			// c.getSession().write(NPCPacket.setNPCSpecialAction(814124,
			// "summon"));
			// c.getSession().write(UIPacket.getUnknownDirectionInfo("Effect/Direction12.img/effect/tuto/BalloonMsg1/1",
			// 900, 0, -120, 814124));//improve
			// c.getSession().write(UIPacket.getDirectionInfo(1, 1800));
			// Thread.sleep(1800);
			// c.getSession().write(NPCPacket.NPCSpecialAction(814124, 1,
			// 1000));
			// c.getSession().write(UIPacket.getDirectionInfoNew((byte) 0,
			// 200));
			// c.getSession().write(UIPacket.getDirectionInfo(1, 4542));
			// Thread.sleep(4542);
			// c.getSession().write(UIPacket.getDirectionInfoNew((byte) 1,
			// 300));
		...
		case sao_enterRewardMap: {
			c.getSession().write(CField.showEffect("Sao/Start1"));
			// c.getPlayer().getMap().startMapEffect("Attempt to wake the
			// Crimson Queen", 5120107);
			break;
		}
		...
		case root_secret: {
			...
			/*
			 * case root_ereb00: { if (c.getPlayer().getQuestStatus(30000) == 1)
			 * { c.getSession().write(CField.UIPacket.IntroEnableUI(1));
			 * c.getSession().write(CField.UIPacket.IntroDisableUI(true)); if
			 * (!c.getPlayer().getMap().containsNPC(1064026)) {
			 ...
			 * c.getPlayer().getMap().removeNpc(1064026); } break; }
			 */
		...
		case cannon_tuto_direction1: {
			...
			c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/cannonshooter/balloon/0", 5000,
					0, 0, 1, 0));
			c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/cannonshooter/balloon/1", 5000,
					0, 0, 1, 0));
			c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction4.img/effect/cannonshooter/balloon/2", 5000,
					0, 0, 1, 0));
		...
		case cygnus_Minimap:
			c.getSession()
					.write(EffectPacket.TutInstructionalBalloon("Effect/OnUserEff.img/guideEffect/cygnusTutorial/0"));
			break;
		case check_q20833:
		...
		case PRaid_Revive: // likely to subtract points or remove a life, but
							// idc rly
		case metro_firstSetting:
		case blackSDI:
		case summonIceWall:
		...
		case np_tuto_0_5: {
			// NPCConversationManager cmnp_tuto_0_5= new
			// NPCConversationManager(c, 0, 0, (byte)0, null);
			c.getSession().write(UIPacket.IntroEnableUI(1));
		...
		case merTutorDrecotion10: {
			...
			// c.getSession().write(UIPacket.getDirectionStatus(true));
			// c.getSession().write(UIPacket.IntroEnableUI(1));
			// c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/6",
			// 2000, 0, -100, 1, 0));
		...
		case merTutorDrecotion20: {
			c.getSession().write(UIPacket.getDirectionStatus(true));
			c.getSession().write(UIPacket.IntroEnableUI(1));
			c.getSession().write(UIPacket.getDirectionInfo("Effect/Direction5.img/effect/mercedesInIce/merBalloon/9",
					2000, 0, -100, 1, 0));
			c.getSession().write(UIPacket.getDirectionInfo(1, 2000));
			c.getPlayer().setDirection(0);
			break;
		...
		case mPark_stageEff:
			...
			case 3:
				...
				c.getSession().write(CField
						.MapEff("monsterPark/stageEff/number/" + (((c.getPlayer().getMapId() % 1000) / 100) + 1)));
				break;
			case 4:
		...
		case enter_masRoom: {
			if (c.getPlayer().getQuestStatus(23213) == 1 && c.getPlayer().getQuestStatus(23214) != 1
				...
				final MapleMap mapp = c.getChannelServer().getMapFactory().getMap(931050120); // exit
																								// Map
		...
		case enter_23214: {
			 ...
			 * c.getSession().write(UIPacket.IntroDisableUI(true));
			 * c.getSession().write(CWvsContext.getTopMsg(
			 * "The Secret Garden Depths"));
		...
		case dublTuto21: {
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9300522),
					new Point(-578, 152));
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9300521),
					new Point(-358, 152));
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9300522),
					new Point(-138, 152));
		...
		case dublTuto23: {
			...
			c.getPlayer().getMap().spawnMonsterOnGroundBelow(MapleLifeFactory.getMonster(9300523),
					new Point(-283, 152));
			break;
		}
		...
		case np_tuto_0_0: {
			...
			try {
				Thread.sleep(2000);
				c.getSession().write(UIPacket.getDirectionInfo("Effect/DirectionNewPirate.img/newPirate/balloonMsg2/0",
						0, 0, -100, 1, 0));
				c.getSession().write(CField.UIPacket.getDirectionInfo((byte) 1, 2000));
				Thread.sleep(2000);
		...
		case PTtutor000: {
			try {
				...
				c.getSession().write(CField.UIPacket.getDirectionInfo((byte) 1, 1000));
				Thread.sleep(1000);
				c.getSession().write(UIPacket.getDirectionInfo("Effect/OnUserEff.img/questEffect/phantom/tutorial",
						2000, 0, -100, 1, 0));
				c.getSession().write(CField.UIPacket.getDirectionInfo((byte) 3, 0));
				c.getSession().write(CField.UIPacket.getDirectionInfo((byte) 1, 1000));
				Thread.sleep(1000);
		...
		case PTtutor300: {
			...
			// 99 71 00 01 12 EA 8D 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00
			// 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 E0 13 00 00 00 00 00
			// 88 01 00 00 00 00 00 00 00 4C 73 00 00 00 00 00 00 00 00 4C 73 00
			// 00 00 00 00 00 00 00 4C 73 00 00 00 00 00 00 00 00 4C 73 00 01 00
			// 00 00 FE F7 4C FB 02 30 00 30 00 FF FF 80 66 AB 13 00 00 00 00 D2
			// F6 FF FF 2A F9 FF FF 96 00 00 00 64 00 00 00 00 00 00 00 00 00 00
		...
		case PTjob2M: {
			...
			if (c.getPlayer().getMap().getMobsSize() < 1) {
				c.getPlayer().getMap().spawnMonsterOnGroudBelow(MapleLifeFactory.getMonster(9001045),
						new Point(171, 182));
			}
			break;
		}
		...
		case PTjob4M: {
			if (c.getPlayer().getQuestStatus(25120) == 1) {// &&
															// c.getPlayer().getQuestStatus(25101)!=1
															// &&
		...
		case q53244_dun_in: {
			...
			c.getSession().write(UIPacket.IntroEnableUI(1));
			c.getSession().write(UIPacket.getDirectionInfo("Effect/DirectionNewPirate.img/newPirate/balloonMsg2/11",
					2000, 0, 1, -100, 1));
			for (int i = 0; i < 10; i++) {
			...
			EventTimer.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					c.getSession().write(UIPacket.getDirectionInfo(3, 0));
					// c.getSession().write(UIPacket.getDirectionInfo(4,
					// 1403002));
		...
		case check_count: {
			if (c.getPlayer().getMapId() == 950101010
					&& (!c.getPlayer().haveItem(4001433, 20) || c.getPlayer().getLevel() < 50)) { // ravana
																									// Map
				final MapleMap mapp = c.getChannelServer().getMapFactory().getMap(950101100); // exit
																								// Map
				c.getPlayer().changeMap(mapp, mapp.getPortal(0));
			}
			break;
		...
		case azwan_stageEff: {
			// c.getSession().write(CWvsContext.getTopMsg("Remove all the
			// monsters in the field need to be able to move to the next
			// stage."));
			switch ((c.getPlayer().getMapId() % 1000) / 100) {
			case 1:
			case 2:
			case 3:
				...
				c.getSession().write(
						CField.showEffect("aswan/stageEff/number/" + (((c.getPlayer().getMapId() % 1000) / 100))));
				break;
			}
		...
		default: {
			System.out.println(
					"Unhandled script : " + scriptName + ", type : onUserEnter - MAPID " + c.getPlayer().getMapId());
	...
	private static void reloadWitchTower(MapleClient c) {
		...
		oms.setOHp((long) Math.ceil(theMob.getMobMaxHp() * (level / 5.0))); // 10k
																			// to
																			// 4m
		theMob.setOverrideStats(oms);
		...
		if (di != null && di.eventQ.size() > 0) {
			if (start) {
				...
				for (String s : di.eventQ) {
					...
					case merTutorDrecotion01: // direction info: 1 is probably
												// the time
						c.getSession().write(UIPacket.getDirectionInfo(
