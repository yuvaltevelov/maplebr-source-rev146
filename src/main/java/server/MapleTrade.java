...
public class MapleTrade {
	...
	public final void CompleteTrade() {
		...
		if (this.exchangeMeso > 0) {
			((MapleCharacter) this.chr.get())
					.gainMeso(this.exchangeMeso - GameConstants.getTaxAmount(this.exchangeMeso), false, false);
		}
		this.exchangeMeso = 0;

		((MapleCharacter) this.chr.get()).getClient().getSession()
				.write(CField.InteractionPacket.TradeMessage(this.tradingslot, (byte) 7));
	}
	...
	public final void setMeso(int meso) {
		...
		if (((MapleCharacter) this.chr.get()).getMeso() >= meso) {
			...
			this.meso += meso;
			((MapleCharacter) this.chr.get()).getClient().getSession()
					.write(CField.InteractionPacket.getTradeMesoSet((byte) 0, this.meso));
			if (this.partner != null) {
				this.partner.getChr().getClient().getSession()
						.write(CField.InteractionPacket.getTradeMesoSet((byte) 1, this.meso));
			}
		}
	}
	...
	public final void addItem(Item item) {
		...
		this.items.add(item);
		((MapleCharacter) this.chr.get()).getClient().getSession()
				.write(CField.InteractionPacket.getTradeItemAdd((byte) 0, item));
		if (this.partner != null) {
			this.partner.getChr().getClient().getSession()
					.write(CField.InteractionPacket.getTradeItemAdd((byte) 1, item));
		}
	}
	...
	public final void chat(String message) throws Exception {
		if (!CommandProcessor.processCommand(chr.get().getClient(), message, ServerConstants.CommandType.TRADE)) {
			((MapleCharacter) this.chr.get()).dropMessage(-2,
					((MapleCharacter) this.chr.get()).getName() + " : " + message);
			if (this.partner != null) {
				this.partner.getChr().getClient().getSession().write(
						PlayerShopPacket.shopChat(((MapleCharacter) this.chr.get()).getName() + " : " + message, 1));
			}
		}
		if (((MapleCharacter) this.chr.get()).getClient().isMonitored()) {
			World.Broadcast.broadcastGMMessage(CWvsContext.broadcastMsg(6, ((MapleCharacter) this.chr.get()).getName()
	...
	public final boolean setItems(MapleClient c, Item item, byte targetSlot, int quantity) {
		...
		if ((this.partner == null) || (target == -1) || (GameConstants.isPet(item.getItemId())) || (isLocked())
		...
		if (this.exchangeItems != null) {
			...
			for (Item item : this.exchangeItems) {
				...
				if ((ii.isPickupRestricted(item.getItemId()))
						&& (((MapleCharacter) this.chr.get()).haveItem(item.getItemId(), 1, true, true))) {
					return 2;
				}
			}
			if ((((MapleCharacter) this.chr.get()).getInventory(MapleInventoryType.EQUIP).getNumFreeSlot() < eq)
					|| (((MapleCharacter) this.chr.get()).getInventory(MapleInventoryType.USE).getNumFreeSlot() < use)
					|| (((MapleCharacter) this.chr.get()).getInventory(MapleInventoryType.SETUP)
							.getNumFreeSlot() < setup)
					|| (((MapleCharacter) this.chr.get()).getInventory(MapleInventoryType.ETC).getNumFreeSlot() < etc)
					|| (((MapleCharacter) this.chr.get()).getInventory(MapleInventoryType.CASH)
							.getNumFreeSlot() < cash)) {
				return 1;
			}
		}
	...
	public static final void startTrade(MapleCharacter c) {
		if (c.getTrade() == null) {
			...
			c.getClient().getSession()
					.write(CField.InteractionPacket.getTradeStart(c.getClient(), c.getTrade(), (byte) 0));
		} else {
	...
	public static final void inviteTrade(MapleCharacter c1, MapleCharacter c2) {
		...
		if ((c2 != null) && (c2.getTrade() == null)) {
			...
			c1.getClient().getSession()
					.write(CWvsContext.broadcastMsg(5, "The other player is already trading with someone else."));
	...
	public static final void visitTrade(MapleCharacter c1, MapleCharacter c2) {
		if ((c2 != null) && (c1.getTrade() != null) && (c1.getTrade().getPartner() == c2.getTrade())
			...
			c1.getClient().getSession()
					.write(CField.InteractionPacket.getTradeStart(c1.getClient(), c1.getTrade(), (byte) 1));
			...
			c1.getClient().getSession()
					.write(CWvsContext.broadcastMsg(5, "The other player has already closed the trade"));
		}
	}
