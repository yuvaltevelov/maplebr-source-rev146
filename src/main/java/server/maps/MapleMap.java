...
public final class MapleMap {
	...
	public final void checkClockContact(final MapleCharacter chr, final MapleMonster monster) {
		...
		if (this.getFieldType().equals("63")) {
			for (MapleMapObject object : this.getMapObjectsInRange(chr.getPosition(), Double.POSITIVE_INFINITY,
					Arrays.asList(MapleMapObjectType.MIST))) {
				final MapleMist clock = (MapleMist) object;
				// System.out.println("미스트!");
				if (clock.isClock()) {
					// System.out.println("시계일시");
					Rectangle rect = new Rectangle(clock.getBox().x, clock.getBox().y, clock.getBox().width,
							clock.getBox().height);
					// if (!clock.isUsed()) {
	...
	private void dropFromMonster(final MapleCharacter chr, final MapleMonster mob, final boolean instanced) {
		if (mob == null || chr == null || ChannelServer.getInstance(channel) == null || dropsDisabled
				|| mob.dropsDisabled() || chr.getPyramidSubway() != null) { // no
																			// drops
																			// in
																			// pyramid
																			// ok?
		...
		if (!instanced && mapobjects.get(MapleMapObjectType.ITEM).size() >= 255) {
			removeDrops();
			broadcastMessage(
					CField.getGameMessage("The drops on the map were cleared in order to reduce lags.", (short) 7));
		}
		...
		// final int caServerrate =
		// ChannelServer.getInstance(channel).getCashRate();
		...
		final int cashModifier = (int) ((mob.getStats().isBoss()
				...
				: (mob.getMobExp() / 1000 + mob.getMobMaxHp() / 20000))); // no
																			// rate
		// if (Randomizer.nextInt(100) < 50) {
		// chr.modifyCSPoints(1, (int) ((Randomizer.nextInt(cashz) + cashz +
		// cashModifier) * (chr.getStat().cashBuff / 100.0) * chr.getCashMod()),
		// true);
		// }
		Item idrop;
		byte d = 1;
		...
		for (final MonsterDropEntry de : dropEntry) {
			...
			if (Randomizer.nextInt(999999) < (int) (de.chance * chServerrate * chr.getDropMod() * chr.getStat().dropBuff
				...
				if (mesoDropped && droptype != 3 && de.itemId == 0) { // not
																		// more
																		// than
																		// 1
																		// sack
			...
			if (Randomizer.nextInt(999999) < de.chance
				...
				if (de.itemId == 0) {
					...
					if (mons.getLevel() > 100) {
						spawnMobMesoDrop(
								(int) (5 + (mons.getLevel() * 10) * (chr.getStat().mesoBuff / 100.0) * chr.getDropMod()
										* cmServerrate),
								calcDropPos(pos, mob.getPosition()), mob, chr, false, droptype);
					} else {
						spawnMobMesoDrop(
								(int) (5 + (mons.getLevel()) * (chr.getStat().mesoBuff / 100.0) * chr.getDropMod()
										* cmServerrate),
								calcDropPos(pos, mob.getPosition()), mob, chr, false, droptype);
					}
					...
					if (GameConstants.getInventoryType(de.itemId) == MapleInventoryType.EQUIP) {
						...
						idrop = new Item(de.itemId, (byte) 0, (short) (de.Maximum != 1
								? Randomizer.nextInt(de.Maximum - de.Minimum) + de.Minimum : 1), (byte) 0);
					}
	...
	public void killMonster(final MapleMonster monster) { // For mobs with
															// removeAfter
		if (monster == null) {
			return;
		}
	...
	public final void killMonster(final MapleMonster monster, final MapleCharacter chr, final boolean withDrops,
		...
		if (monster.getId() == 9400902) {
			...
			chr.getMap().startMapEffect(
					"So what do you think? Just as though as the original, right?! Move by going to Resercher H. Your first reward is this soundtrack",
					5120039);
		...
		if (monster.getId() == 8820014) { // pb sponge, kills pb(w) first before
											// dying
			killMonster(8820000);
					...
					if (mc.isAlive()) {
						...
						case 8820001:
							mc.getClient().getSession()
									.write(EffectPacket.showOwnBuffEffect(buffid, 13, mc.getLevel(), 1)); // HT
																											// nine
																											// spirit
		...
		if (mobid == 8810018 && mapid == 240060200) { // Horntail
			World.Broadcast.broadcastMessage(CWvsContext.broadcastMsg(6,
		...
		final int cashModifier = (int) ((monster.getStats().isBoss()
				? (monster.getStats().isPartyBonus() ? (Randomizer.nextInt(6)) : (Randomizer.nextInt(2)))
				: (Randomizer.nextInt(2)))); // no rate
	}
	...
	public final void spawnPinkZakum(final int x, final int y) {
		...
		final int[] zakpart = { 9400903, 9400904, 9400905, 9400906, 9400907, 9400908, 9400909, 9400910 };
		...
		spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
			...
			public final void sendPackets(MapleClient c) {
				if (GameConstants.isAzwanMap(c.getPlayer().getMapId())) {
					c.getSession()
							.write(MobPacket.spawnMonster(monster,
									monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(),
									oid, true)); // TODO effect
				} else {
					c.getSession()
							.write(MobPacket.spawnMonster(monster,
									monster.getStats().getSummonType() <= 1 ? -3 : monster.getStats().getSummonType(),
									oid, false)); // TODO effect
				}
			}
		});
	...
	/*
	 * public final MapleCharacter getLevel() { return getLevel(); }
	 */
		...
		if (monster.getId() == 9300166) {
			MapTimer.getInstance().schedule(new Runnable() {
				...
				public void run() {
					...
					/* Bomberman */ : this.getId() == 910025200 ? 200
							/* The Dragon's Shout */ : 3000);
		}
		/*
		 * if (MoonlightRevamp.MoonlightRevamp) { MapleMonsterStats stats =
		 * MapleLifeFactory.getMonsterStats(monster.getId());
		 * OverrideMonsterStats overrideStats = new
		 * OverrideMonsterStats(monster.getHp() /
		 * MoonlightRevamp.monsterHpDivision, monster.getMp(), stats.getExp());
		 * monster.setOverrideStats(overrideStats);
		 * //monster.setHp(monster.getHp() / MoonlightRevamp.monsterHpDivision);
		 * //Isn't needed because of override stats
		 * stats.setPDRate(stats.isBoss() ?
		 * MoonlightRevamp.getBossPDRateMultipy(stats.getPDRate()) : (byte)
		 * (stats.getPDRate() +
		 * MoonlightRevamp.getPDRateAddition(stats.getPDRate())));
		 * stats.setMDRate(stats.isBoss() ?
		 * MoonlightRevamp.getBossMDRateMultipy(stats.getMDRate()) : (byte)
		 * (stats.getMDRate() +
		 * MoonlightRevamp.getMDRateAddition(stats.getMDRate()))); }
		 */
		spawnAndAddRangedMapObject(monster, new DelayedPacketCreation() {
			@Override
			public final void sendPackets(MapleClient c) {
				if (GameConstants.isAzwanMap(c.getPlayer().getMapId())) {
					...
					c.getSession()
							.write(MobPacket
									.spawnMonster(monster,
											monster.getStats().getSummonType() <= 1
													|| monster.getStats().getSummonType() == 27 || true ? spawnType
															: monster.getStats().getSummonType(),
											0, false));
				} else {
					c.getSession()
	...
	public final void spawnMobMesoDrop(final int meso, final Point position, final MapleMapObject dropper,
		...
		spawnAndAddRangedMapObject(mdrop, new DelayedPacketCreation() {
			...
			public void sendPackets(MapleClient c) {
				c.getSession()
						.write(CField.dropItemFromMapObject(mdrop, dropper.getTruePosition(), position, (byte) 1));
			}
		});
	...
	public final void spawnMobDrop(final Item idrop, final Point dropPos, final MapleMonster mob,
		...
		// broadcastMessage(CField.dropItemFromMapObject(mdrop,
		// mob.getTruePosition(), dropPos, (byte) 0));
	...
	public final void spawnRandDrop() {
		...
		MapTimer.getInstance().schedule(new Runnable() {
			...
			public void run() {
				...
				int itemid = 0;
				if (theItem < 950) { // 0-949 = normal, 950-989 = rare, 990-999
										// = super
	...
	public final void addPlayer(final MapleCharacter chr) {
		...
		/*
		 * for (int npc : GameConstants.unusedNpcs) { if (getNPCById(npc) !=
		 * null && getId() != 910000000) {
		 * chr.getClient().getSession().write(CField.NPCPacket.toggleNPCShow(
		 * getNPCById(npc).getObjectId(), true)); //hide unused npcs } }
		 */
		...
		if (GameConstants.isKanna(chr.getJob())) {
			...
			if (chr.getHaku() != null) {
				if (chr.getBuffSource(MapleBuffStat.HAKU_REBORN) > 0) {
					...
					chr.getMap().broadcastMessage(chr, CField.spawnHaku_bianshen(chr.getId(),
							chr.getHaku().getObjectId(), chr.getHaku().getStats()), true);
				} else {
		...
		if (permanentWeather > 0) {
			chr.getClient().getSession().write(CField.startMapEffect("", permanentWeather, false)); // snow,
																									// no
																									// msg
		}
		...
		// if (partyBonusRate > 0) {
		// chr.dropMessage(-1, partyBonusRate + "% additional EXP will be
		// applied per each party member here.");
	...
	public void doShrine(final boolean spawned) { // false = entering map, true
													// = defeated
		if (squadSchedule != null) {
		...
		if (sqd != null && em != null && getCharactersSize() > 0) {
			...
			if (!spawned) { // no monsters yet; inforce timer to spawn it
							// quickly
				...
				run = new Runnable() {
					...
					public void run() {
						...
						if (MapleMap.this.getCharactersSize() > 0 && MapleMap.this.getNumMonsters() == monsterz.size()
							...
							if (passed) {
								// are we still the same squad? are monsters
								// still == 0?
								byte[] packet;
								if (mode == 1 || mode == 2) { // chaoszakum
									packet = CField.showChaosZakumShrine(spawned, 0);
								} else {
									packet = CField.showHorntailShrine(spawned, 0); // chaoshorntail
																					// message
																					// is
																					// weird
								}
								for (MapleCharacter chr : MapleMap.this.getCharactersThreadsafe()) { // warp
																										// all
																										// in
																										// map
									chr.getClient().getSession().write(packet);
									chr.changeMap(returnMapz, returnMapz.getPortal(0)); // hopefully
																						// event
																						// will
																						// still
																						// take
				...
				run = new Runnable() {
					...
					public void run() {
						...
						// we dont need to stop clock here because they're
						// getting warped out anyway
						if (MapleMap.this.getCharactersSize() > 0 && sqnow != null && sqnow.getStatus() == 2
								&& sqnow.getLeaderName().equals(leaderName)
								&& MapleMap.this.getEMByMap().getProperty("state").equals(state)) {
							// are we still the same squad? monsters however
							// don't count
							byte[] packet;
							if (mode == 1 || mode == 2) { // chaoszakum
								packet = CField.showChaosZakumShrine(spawned, 0);
			...
			squadSchedule = MapTimer.getInstance().schedule(run, 300000); // 5
																			// mins
		}
	}
	...
	public final void loadMonsterRate(final boolean first) {
		...
		if (first && spawnSize > 0) {
			...
			respawn(false); // this should do the trick, we don't need to wait
							// upon entering map
		}
	}
	...
	public void respawn(final boolean force, final long now) {
		...
		if (force) { // cpq quick hack
			...
			final int numShouldSpawn = (GameConstants.isForceRespawn(mapid)
					? monsterSpawn.size() * MoonlightRevamp.monsterSpawn
					: maxRegularSpawn * MoonlightRevamp.monsterSpawn) - spawnedMonstersOnMap.get();
			if (numShouldSpawn > 0) {
				int spawned = 0;
				...
				for (Spawns spawnPoint : randomSpawn) {
					...
					if (spawnPoint.shouldSpawn(lastSpawnTime) || GameConstants.isForceRespawn(mapid)
							|| (monsterSpawn.size() * MoonlightRevamp.monsterSpawn < 10 && maxRegularSpawn
									* MoonlightRevamp.monsterSpawn > monsterSpawn.size() * MoonlightRevamp.monsterSpawn
							&& partyBonusRate > 0)) {
						spawnPoint.spawnMonster(this);
						spawned++;
					}
	...
	public void checkStates(final String chr) {
		...
		// broadcastMessage(CWvsContext.broadcastMsg(5, "The queued player " +
